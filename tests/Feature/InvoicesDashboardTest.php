...
class InvoicesDashboardTest extends TestCase
{
    ...
    public function test_dashboard_route_uses_the_expected_contract_and_preserves_index_redirect(): void
    {
        ...
        $this->actingAs($admin, 'admin')->get(route('admin.invoices.dashboard'))->assertForbidden();
        $viewer = $this->adminWithPermissions(['invoices-list']);
        $this->actingAs($viewer, 'admin')->get(route('admin.invoices.index'))->assertRedirect(route('admin.invoices.hoadon-list'));
        $superAdmin = User::factory()->create();
        $superAdmin->assignRole(Role::findOrCreate('Super Admin', 'admin'));
        $this->actingAs($superAdmin->fresh(), 'admin')->get(route('admin.invoices.dashboard'))->assertOk();
    }
    ...
    public function test_dashboard_renders_capability_aware_operations_center_without_sensitive_data_or_remote_calls(): void
    {
        ...
        InvoiceFile::query()->create(['invoice_id' => $invoice->id, 'provider' => 'local', 'status' => 'error', 'path' => 'private/secret-invoice-path.pdf', 'last_error' => 'raw-pdf-error-secret']);
        InvoiceBackupRun::query()->create(['mode' => 'automatic', 'status' => 'failed', 'recipient' => 'backup-secret@example.test', 'files' => [['name' => 'secret-file.pdf', 'fingerprint' => 'secret-fingerprint']], 'message' => 'raw-backup-error-secret', 'started_at' => now()->subMinute(), 'finished_at' => now()]);
        config(['invoices.gdt.username' => 'gdt-user-secret', 'invoices.gdt.password' => 'gdt-password-secret', 'invoices.gdt.cache_key' => 'dashboard-gdt-token']);
        Cache::put('dashboard-gdt-token', 'gdt-token-secret', 600);

        $viewer = $this->adminWithPermissions(['invoices-list']);
        $this->actingAs($viewer, 'admin')->get(route('admin.invoices.dashboard'))->assertOk()->assertSee('Dashboard hóa đơn')->assertSee('Invoices operations center')->assertSee('Danh sách hóa đơn')->assertSee('Tổng hợp đối tác')->assertDontSee('href="'.route('admin.invoices.hoadon').'"', false)->assertDontSee('href="'.route('admin.invoices.backup-restore').'"', false)->assertDontSee('PDF đã lưu')->assertDontSee('Backup gần đây');

        $operator = $this->adminWithPermissions(['invoices-list', 'invoices-create', 'invoices-export', 'invoices-download', 'invoices-configure']);
        $response = $this->actingAs($operator, 'admin')->get(route('admin.invoices.dashboard'))->assertOk()
            ...
            ->assertSee('PDF đã lưu')
            ->assertSee('PDF chưa hoàn tất')
        ...
        foreach (['lookup-secret-1', 'INV-SECRET-1', 'Sensitive Partner 1', 'tax-secret-1', 'sensitive-1@example.test', 'private/secret-invoice-path.pdf', 'raw-pdf-error-secret', 'backup-secret@example.test', 'secret-file.pdf', 'secret-fingerprint', 'raw-backup-error-secret', 'gdt-user-secret', 'gdt-password-secret', 'gdt-token-secret', '12345.67'] as $secret) {
    ...
    {
        $admin = $this->adminWithPermissions(['invoices-list', 'invoices-download', 'invoices-configure']);
        foreach (range(1, 7) as $index) {
            $invoice = $this->createInvoice($index, $index % 2 === 0 ? 'purchase' : 'sold');
            if ($index === 1) {
                InvoiceFile::query()->create(['invoice_id' => $invoice->id, 'provider' => 'local', 'status' => 'available', 'path' => 'private/dashboard-file-secret.pdf']);
            }
            if ($index === 2) {
                InvoiceFile::query()->create(['invoice_id' => $invoice->id, 'provider' => 'gdt', 'status' => 'error', 'last_error' => 'dashboard-pdf-error-secret']);
            }
            InvoiceBackupRun::query()->create(['mode' => $index % 2 === 0 ? 'manual' : 'automatic', 'status' => $index === 7 ? 'failed' : 'success', 'recipient' => "backup-{$index}@example.test", 'files_count' => $index, 'emails_sent' => 1, 'files' => [['name' => "backup-secret-{$index}.pdf"]], 'message' => "backup-message-secret-{$index}", 'started_at' => now()->subMinutes(7 - $index), 'finished_at' => now()->subMinutes(7 - $index)]);
        }
    ...
    public function test_dashboard_renders_unavailable_and_empty_states_without_querying_missing_tables(): void
    {
        ...
        $this->actingAs($viewer, 'admin')->get(route('admin.invoices.dashboard'))->assertOk()->assertSee('nhóm dữ liệu chưa sẵn sàng')->assertSee('Chưa có hoạt động hóa đơn để hiển thị')->assertSee('Chưa có dữ liệu trạng thái PDF');
    }
    ...
    {
        return Invoices::query()->create(['lookup_code' => "lookup-secret-{$index}", 'symbol' => 'C26TAA', 'invoice_number' => "INV-SECRET-{$index}", 'type' => 'VAT', 'issued_date' => now()->subDays($index), 'tax_code' => "tax-secret-{$index}", 'name' => "Sensitive Partner {$index}", 'address' => "Sensitive Address {$index}", 'email' => "sensitive-{$index}@example.test", 'phone' => '0900000000', 'tax_rate' => 10, 'vat_amount' => 12345.67, 'amount_before_vat' => 123456.78, 'total_amount' => 135802.45, 'invoice_type' => $type]);
    }
}
