...
class AdminHeaderConfigurationContractTest extends TestCase
{
    public function test_header_defaults_define_brand_user_menu_actions_presentation_and_responsive_groups(): void
    {
        ...
        foreach (['brand', 'user_menu_config', 'actions', 'presentation', 'responsive'] as $group) $this->assertStringContainsString("'{$group}' => [", $config);
        foreach (['logo_size', 'show_title', 'show_subtitle', 'show_avatar', 'show_name', 'show_email', 'show_role', 'mobile_overflow', 'action_gap', 'backdrop_blur', 'mobile_brand', 'overflow_secondary_actions'] as $key) $this->assertStringContainsString("'{$key}'", $config);
    }
