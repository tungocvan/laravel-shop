...
### MR-04 — Submission and sequential single approval

| Traceability | Implementation evidence | Automated evidence | Result |
|---|---|---|---|
| `REQ-02`, `REL-01`, `SEC-04` atomic submission | `SubmitInternalRequest` locks the aggregate, revalidates audience/type/schema/version, creates an immutable submit revision/run, activates stage one, advances runtime pointers, and appends audit/outbox through one idempotent transaction | `tests/Feature/Request/Submission/RequestSubmissionServiceTest.php` | Implemented for RS-01..04; failed submit leaves no partial runtime rows |
| `APR-01`, `APR-02`, `APR-03` approved actor resolution | Request-owned `ActorResolverRegistry` exposes only fixed users, role members, and validated form-user-field resolvers through User/Role Shell contracts; `ApprovalStageActivator` removes self, normalizes cardinality, and snapshots the effective candidate | `RequestSubmissionServiceTest.php` resolver/self-only/late-role tests | Implemented for MR-04 single stages; parallel evaluation and activation recovery remain MR-05 |
| `REQ-03`, `APR-04` sequential single decisions | `DecideRequestTask` enforces aggregate/run/stage/task/candidate state, canonical lock ordering, expected versions, idempotency, immutable decisions, next-stage activation, and final approval | `RequestSubmissionServiceTest.php` AP-01/AP-07/AP-09 and duplicate-decision cases | Implemented for sequential `single` approve; reject/return/parallel/recovery remain MR-05 |
| `DAT-01`, `DAT-02` approval evidence persistence | Migrations 5–6 add tasks, candidate snapshots, immutable decisions, current payload/run pointers, and the audit instance link with restrictive relationships and inbox indexes | `RequestDefinitionMigrationTest.php`; isolated fresh/migrate/rollback validation | Implemented for the MR-04 schema slice |
| `API-01`, `API-02`, `SEC-01` transport parity and visibility | Livewire submit/decision and Sanctum API submit/inbox/decision call the same services and policies; API permissions are checked against the canonical `admin` guard and query scopes hide foreign records | `tests/Feature/Request/Submission/RequestSubmissionTransportTest.php` | Implemented for MR-04 commands with bounded API pagination and IDOR denial |
| `UX-01`, `UX-03` mobile approver surface | Request detail adds explicit submit review; inbox uses bounded search/reset/page sizes and mobile cards; approval requires a confirmation dialog and authoritative server commit | `RequestSubmissionTransportTest.php`, `RequestBootstrapTest.php` | Implemented for online submit and single approval; complete lifecycle/timeline and offline states remain later slices |
| `PERF-01` bounded inbox | `ApproverInboxQuery` uses assignee/status/stage scopes and eager-loaded safe request summaries without role resolution per row | `RequestSubmissionServiceTest::test_inbox_query_count_is_bounded_for_twenty_five_tasks` | Passes the <=15-query budget for 25 rows on test SQLite; production-like EXPLAIN remains a release gate |
| `ARC-02`, `ARC-04`, `ARC-05` architecture invariants | Request remains default OFF, imports only approved Shell contracts, uses root resource discovery once, and adds no Workflow/MR-05 lifecycle implementation | `tests/Feature/Request/Architecture/*`, module-state tests, repository scans | Preserved |

MR-04 intentionally implements only submit and sequential `single` approval. It does not implement parallel ALL/ANY, reject, return/resubmit, cancellation of pending requests, reassignment, failed-activation recovery, collaboration/files, notifications, offline persistence, or Workflow runtime behavior.

### MR-06 — Collaboration, private files, and audit UI

| Traceability | Implementation evidence | Automated evidence | Result |
|---|---|---|---|
| `DAT-01`, `REL-01` collaboration/delivery persistence | Migration 7 adds comments, attachments, and the reserved export/notification delivery state tables with restrictive evidence links, opaque paths, bounded metadata, and logical-delivery/idempotency uniqueness | `RequestDefinitionMigrationTest::test_mr_06_adds_collaboration_and_delivery_capability_tables`, factory and rollback/remigrate tests | Implemented for MR-06 persistence; export and notification execution remain inactive until their owning slices |
| `SEC-01`, `SEC-04` scoped collaboration and audit | Comment, attachment, and audit policies combine named permissions with owner/participant/admin visibility; comments are immutable except explicit redaction metadata; request audit queries expose only timeline fields | `tests/Feature/Request/Collaboration/RequestCollaborationTest.php` CO-01/02, RF-05, audit-scope cases | Implemented for comments, attachment field ownership, escaped display, redaction, and instance-scoped audit timeline |
| `SEC-02` private file lifecycle | `UploadRequestAttachment` validates count/size/extension/MIME/signature/Office-package structure, stores opaque names on a non-public disk, and keeps configured-scanner uploads unavailable until clean; download reauthorizes and streams with no-store/nosniff headers | `RequestCollaborationTest` CO-03..05 and file attack corpus | Implemented for private upload/quarantine/download and IDOR/public-path denial; configured scanner execution remains optional and is not fabricated |
| `API-01`, `UX-01`, `UX-03` collaboration transport/UI | Livewire comment, attachment, and bounded audit timeline components share application services with Sanctum comment/upload endpoints and responsive stack/card states | `RequestCollaborationTest::test_api_comment_and_upload_use_same_private_services`, Request Livewire/bootstrap regression suite | Implemented for the online MR-06 surface; browser viewport/manual accessibility evidence remains a release gate |
