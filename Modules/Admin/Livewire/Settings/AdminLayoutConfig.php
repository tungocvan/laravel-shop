...
class AdminLayoutConfig extends Component
{
    ...
    {
        $rules = [
            'general' => ['config.locale' => 'required|in:vi,en', 'config.layout.preset' => 'required|in:default,data-heavy,focus,settings', 'config.layout.container' => 'required|in:full,narrow,7xl,screen-2xl', 'config.layout.density' => 'required|in:comfortable,compact,dense', 'config.layout.sticky_header' => 'boolean'],
    ...
    private function sectionPayload(array $config): array
    {
        return match ($this->section) {
            'general' => ['locale' => data_get($config, 'locale'), 'layout' => ['preset' => data_get($config, 'layout.preset'), 'container' => data_get($config, 'layout.container'), 'density' => data_get($config, 'layout.density'), 'sticky_header' => data_get($config, 'layout.sticky_header')]],
