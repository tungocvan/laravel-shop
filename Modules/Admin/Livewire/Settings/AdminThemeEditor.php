...
class AdminThemeEditor extends Component
{
    ...
    public function mount(AdminLayoutManager $layoutManager, AdminThemeProfileService $profileService, ThemeManager $themeManager, AdminDesignService $designService): void
    {
        ...
        $this->normalizeSidebarPresentationCompatibility();
    ...
    public function render(AdminDesignService $designService)
    {
        return view('Admin::livewire.settings.admin-theme-editor', [
            ...
            'fontFamilyOptions' => $designService->fontFamilyOptions(),
    ...
    {
        // Regional design colors no longer mutate Sidebar presentation mode.
        // Sidebar mode is owned by the dedicated Sidebar visual settings.
    }

    private function normalizeSidebarPresentationCompatibility(): void
    {
        $mode = (string) data_get($this->config, 'sidebar.presentation.background', 'theme');

        data_set($this->config, 'sidebar.presentation.background', match ($mode) {
            'system', 'white' => 'light',
            default => $mode,
        });
    }
