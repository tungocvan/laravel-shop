...
class AdminDesignService
{
    ...
    private const MENU_FONT_FAMILIES = ['inherit' => 'inherit', 'sans' => 'ui-sans-serif, system-ui, sans-serif', 'serif' => 'ui-serif, Georgia, serif', 'mono' => 'ui-monospace, SFMono-Regular, Menlo, monospace'];
    ...
    public function menuFontFamilyOptions(): array { return ['inherit'=>'Kế thừa font Admin','sans'=>'Sans Serif','serif'=>'Serif','mono'=>'Monospace']; }
    ...
    private function sanitizeSidebarMenu(array $tokens,array $defaults): array
    {
        ...
        return [
            'item'=>[
                ...
                'title_color'=>$this->allowed(data_get($m,'item.title_color'),$colors,data_get($d,'item.title_color','slate-900')),
                'icon_color'=>$this->allowed(data_get($m,'item.icon_color'),$colors,data_get($d,'item.icon_color','slate-400')),
            ...
            'submenu'=>[
                ...
                'title_color'=>$this->allowed(data_get($m,'submenu.title_color'),$colors,data_get($d,'submenu.title_color','slate-500')),
                'icon_color'=>$this->allowed(data_get($m,'submenu.icon_color'),$colors,data_get($d,'submenu.icon_color','slate-400')),
            ...
            'active'=>['title_color'=>$this->allowed(data_get($m,'active.title_color'),$colors,data_get($d,'active.title_color','white')),'icon_color'=>$this->allowed(data_get($m,'active.icon_color'),$colors,data_get($d,'active.icon_color','white')),'font_weight'=>$this->allowed(data_get($m,'active.font_weight'),array_keys(self::FONT_WEIGHTS),data_get($d,'active.font_weight','semibold'))],
        ];
    }
    ...
    private function allowed(mixed $value,array $allowed,mixed $fallback): mixed { return in_array($value,$allowed,true)?$value:$fallback; }
}
