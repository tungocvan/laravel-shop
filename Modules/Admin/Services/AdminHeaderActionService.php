...
class AdminHeaderActionService
{
    ...
    public function context(): array
    {
        ...
        $notificationBehavior = data_get($header, 'actions.notification.behavior') === 'link' ? 'link' : 'dropdown';

        return [
            'notifications' => (bool) data_get($header, 'notifications', true),
            'notification' => [
                'icon' => $this->safeIcon(data_get($header, 'actions.notification.icon')) ?? self::ICONS['bell'],
                'behavior' => $notificationBehavior,
                'url' => $notificationBehavior === 'link' ? $this->safeUrl(data_get($header, 'actions.notification.url')) : null,
                'target' => data_get($header, 'actions.notification.target') === '_blank' ? '_blank' : '_self',
            ],
            'items' => $items,
    ...
    {
        return collect($items)->filter(fn ($item) => is_array($item) && (bool) data_get($item, 'enabled', true))->filter(fn ($item) => $this->allowed($item, $user))->sortBy(fn ($item) => (int) data_get($item, 'order', 0))->map(fn ($item) => $this->normalizeItem($item))->filter()->values()->all();
    }
    ...
    private function normalizeItem(array $item): ?array
    {
        ...
        if ($title === '' || $url === null || $icon === null) return null;

        return ['title' => mb_substr($title, 0, 80), 'url' => $url, 'icon' => $icon, 'target' => data_get($item, 'target') === '_blank' ? '_blank' : '_self', 'priority' => data_get($item, 'priority') === 'primary' ? 'primary' : 'secondary', 'badge' => $this->badge(data_get($item, 'badge'))];
    }
    ...
    private function safeUrl(mixed $value): ?string
    {
        ...
        if ($url === '') return null;
        if (str_starts_with($url, '/') && ! str_starts_with($url, '//')) return mb_substr($url, 0, 255);
        if (filter_var($url, FILTER_VALIDATE_URL) && in_array(parse_url($url, PHP_URL_SCHEME), ['http', 'https'], true)) return mb_substr($url, 0, 255);
        return null;
    }
    ...
    private function safeIcon(mixed $value): ?string
    {
        ...
        if (array_key_exists($icon, self::ICONS)) return self::ICONS[$icon];
