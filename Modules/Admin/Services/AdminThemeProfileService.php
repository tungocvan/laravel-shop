...
class AdminThemeProfileService
{
    ...
    public function apply(string $name, array $config): array
    {
        ...
        return array_replace_recursive($config, (array) ($profile['payload'] ?? []));
    }
    ...
    public function saveCustom(string $label, array $config): string
    {
        ...
        if (array_key_exists($slug, $this->builtIns())) {
            $slug .= '-custom';
        }
        ...
        $base = $slug;
        $counter = 2;
        while (array_key_exists($slug, $profiles)) {
            $slug = $base.'-'.$counter++;
        }
    ...
    public function extractPayload(array $config): array
    {
        return [
            ...
            'theme' => [
                'default' => data_get($config, 'theme.default', 'soft-light'),
                'dark_mode' => 'class',
                'accent' => data_get($config, 'theme.accent', 'indigo'),
            ],
            'layout' => [
                'surface' => (array) data_get($config, 'layout.surface', []),
            ],
            'sidebar' => [
                'presentation' => [
            ...
            'header' => [
                'presentation' => (array) data_get($config, 'header.presentation', []),
            ],
            'footer' => [
                'presentation' => (array) data_get($config, 'footer.presentation', []),
            ],
        ];
    }
