...
class AdminThemeProfileService
{
    ...
    {
        return $this->storeCustom($label, $this->extractPayload($config), 'Theme tùy chỉnh được lưu từ Theme Editor.');
    }

    public function duplicate(string $name): string
    {
        $profiles = $this->profiles();
        abort_unless(isset($profiles[$name]), 404);
        $source = $profiles[$name];
        return $this->storeCustom('Bản sao '.$source['label'], (array) $source['payload'], 'Bản sao của theme '.$source['label'].'.');
    }
    ...
    public function deleteCustom(string $name): bool
    {
        ...
        if ($this->activeName() === $name) $this->setActive(self::DEFAULT_PROFILE);
        return true;
    }
    ...
    private function storeCustom(string $label, array $payload, string $description): string
    {
        ...
        $profiles[$slug] = ['label' => $label, 'description' => mb_substr($description, 0, 160), 'built_in' => false, 'payload' => $payload];
        Setting::setValue(self::CUSTOM_SETTING, $profiles, 'admin_layout', 'json');
        $this->setActive($slug);
        return $slug;
    }
    ...
    public function extractPayload(array $config): array
    {
        return [
            ...
            'sidebar' => ['presentation' => ['background' => data_get($config, 'sidebar.presentation.background', 'system')]],
    ...
    private function customProfiles(): array
    {
        ...
        return collect($value)->filter(fn ($profile, $key) => is_string($key) && is_array($profile) && isset($profile['payload']))->map(fn (array $profile) => [
    ...
    {
        $baseDesign = ['typography' => ['font_family' => 'sans', 'body_size' => 'sm', 'page_title_size' => '2xl', 'heading_weight' => 'semibold'], 'spacing' => ['tight' => '2', 'control' => '3', 'content' => '4', 'section' => '6'], 'radius' => ['control' => 'lg', 'panel' => 'lg', 'overlay' => 'xl']];
        $shell = fn (array $design, string $sidebarPalette, string $accent = 'indigo', string $sidebarBackground = 'system') => [
            'design' => array_replace_recursive($baseDesign, $design), 'theme' => ['default' => $sidebarPalette, 'dark_mode' => 'class', 'accent' => $accent],
