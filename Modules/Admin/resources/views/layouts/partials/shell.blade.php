...
>
    @if ($sidebarEnabled)
        <button
            ...
            :aria-expanded="(!sidebarFullscreen).toString()"
            :aria-label="sidebarFullscreen ? 'Mở Sidebar toàn màn hình' : 'Ẩn Sidebar để làm việc toàn màn hình'"
            :title="sidebarFullscreen ? 'Mở Sidebar' : 'Ẩn Sidebar toàn màn hình'"
            data-admin-sidebar-fullscreen-toggle
        >
            <svg x-show="!sidebarFullscreen" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M4 5h16v14H4zM9 5v14M14 9l-3 3 3 3" />
            </svg>
            <svg x-cloak x-show="sidebarFullscreen" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
