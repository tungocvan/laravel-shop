@php
    ...
    $mobileOverflow = (bool) data_get($actions, 'mobile_overflow', true)
        && (bool) data_get($actions, 'overflow_secondary_actions', true)
        && count($secondaryActions) > 0;
@endphp
...
<div class="flex shrink-0 items-center gap-1.5 sm:gap-2" data-admin-header-actions>
    ...
    @foreach ($primaryActions as $action)
        <a href="{{ $action['url'] }}" target="{{ $action['target'] ?? '_self' }}"
            ...
            @if (! empty($action['badge']))
                <span class="absolute right-0.5 top-0.5 min-w-4 rounded-full bg-rose-500 px-1 text-center text-[9px] font-bold leading-4 text-white">{{ $action['badge'] }}</span>
            @endif
        </a>
    @endforeach
    ...
    @foreach ($secondaryActions as $action)
        <a href="{{ $action['url'] }}" target="{{ $action['target'] ?? '_self' }}"
            ...
            class="relative h-10 w-10 shrink-0 items-center justify-center rounded-xl text-slate-500 transition duration-200 hover:bg-slate-100 hover:text-slate-900 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 {{ $mobileOverflow ? 'hidden sm:inline-flex' : 'inline-flex' }}">
            <i class="{{ $action['icon'] }} text-sm" aria-hidden="true"></i>
            @if (! empty($action['badge']))
                <span class="absolute right-0.5 top-0.5 min-w-4 rounded-full bg-rose-500 px-1 text-center text-[9px] font-bold leading-4 text-white">{{ $action['badge'] }}</span>
            @endif
    ...
    @if ($mobileOverflow)
        <div class="relative sm:hidden" x-data="{ open: false }" data-admin-header-mobile-overflow>
            <button type="button" @click="open = !open" @keydown.escape.window="open = false"
                class="inline-flex h-10 w-10 items-center justify-center rounded-xl text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 focus:outline-none focus:ring-4 focus:ring-indigo-500/10"
                aria-label="Thêm thao tác" :aria-expanded="open.toString()">
            ...
            <div x-cloak x-show="open" @click.away="open = false" x-transition
                ...
                @foreach ($secondaryActions as $action)
                    <a href="{{ $action['url'] }}" target="{{ $action['target'] ?? '_self' }}"
                        @if (($action['target'] ?? '_self') === '_blank') rel="noopener noreferrer" @endif
                        class="flex min-h-10 items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-950">
                        <i class="{{ $action['icon'] }} w-4 text-center text-xs text-slate-400" aria-hidden="true"></i>
