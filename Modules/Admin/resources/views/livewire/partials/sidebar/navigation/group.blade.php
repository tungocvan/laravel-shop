<div x-data="{ open: {{ $item['active'] ? 'true' : 'false' }}, filterEnabled: @js($showNavigationSearch) }">
    <button
        ...
        class="group relative flex min-h-11 w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition duration-150 focus:outline-none focus:ring-2 focus:ring-current focus:ring-offset-1 motion-reduce:transition-none {{ $item['active'] ? $theme['active_bg'].' '.$theme['active_text'] : $theme['text'].' '.$theme['hover'] }}"
    ...
    <div id="{{ $item['group_id'] }}" x-cloak x-show="(open || (filterEnabled && navQuery.trim() !== '')) && sidebarOpen" x-collapse class="ml-7 mt-1 space-y-0.5 border-l pl-3 {{ $theme['border'] }}">
        @foreach ($item['children'] as $child)
            <a
                href="{{ $child['href'] }}"
                @if($showNavigationSearch) x-show="navQuery.trim() === '' || matches(@js($child['name'])) || matches(@js($item['name']))" @endif
                class="relative flex min-h-9 items-center rounded-md px-3 py-1.5 text-[13px] transition duration-150 focus:outline-none focus:ring-2 focus:ring-current focus:ring-offset-1 motion-reduce:transition-none {{ $child['active'] ? $theme['child_active_bg'].' '.$theme['child_active_text'].' font-medium' : $theme['child_text'].' '.$theme['child_hover'] }}"
                @if ($child['active']) aria-current="page" @endif
            >
                @if ($child['active'])
                    <span class="absolute -left-[13px] h-5 w-0.5 rounded-full bg-current" aria-hidden="true"></span>
                @endif
