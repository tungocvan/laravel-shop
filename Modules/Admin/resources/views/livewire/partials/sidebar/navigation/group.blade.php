<div x-data="{ open: {{ $item['active'] ? 'true' : 'false' }} }">
    <button
        ...
        :aria-expanded="((open || ({{ $showNavigationSearch ? 'true' : 'false' }} && navQuery.trim() !== '')) && sidebarOpen).toString()"
    ...
    >
        ...
        <svg x-cloak x-show="sidebarOpen" :class="(open || ({{ $showNavigationSearch ? 'true' : 'false' }} && navQuery.trim() !== '')) ? 'rotate-90' : ''" class="h-3.5 w-3.5 shrink-0 opacity-50 transition-transform duration-150 motion-reduce:transition-none" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path d="M6 6L14 10L6 14V6Z" /></svg>
    </button>

    <div id="{{ $item['group_id'] }}" x-cloak x-show="(open || ({{ $showNavigationSearch ? 'true' : 'false' }} && navQuery.trim() !== '')) && sidebarOpen" x-collapse class="ml-7 mt-1 space-y-0.5 border-l border-slate-200 pl-3">
        @foreach ($item['children'] as $child)
            <a href="{{ $child['href'] }}" @if($showNavigationSearch) x-show="navQuery.trim() === '' || matches(@js($child['name'])) || matches(@js($item['name']))" @endif class="relative flex min-h-9 items-center rounded-md px-3 py-1.5 text-[13px] transition duration-150 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 motion-reduce:transition-none {{ $child['active'] ? 'bg-indigo-50 font-medium text-indigo-700' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800' }}" @if ($child['active']) aria-current="page" @endif>
