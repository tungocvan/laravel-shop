    ...
    <div id="{{ $item['group_id'] }}" x-cloak x-show="(open || (filterEnabled && navQuery.trim() !== '')) && sidebarOpen" x-collapse class="flex flex-col border-l {{ $theme['border'] }}" style="margin-left:var(--admin-sidebar-submenu-indent);margin-top:var(--admin-sidebar-menu-group-gap);padding-left:var(--admin-sidebar-submenu-offset);gap:var(--admin-sidebar-submenu-item-gap)">
        @foreach ($item['children'] as $child)
            <a
                ...
                style="min-height:var(--admin-sidebar-submenu-item-height);padding:var(--admin-sidebar-submenu-padding-y) var(--admin-sidebar-submenu-padding-x);font-family:var(--admin-sidebar-submenu-font-family);font-size:var(--admin-sidebar-submenu-font-size);font-weight:{{ $child['active'] ? 'var(--admin-sidebar-active-font-weight)' : 'var(--admin-sidebar-submenu-font-weight)' }};color:{{ $child['active'] ? 'var(--admin-sidebar-active-title-color)' : 'var(--admin-sidebar-submenu-title-color)' }};{{ $child['active'] ? 'border-width:var(--admin-sidebar-submenu-active-border-width);border-style:var(--admin-sidebar-submenu-active-border-style);border-color:var(--admin-sidebar-submenu-active-border-color);' : '' }}"
