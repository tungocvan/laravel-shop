<div x-data="{ open: {{ $item['active'] ? 'true' : 'false' }}, filterEnabled: @js($showNavigationSearch) }">
    <button
        ...
        class="group relative flex w-full items-center gap-3 rounded-lg px-3 py-2 transition duration-150 focus:outline-none focus:ring-2 focus:ring-current focus:ring-offset-1 motion-reduce:transition-none {{ $item['active'] ? $theme['active_bg'] : $theme['hover'] }}"
    ...
    <div id="{{ $item['group_id'] }}" x-cloak x-show="(open || (filterEnabled && navQuery.trim() !== '')) && sidebarOpen" x-collapse class="mt-1 space-y-0.5 border-l {{ $theme['border'] }}" style="margin-left:var(--admin-sidebar-submenu-indent);padding-left:12px">
        @foreach ($item['children'] as $child)
            <a
                href="{{ $child['href'] }}"
                @if($showNavigationSearch) x-show="navQuery.trim() === '' || matches(@js($child['name'])) || matches(@js($item['name']))" @endif
                class="relative flex items-center rounded-md px-3 py-1.5 transition duration-150 focus:outline-none focus:ring-2 focus:ring-current focus:ring-offset-1 motion-reduce:transition-none {{ $child['active'] ? $theme['child_active_bg'] : $theme['child_hover'] }}"
                style="min-height:var(--admin-sidebar-submenu-item-height);font-family:var(--admin-sidebar-submenu-font-family);font-size:var(--admin-sidebar-submenu-font-size);font-weight:{{ $child['active'] ? 'var(--admin-sidebar-active-font-weight)' : 'var(--admin-sidebar-submenu-font-weight)' }};color:{{ $child['active'] ? 'var(--admin-sidebar-active-title-color)' : 'var(--admin-sidebar-submenu-title-color)' }}"
