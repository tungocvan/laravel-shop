            ...
            <div class="min-w-0 space-y-6">
                ...
                <section class="{{ $section }}" data-admin-header-actions-editor>
                    <div class="{{ $sectionHead }} flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div><h2 class="text-base font-semibold text-slate-900">Header Actions</h2><p class="mt-1 text-sm text-slate-500">Mặc định chỉ hiển thị thông tin cần xem nhanh. Bấm Chỉnh sửa khi cần thay đổi chi tiết.</p></div>
                    ...
                    <div class="space-y-5 p-5">
                        ...
                        <div class="space-y-3">
                            @forelse((array)data_get($config,'header.actions.items',[]) as $index=>$action)
                                <details class="group overflow-hidden rounded-xl border border-slate-200 bg-white transition open:border-indigo-200 open:shadow-sm" wire:key="header-action-{{ $index }}" data-admin-compact-action-row>
                                    <summary class="flex cursor-pointer list-none items-center gap-3 px-4 py-3.5 marker:hidden">
                                        <span class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-indigo-200 bg-indigo-100 text-indigo-700">@include('Admin::livewire.partials.header.components.action-icon', ['icon' => data_get($action,'icon','link'), 'class'=>'h-4 w-4'])</span>
                                        <span class="min-w-0 flex-1">
                                            <span class="flex min-w-0 items-center gap-2"><span class="truncate text-sm font-semibold text-slate-900">{{ data_get($action,'title') ?: 'Action chưa đặt tên' }}</span>@if(data_get($action,'badge'))<span class="shrink-0 rounded-full bg-rose-50 px-2 py-0.5 text-[10px] font-semibold text-rose-600">{{ data_get($action,'badge') }}</span>@endif</span>
                                            <span class="mt-0.5 block truncate text-xs text-slate-500">{{ data_get($action,'url') ?: 'Chưa có URL' }}</span>
                                        </span>
                                        <span class="hidden shrink-0 items-center gap-2 md:flex"><span class="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-slate-600">{{ data_get($action,'priority','secondary') }}</span><span class="rounded-full px-2.5 py-1 text-[10px] font-semibold {{ data_get($action,'enabled',true) ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500' }}">{{ data_get($action,'enabled',true) ? 'Đang hiện' : 'Đang ẩn' }}</span></span>
                                        <span class="inline-flex h-9 shrink-0 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 group-open:border-indigo-200 group-open:text-indigo-700"><span>Chỉnh sửa</span><span class="transition group-open:rotate-180">⌄</span></span>
                                    </summary>
                                    <div class="border-t border-slate-200 bg-slate-50/50 p-4" data-admin-action-edit-panel>
                                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
                                            <label><span class="{{ $fieldLabel }}">Tên action</span><input wire:model="config.header.actions.items.{{ $index }}.title" placeholder="Title" class="{{ $control }} font-medium"></label>
                                            <label><span class="{{ $fieldLabel }}">Icon</span><select wire:model.live="config.header.actions.items.{{ $index }}.icon" class="{{ $select }}">@foreach($headerActionIcons as $icon)<option value="{{ $icon }}">{{ ucfirst($icon) }}</option>@endforeach</select></label>
                                            <label><span class="{{ $fieldLabel }}">URL</span><input wire:model="config.header.actions.items.{{ $index }}.url" placeholder="/admin/..." class="{{ $control }}"></label>
                                            <label><span class="{{ $fieldLabel }}">Priority</span><select wire:model="config.header.actions.items.{{ $index }}.priority" class="{{ $select }}"><option value="primary">Primary</option><option value="secondary">Secondary</option></select></label>
                                            <label><span class="{{ $fieldLabel }}">Target</span><select wire:model="config.header.actions.items.{{ $index }}.target" class="{{ $select }}"><option value="_self">Cùng tab</option><option value="_blank">Tab mới</option></select></label>
                                            <label><span class="{{ $fieldLabel }}">Order</span><input type="number" min="0" max="99" wire:model="config.header.actions.items.{{ $index }}.order" class="{{ $control }}"></label>
                                            <label><span class="{{ $fieldLabel }}">Badge</span><input maxlength="4" wire:model="config.header.actions.items.{{ $index }}.badge" placeholder="optional" class="{{ $control }}"></label>
                                            <label class="md:col-span-2"><span class="{{ $fieldLabel }}">Permission</span><input wire:model="config.header.actions.items.{{ $index }}.permission" placeholder="optional" class="{{ $control }}"></label>
                                        </div>
                                        <div class="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4"><label class="inline-flex items-center gap-2 text-sm font-semibold text-slate-700"><input type="checkbox" wire:model="config.header.actions.items.{{ $index }}.enabled" class="h-4 w-4 rounded border-slate-300 text-indigo-600"> Hiển thị action</label><button type="button" wire:click="removeHeaderAction({{ $index }})" class="inline-flex h-9 items-center rounded-lg border border-rose-200 bg-white px-3 text-xs font-semibold text-rose-600 transition hover:bg-rose-50">Xóa action</button></div>
                                    </div>
                                </details>
                            @empty
                ...
                <section class="{{ $section }}" data-admin-user-menu-editor>
                    <div class="{{ $sectionHead }}"><h2 class="text-base font-semibold text-slate-900">UserMenu</h2><p class="mt-1 text-sm text-slate-500">Hiển thị gọn danh sách menu; chỉ mở form chi tiết khi cần chỉnh sửa. Logout luôn do hệ thống quản lý.</p></div>
                    <div class="p-5">
                        ...
                        <div class="mt-6 flex items-end justify-between gap-4"><div><h3 class="text-sm font-semibold text-slate-900">Menu items hiện tại</h3><p class="mt-1 text-xs text-slate-500">Tên, URL và trạng thái luôn nhìn thấy; các field khác nằm trong Chỉnh sửa.</p></div><button type="button" wire:click="addUserMenuItem" class="inline-flex h-9 shrink-0 items-center rounded-lg border border-indigo-200 bg-indigo-50 px-3 text-xs font-semibold text-indigo-700 hover:bg-indigo-100">+ Thêm menu item</button></div>
                        <div class="mt-3 space-y-3">
                            @forelse((array)data_get($config,'header.user_menu_config.items',[]) as $index=>$item)
                                <details class="group overflow-hidden rounded-xl border border-slate-200 bg-white transition open:border-indigo-200 open:shadow-sm" wire:key="user-menu-item-{{ $index }}" data-admin-compact-user-menu-row>
                                    <summary class="flex cursor-pointer list-none items-center gap-3 px-4 py-3.5 marker:hidden">
                                        <span class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 text-slate-600">@include('Admin::livewire.partials.header.components.action-icon', ['icon' => data_get($item,'icon','user'), 'class'=>'h-4 w-4'])</span>
                                        <span class="min-w-0 flex-1"><span class="block truncate text-sm font-semibold text-slate-900">{{ data_get($item,'label') ?: 'Menu chưa đặt tên' }}</span><span class="mt-0.5 block truncate text-xs text-slate-500">{{ data_get($item,'url') ?: 'Chưa có URL' }}</span></span>
                                        <span class="hidden shrink-0 items-center gap-2 md:flex"><span class="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-600">#{{ data_get($item,'order',0) }}</span><span class="rounded-full px-2.5 py-1 text-[10px] font-semibold {{ data_get($item,'enabled',true) ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500' }}">{{ data_get($item,'enabled',true) ? 'Đang hiện' : 'Đang ẩn' }}</span></span>
                                        <span class="inline-flex h-9 shrink-0 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 group-open:border-indigo-200 group-open:text-indigo-700"><span>Chỉnh sửa</span><span class="transition group-open:rotate-180">⌄</span></span>
                                    </summary>
                                    <div class="border-t border-slate-200 bg-slate-50/50 p-4" data-admin-user-menu-edit-panel>
                                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
                                            ...
                                            <label><span class="{{ $fieldLabel }}">Icon</span><select wire:model.live="config.header.user_menu_config.items.{{ $index }}.icon" class="{{ $select }}">@foreach($userMenuIcons as $icon)<option value="{{ $icon }}">{{ ucfirst($icon) }}</option>@endforeach</select></label>
                                            ...
                                            <label><span class="{{ $fieldLabel }}">Permission</span><input wire:model="config.header.user_menu_config.items.{{ $index }}.permission" placeholder="optional" class="{{ $control }}"></label>
                                        </div>
                                        <div class="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4"><label class="inline-flex items-center gap-2 text-sm font-semibold text-slate-700"><input type="checkbox" wire:model="config.header.user_menu_config.items.{{ $index }}.enabled" class="h-4 w-4 rounded border-slate-300 text-indigo-600"> Hiển thị menu item</label><button type="button" wire:click="removeUserMenuItem({{ $index }})" class="inline-flex h-9 items-center rounded-lg border border-rose-200 bg-white px-3 text-xs font-semibold text-rose-600 transition hover:bg-rose-50">Xóa menu item</button></div>
                                    </div>
                                </details>
                            @empty
