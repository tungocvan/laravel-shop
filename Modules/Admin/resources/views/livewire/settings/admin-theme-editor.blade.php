@php
    ...
    $headerBgMode = data_get($config, 'header.presentation.background', 'system');
    $footerBgMode = data_get($config, 'footer.presentation.background', 'system');
    $pageBgMode = data_get($config, 'layout.surface.page_background', 'system');
    $contentBgMode = data_get($config, 'layout.surface.content_surface', 'transparent');
    ...
    $headerThemeColor = $colorOptions[data_get($config, 'design.colors.header_background', 'white')] ?? '#ffffff';
    $footerThemeColor = $colorOptions[data_get($config, 'design.colors.footer_background', 'white')] ?? '#ffffff';
    $pageThemeColor = $colorOptions[data_get($config, 'design.colors.page_background', 'slate-50')] ?? '#f8fafc';
    $contentThemeColor = $colorOptions[data_get($config, 'design.colors.content_background', 'white')] ?? '#ffffff';
    $previewHeaderBg = $headerBgMode === 'transparent' ? 'transparent' : ($headerBgMode === 'white' ? '#ffffff' : $headerThemeColor);
    $previewFooterBg = $footerBgMode === 'transparent' ? 'transparent' : $footerThemeColor;
    $previewPageBg = $pageBgMode === 'white' ? '#ffffff' : ($pageBgMode === 'slate-50' ? '#f8fafc' : $pageThemeColor);
    $previewContentBg = $contentBgMode === 'transparent' ? 'transparent' : ($contentBgMode === 'white' ? '#ffffff' : $contentThemeColor);
@endphp
    ...
    <div class="mb-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <p class="mt-1 max-w-3xl text-sm leading-6 text-slate-500">Theme điều khiển semantic colors và presentation của Sidebar, Header, Content surface và Footer. Mỗi vùng có màu nền riêng nhưng vẫn dùng token an toàn để giữ contrast/UX.</p>
            </div>
        ...
        <div class="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_22rem] xl:items-start">
            <div class="space-y-6">
                <section class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="mb-5"><h2 class="text-base font-semibold text-slate-900">Semantic colors</h2><p class="mt-1 text-sm leading-6 text-slate-500">Màu semantic chung cho text, border, accent và trạng thái toàn Admin.</p></div>
                ...
                <section class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="mb-5"><h2 class="text-base font-semibold text-slate-900">Sidebar Theme</h2><p class="mt-1 text-sm leading-6 text-slate-500">Palette menu và surface Sidebar. Behavior/width vẫn thuộc trang Sidebar.</p></div>
                ...
                <section class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="mb-5"><h2 class="text-base font-semibold text-slate-900">Header presentation</h2><p class="mt-1 text-sm leading-6 text-slate-500">Ở chế độ System, Header dùng màu Theme riêng và tự điều chỉnh contrast text/icon.</p></div>
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        <label class="block"><span class="text-sm font-medium text-slate-700">Background mode</span><select wire:model.live="config.header.presentation.background" class="{{ $control }}"><option value="system">Theme color — khuyến nghị</option><option value="white">White cố định</option><option value="transparent">Transparent</option></select></label>
                        <label class="block"><span class="text-sm font-medium text-slate-700">Theme color</span><div class="relative"><select wire:model.live="config.design.colors.header_background" class="{{ $control }} pr-10">@foreach($colorOptions as $name => $hex)<option value="{{ $name }}">{{ $name }}</option>@endforeach</select><span class="pointer-events-none absolute right-3 top-1/2 mt-1 h-5 w-5 -translate-y-1/2 rounded-md border border-black/10" style="background: {{ $headerThemeColor }}"></span></div><span class="mt-1.5 block text-xs text-slate-500">Có hiệu lực khi Background mode = Theme color.</span></label>
                        ...
                        <label class="flex items-center justify-between gap-4 rounded-lg border border-slate-200 px-4 py-3"><span><span class="block text-sm font-medium text-slate-800">Backdrop blur</span><span class="mt-0.5 block text-xs text-slate-500">Giữ Header nhẹ khi có nội dung cuộn.</span></span><input type="checkbox" wire:model.live="config.header.presentation.backdrop_blur" class="{{ $toggle }}"></label>
                    </div>
                    <input type="hidden" wire:model="config.header.presentation.action_gap">
                </section>

                <section class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="mb-5"><h2 class="text-base font-semibold text-slate-900">Content & Footer</h2><p class="mt-1 text-sm leading-6 text-slate-500">Page/Content dùng surface palette an toàn; Footer có thể dùng toàn bộ palette và tự contrast.</p></div>
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        <label class="block"><span class="text-sm font-medium text-slate-700">Page background mode</span><select wire:model.live="config.layout.surface.page_background" class="{{ $control }}"><option value="system">Theme color — khuyến nghị</option><option value="white">White cố định</option><option value="slate-50">Slate 50 cố định</option></select></label>
                        <label class="block"><span class="text-sm font-medium text-slate-700">Page theme color</span><div class="relative"><select wire:model.live="config.design.colors.page_background" class="{{ $control }} pr-10">@foreach($surfaceColorOptions as $name => $hex)<option value="{{ $name }}">{{ $name }}</option>@endforeach</select><span class="pointer-events-none absolute right-3 top-1/2 mt-1 h-5 w-5 -translate-y-1/2 rounded-md border border-black/10" style="background: {{ $pageThemeColor }}"></span></div></label>
                        <label class="block"><span class="text-sm font-medium text-slate-700">Content surface mode</span><select wire:model.live="config.layout.surface.content_surface" class="{{ $control }}"><option value="transparent">Transparent — khuyến nghị</option><option value="system">Theme color</option><option value="white">White cố định</option></select></label>
                        <label class="block"><span class="text-sm font-medium text-slate-700">Content theme color</span><div class="relative"><select wire:model.live="config.design.colors.content_background" class="{{ $control }} pr-10">@foreach($surfaceColorOptions as $name => $hex)<option value="{{ $name }}">{{ $name }}</option>@endforeach</select><span class="pointer-events-none absolute right-3 top-1/2 mt-1 h-5 w-5 -translate-y-1/2 rounded-md border border-black/10" style="background: {{ $contentThemeColor }}"></span></div></label>
                        <label class="block"><span class="text-sm font-medium text-slate-700">Footer background mode</span><select wire:model.live="config.footer.presentation.background" class="{{ $control }}"><option value="system">Theme color — khuyến nghị</option><option value="transparent">Transparent</option></select></label>
                        <label class="block"><span class="text-sm font-medium text-slate-700">Footer theme color</span><div class="relative"><select wire:model.live="config.design.colors.footer_background" class="{{ $control }} pr-10">@foreach($colorOptions as $name => $hex)<option value="{{ $name }}">{{ $name }}</option>@endforeach</select><span class="pointer-events-none absolute right-3 top-1/2 mt-1 h-5 w-5 -translate-y-1/2 rounded-md border border-black/10" style="background: {{ $footerThemeColor }}"></span></div></label>
                        ...
                        <label class="flex items-center justify-between gap-4 rounded-lg border border-slate-200 px-4 py-3"><span><span class="block text-sm font-medium text-slate-800">Compact Footer</span><span class="mt-0.5 block text-xs text-slate-500">Giữ Footer thấp và tinh gọn.</span></span><input type="checkbox" wire:model.live="config.footer.presentation.compact" class="{{ $toggle }}"></label>
                    </div>
            ...
            <aside class="xl:sticky xl:top-24" aria-label="Theme preview">
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
                    ...
                    <div class="mt-4 overflow-hidden rounded-xl border shadow-sm" style="border-color: {{ $border }}; background: {{ $previewPageBg }}; color: {{ $textPrimary }}">
                        <div class="flex h-56">
                            ...
                            <div class="flex min-w-0 flex-1 flex-col">
                                <div class="flex h-11 items-center justify-between border-b px-3" style="border-color: {{ $border }}; background: {{ $previewHeaderBg }}"><div class="h-2.5 w-24 rounded bg-black/20"></div><div class="flex gap-1"><span class="h-6 w-6 rounded-md bg-black/10"></span><span class="h-6 w-6 rounded-md bg-black/10"></span></div></div>
                                <div class="min-h-0 flex-1 p-3" style="background: {{ $previewContentBg }}"><div class="h-3 w-2/3 rounded" style="background: {{ $textPrimary }}; opacity:.75"></div><div class="mt-2 h-2 w-1/2 rounded" style="background: {{ $textMuted }}; opacity:.5"></div><div class="mt-4 grid grid-cols-2 gap-2"><div class="h-14 rounded-lg border" style="border-color: {{ $border }}; background: {{ $surfaceRaised }}"></div><div class="h-14 rounded-lg border" style="border-color: {{ $border }}; background: {{ $surfaceRaised }}"></div></div></div>
                                <div class="flex h-9 items-center justify-between border-t px-3 text-[8px]" style="border-color: {{ $border }}; background: {{ $previewFooterBg }}; color: {{ $textMuted }}"><span>© Admin</span><span>23/08/2026 · 14:30</span></div>
                            </div>
                        </div>
                    </div>
                    <dl class="mt-4 grid grid-cols-2 gap-3 text-xs"><div><dt class="text-slate-400">Theme</dt><dd class="mt-0.5 font-semibold text-slate-700">{{ $profiles[$selectedTheme]['label'] ?? $selectedTheme }}</dd></div><div><dt class="text-slate-400">Sidebar</dt><dd class="mt-0.5 font-semibold text-slate-700">{{ data_get($config,'theme.default') }}</dd></div><div><dt class="text-slate-400">Header color</dt><dd class="mt-0.5 font-semibold text-slate-700">{{ data_get($config,'design.colors.header_background') }}</dd></div><div><dt class="text-slate-400">Footer color</dt><dd class="mt-0.5 font-semibold text-slate-700">{{ data_get($config,'design.colors.footer_background') }}</dd></div></dl>
                </div>
            </aside>
        </div>
