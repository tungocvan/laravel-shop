...
<div class="mx-auto max-w-4xl space-y-6">
    <div>
        ...
        <h1 class="mt-2 text-2xl font-bold text-gray-900">Phân quyền Application cho {{ $user->name }}</h1>
        <p class="mt-1 text-sm text-gray-500">{{ $user->email }} · Chỉ cập nhật permission namespace client.*</p>
    </div>

    @if(session('success'))<div class="rounded-lg bg-emerald-50 px-4 py-3 text-sm text-emerald-800">{{ session('success') }}</div>@endif

    <form method="POST" action="{{ route('admin.client-apps.users.update', $user) }}" class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        @csrf @method('PUT')
