...
class AdminLayoutManager
{
    ...
    private function normalize(array $payload): array
    {
        ...
        return [
            ...
            'layout' => ['preset' => $this->in(data_get($payload, 'layout.preset'), ['default', 'data-heavy', 'focus', 'settings'], data_get($defaults, 'layout.preset')), 'container' => $this->in(data_get($payload, 'layout.container'), ['full', 'narrow', '7xl', 'screen-2xl'], data_get($defaults, 'layout.container')), 'density' => $this->in(data_get($payload, 'layout.density'), ['comfortable', 'compact', 'dense'], data_get($defaults, 'layout.density')), 'sticky_header' => (bool) data_get($payload, 'layout.sticky_header', data_get($defaults, 'layout.sticky_header')), 'show_footer' => (bool) data_get($payload, 'layout.show_footer', data_get($defaults, 'layout.show_footer'))],
            ...
            'header' => ['height' => $this->in(data_get($payload, 'header.height'), ['4rem'], data_get($defaults, 'header.height')), 'sticky' => (bool) data_get($payload, 'header.sticky', data_get($defaults, 'header.sticky')), 'search' => (bool) data_get($payload, 'header.search', data_get($defaults, 'header.search')), 'notifications' => (bool) data_get($payload, 'header.notifications', data_get($defaults, 'header.notifications')), 'theme_switcher' => (bool) data_get($payload, 'header.theme_switcher', data_get($defaults, 'header.theme_switcher')), 'user_menu' => (bool) data_get($payload, 'header.user_menu', data_get($defaults, 'header.user_menu')), 'mobile_search_mode' => $this->in(data_get($payload, 'header.mobile_search_mode'), ['overlay'], data_get($defaults, 'header.mobile_search_mode'))],
            ...
            'theme' => ['default' => $this->in(data_get($payload, 'theme.default'), array_keys(config('admin.sidebar.themes', [])), data_get($defaults, 'theme.default')), 'dark_mode' => $this->in(data_get($payload, 'theme.dark_mode'), ['class'], data_get($defaults, 'theme.dark_mode')), 'accent' => $this->in(data_get($payload, 'theme.accent'), ['blue', 'indigo', 'emerald', 'rose', 'amber'], data_get($defaults, 'theme.accent'))],
