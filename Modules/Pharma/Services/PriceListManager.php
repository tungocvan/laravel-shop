...
class PriceListManager
{
    ...
    public function validateItem(array $data): array
    {
        ...
        if ($packageId !== null) {
            $package = MedicinePackage::query()
                ->whereKey($packageId)
                ->where('medicine_variant_id', $variant->id)
                ->first();
            if (! $package) {
        ...
        if ($company !== null && $declared !== null && (float) $company > (float) $declared) {
            throw ValidationException::withMessages([
                'company_sale_price' => 'Giá bán công ty không được vượt quá giá kê khai '.number_format((float) $declared, 0, ',', '.').' ₫.',
            ]);
        }
    ...
    public function activate(PriceList $priceList, ?int $approvedBy = null): PriceList
    {
        ...
        foreach ($priceList->items as $item) {
            if ($item->company_sale_price !== null && $item->declared_price_snapshot !== null
                && (float) $item->company_sale_price > (float) $item->declared_price_snapshot) {
                throw ValidationException::withMessages(['items' => "SKU {$item->medicine_variant_id} có giá bán vượt giá kê khai snapshot."]);
            }
        }

        $overlap = PriceList::query()
            ->whereKeyNot($priceList->id)
            ->where('status', PriceList::STATUS_ACTIVE)
            ->where('type', $priceList->type)
            ->when($priceList->type === PriceList::TYPE_CUSTOMER,
                fn ($query) => $query->where('partner_id', $priceList->partner_id),
                fn ($query) => $query->whereNull('partner_id'))
