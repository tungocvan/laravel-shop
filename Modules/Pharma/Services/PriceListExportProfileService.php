...
class PriceListExportProfileService
{
    ...
    public function defaults():array{$k=array_keys(self::COLUMNS);return['profile_id'=>null,'profile_name'=>'Mặc định','is_default'=>false,'column_order'=>$k,'selected_columns'=>$k,'headers'=>$this->normalizeMap([],'label'),'alignments'=>$this->normalizeMap([],'align'),'widths'=>$this->normalizeWidths([]),'data_types'=>$this->normalizeMap([],'type'),'decimals'=>$this->normalizeDecimals([]),'header_footer'=>self::DEFAULT_HEADER_FOOTER,'page_setup'=>self::DEFAULT_PAGE_SETUP,'logo_path'=>null,'signature_path'=>null];}
