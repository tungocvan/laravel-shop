...
class PriceListExportProfileService
{
    ...
    public function save(int $userId, array $data, ?int $profileId = null): array
    {
        ...
        $profile->forceFill(['user_id'=>$userId,'name'=>mb_substr(trim((string)($data['name']??'Cấu hình Bảng Giá'))?:'Cấu hình Bảng Giá',0,120),'is_default'=>$makeDefault||(bool)$profile->is_default,'column_order'=>$order,'selected_columns'=>$selected,'headers'=>$this->normalizeMap((array)($data['headers']??[]),'label'),'alignments'=>$this->normalizeMap((array)($data['alignments']??[]),'align'),'widths'=>$this->normalizeWidths((array)($data['widths']??[])),'data_types'=>$this->normalizeMap((array)($data['data_types']??[]),'type'),'decimals'=>$this->normalizeDecimals((array)($data['decimals']??[])),'header_footer'=>array_replace(self::DEFAULT_HEADER_FOOTER,(array)($data['header_footer']??[])),'page_setup'=>array_replace(self::DEFAULT_PAGE_SETUP,(array)($data['page_setup']??[])),'logo_path'=>$data['logo_path']??$profile->logo_path,'signature_path'=>$data['signature_path']??$profile->signature_path])->save(); return $this->payload($profile->fresh());
    }
    public function duplicate(int $userId,int $profileId):array{$s=PriceListExportProfile::query()->where('user_id',$userId)->findOrFail($profileId);$c=$s->replicate();$c->name=mb_substr($s->name.' - Bản sao',0,120);$c->is_default=false;$c->save();return $this->payload($c);}
    ...
    public function importPayload(int $userId,array $payload):array
    {
        ...
        $p=$payload['profile']; return $this->save($userId,['name'=>mb_substr(trim((string)($p['profile_name']??'Cấu hình import')).' - Import',0,120),'is_default'=>false,'column_order'=>(array)($p['column_order']??[]),'selected_columns'=>(array)($p['selected_columns']??[]),'headers'=>(array)($p['headers']??[]),'alignments'=>(array)($p['alignments']??[]),'widths'=>(array)($p['widths']??[]),'data_types'=>(array)($p['data_types']??[]),'decimals'=>(array)($p['decimals']??[]),'header_footer'=>(array)($p['header_footer']??[]),'page_setup'=>(array)($p['page_setup']??[])],null);
    }
    public function defaults():array{$k=array_keys(self::COLUMNS);return['profile_id'=>null,'profile_name'=>'Mặc định','is_default'=>false,'column_order'=>$k,'selected_columns'=>self::DEFAULT_SELECTED,'headers'=>$this->normalizeMap([],'label'),'alignments'=>$this->normalizeMap([],'align'),'widths'=>$this->normalizeWidths([]),'data_types'=>$this->normalizeMap([],'type'),'decimals'=>$this->normalizeDecimals([]),'header_footer'=>self::DEFAULT_HEADER_FOOTER,'page_setup'=>self::DEFAULT_PAGE_SETUP,'logo_path'=>null,'signature_path'=>null];}
    private function payload(PriceListExportProfile $p):array{return['profile_id'=>(int)$p->id,'profile_name'=>$p->name,'is_default'=>(bool)$p->is_default,'column_order'=>$this->normalizeOrder((array)$p->column_order),'selected_columns'=>array_values(array_intersect(array_keys(self::COLUMNS),(array)$p->selected_columns)),'headers'=>$this->normalizeMap((array)$p->headers,'label'),'alignments'=>$this->normalizeMap((array)$p->alignments,'align'),'widths'=>$this->normalizeWidths((array)$p->widths),'data_types'=>$this->normalizeMap((array)$p->data_types,'type'),'decimals'=>$this->normalizeDecimals((array)$p->decimals),'header_footer'=>array_replace(self::DEFAULT_HEADER_FOOTER,(array)$p->header_footer),'page_setup'=>array_replace(self::DEFAULT_PAGE_SETUP,(array)$p->page_setup),'logo_path'=>$p->logo_path,'signature_path'=>$p->signature_path];}
