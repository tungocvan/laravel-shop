...
class DrugBidAwardContractService
{
        ...
        return DB::transaction(function () use ($allocationId, $contractId, $data, $adminId) {
            ...
            if (! $allocation->isActive()) {
                throw ValidationException::withMessages([
                    'contract' => 'Không thể quản lý hợp đồng trên phân bổ đã hủy.',
                ]);
            }

            $contract = $contractId
                ? DrugBidAwardContract::query()
            ...
            if (! in_array($status, $allowed, true)) {
                throw ValidationException::withMessages([
                    'contract_status' => 'Trạng thái hợp đồng không hợp lệ.',
                ]);
            }
            ...
            if ($quantity <= 0) {
                throw ValidationException::withMessages([
                    'contract_quantity' => 'Số lượng hợp đồng phải lớn hơn 0.',
                ]);
            }
            ...
            $otherCommitted = (float) DrugBidAwardContract::query()
                ...
                ->when($contract->exists, fn ($query) => $query->where('id', '!=', $contract->id))
                ->sum('contract_quantity');
            $candidateCommitted = in_array($status, DrugBidAwardContract::COMMITTED_STATUSES, true)
                ? $quantity
                : 0.0;

            if (($otherCommitted + $candidateCommitted) > ((float) $allocation->allocated_quantity + 0.00005)) {
                throw ValidationException::withMessages([
                    'contract_quantity' => 'Tổng số lượng hợp đồng hiệu lực không được vượt số lượng đã phân bổ.',
                ]);
            }
            ...
            $contract->fill([
                ...
                'contract_value' => $data['contract_value'] !== null && $data['contract_value'] !== ''
                    ? $data['contract_value']
                    : null,
    ...
    public function cancel(int $allocationId, int $contractId, string $reason, ?int $adminId): void
    {
        DB::transaction(function () use ($allocationId, $contractId, $reason, $adminId) {
            ...
            if ($contract->status === DrugBidAwardContract::STATUS_COMPLETED) {
                throw ValidationException::withMessages([
                    'contract' => 'Không thể hủy trực tiếp hợp đồng đã hoàn thành.',
                ]);
            }
