...
class DrugBidAwardCommercialPolicyService
{
    ...
    public function saveDraft(DrugBidAward $award, array $data, ?int $actorId): DrugBidAwardCommercialPolicy
    {
        return DB::transaction(function () use ($award, $data, $actorId) {
            ...
            if ($policy?->status === DrugBidAwardCommercialPolicy::STATUS_ACTIVE) {
                throw ValidationException::withMessages(['policy' => 'Chính sách đã kích hoạt. Hãy lưu lịch sử và tạo phiên bản mới thay vì ghi đè.']);
            }
            $policy ??= new DrugBidAwardCommercialPolicy(['result_key' => $this->groups->resultKey($award), 'bidding_notice_code' => $award->bidding_notice_code, 'status' => DrugBidAwardCommercialPolicy::STATUS_DRAFT, 'created_by' => $actorId]);
    ...
    public function assign(DrugBidAwardCommercialPolicy $policy, array $awardIds, int $userId, float $share, string $from, ?string $until, ?int $actorId): void
    {
        ...
        $validIds = $this->groups->awardsQuery(DrugBidAward::query()->findOrFail($awardIds[0] ?? 0))->whereIn('id', $awardIds)->pluck('id')->map(fn($id)=>(int)$id)->all();
        if (count($validIds) !== count(array_unique(array_map('intval',$awardIds)))) throw ValidationException::withMessages(['selectedAwardIds' => 'Có sản phẩm không thuộc cùng kết quả trúng thầu.']);
    ...
    public function activationIssues(DrugBidAward $award, DrugBidAwardCommercialPolicy $policy): array
    {
        ...
        if (! $scope) $issues[] = 'Chưa thiết lập phạm vi phân bổ cho TBMT.';
    ...
    public function activate(DrugBidAward $award, DrugBidAwardCommercialPolicy $policy, ?int $actorId): void
    {
        ...
        DB::transaction(function () use ($policy,$actorId) {
            DrugBidAwardCommercialPolicy::query()->where('result_key',$policy->result_key)->where('status',DrugBidAwardCommercialPolicy::STATUS_ACTIVE)->whereKeyNot($policy->id)->update(['status'=>DrugBidAwardCommercialPolicy::STATUS_ARCHIVED,'archived_by'=>$actorId,'archived_at'=>now(),'updated_by'=>$actorId]);
