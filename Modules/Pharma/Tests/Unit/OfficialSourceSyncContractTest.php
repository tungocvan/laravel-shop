...
class OfficialSourceSyncContractTest extends TestCase
{
    ...
    #[Test]
    public function workspace_uses_shared_search_live_filters_canonical_province_and_source_partition(): void
    {
        ...
        $this->assertStringContainsString('name="province"', $view);
        $this->assertStringContainsString('name="partition"', $view);
        $this->assertStringContainsString('Vùng nguồn BHXH', $view);
        ...
        $this->assertStringContainsString("element.addEventListener('change', submit)", $view);
        $this->assertStringContainsString('window.setTimeout(submit, 450)', $view);
        ...
        $this->assertStringContainsString("->where('province_name', \$province)", $controller);
        $this->assertStringContainsString("->where('source_province_code', \$partition)", $controller);
