...
class PriceListController extends Controller
{
    ...
    public function show(PriceList $priceList, PriceListExportProfileService $profiles): View { $priceList->load(['partner','officialFacility','manager','purpose','items.medicine','items.variant','items.package','items.bidEvidence']);$exportProfiles=$profiles->profilesForUser((int) auth('admin')->id());$defaultExportProfileId=collect($exportProfiles)->firstWhere('is_default',true)['id']??null;return view('Pharma::pages.price-list.show',compact('priceList','exportProfiles','defaultExportProfileId')); }
    public function edit(PriceList $priceList): View { abort_unless($priceList->isDirectlyEditable(),422,'Chỉ bảng giá Draft hoặc Inactive mới được chỉnh trực tiếp. Hãy clone bảng giá Active để tạo phiên bản mới.');return view('Pharma::pages.price-list.edit',compact('priceList')); }
    public function update(Request $request,PriceList $priceList,PriceListManager $manager):RedirectResponse { abort_unless($priceList->isDirectlyEditable(),422,'Chỉ bảng giá Draft hoặc Inactive mới được chỉnh trực tiếp.');$priceList->update($manager->validateHeader($request->validate($this->headerRules()),$priceList));return back()->with('success','Đã cập nhật bảng giá.'); }
    ...
    public function export(Request $request,PriceList $priceList,PriceListExportProfileService $profiles,PriceListExcelTypography $typography,PriceListExcelDocumentLayout $layout):BinaryFileResponse
    {
        ...
        $spreadsheet=new Spreadsheet();$sheet=$spreadsheet->getActiveSheet();$sheet->setTitle('Bang gia');$lastColumn=Coordinate::stringFromColumnIndex(count($columns));
        $row=$layout->header($sheet,$profile,count($columns));
        ...
        $layout->footer($sheet,$profile,count($columns),$headerRow+$items->count());
