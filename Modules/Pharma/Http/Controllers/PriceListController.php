...
class PriceListController extends Controller
{
    ...
    public function export(Request $request, PriceList $priceList, PriceListExportProfileService $profiles): BinaryFileResponse
    {
        ...
        $columns = array_values(array_filter(
            ...
            fn ($key) => in_array($key, $profile['selected_columns'], true) && isset(PriceListExportProfileService::COLUMNS[$key]),
        ));
        ...
        if ((bool) ($hf['enabled'] ?? true)) {
            ...
            $sheet->setCellValue("A{$row}", $hf['company_name'] ?? '');
            $sheet->getStyle("A{$row}")->getFont()->setBold(true)->setSize(12);
            $this->addDrawing($sheet, $profile['logo_path'] ?? null, 'Logo', 'A'.$row, (float) ($hf['logo_width_cm'] ?? 2.48), (float) ($hf['logo_height_cm'] ?? 3.83));
            if (! empty($profile['logo_path'])) $sheet->getRowDimension($row)->setRowHeight(max(22, (float) ($hf['logo_height_cm'] ?? 3.83) * 28.35));
            $row++;

            $contact = implode(' · ', array_filter([
                $hf['address'] ?? null,
                ($hf['tax_code'] ?? '') !== '' ? 'MST: '.$hf['tax_code'] : null,
                $hf['phone'] ?? null,
                $hf['email'] ?? null,
            ]));
            if ($contact !== '') {
        ...
        foreach ($items as $index => $item) {
            ...
            $discount = ($item->company_sale_price && $item->actual_receivable_price !== null)
                ? round((1 - ((float) $item->actual_receivable_price / (float) $item->company_sale_price)) * 100, 2)
                : null;
            $values = [
                'stt' => $index + 1,
                'medicine_code' => $item->medicine?->medicine_code,
                'medicine_name' => $item->medicine?->name,
                'active_ingredients' => $item->medicine?->active_ingredients,
                'strength' => $item->variant?->strength_text ?: $item->medicine?->concentration,
                'registration_number' => $item->medicine?->registration_number,
                'sku' => $item->variant?->sku,
                'package' => $item->package?->packaging_text ?: $item->medicine?->packaging_specification,
                'declared_price' => $item->declared_price_snapshot,
                'bid_price' => $evidence?->bid_price,
                'bid_quantity' => $evidence?->quantity,
                'bid_decision' => $evidence?->decision_number,
                'bid_date' => $evidence?->award_date,
                'bid_contractor' => $evidence?->contractor_name,
                'bid_source' => $evidence?->source_system,
                'company_sale_price' => $item->company_sale_price,
                'discount_percent' => $discount,
                'actual_receivable_price' => $item->actual_receivable_price,
            ...
            foreach ($columns as $columnIndex => $key) {
                $coordinate = Coordinate::stringFromColumnIndex($columnIndex + 1).$dataRow;
                $this->writeConfiguredCell($sheet, $coordinate, $values[$key] ?? null, $profile['data_types'][$key] ?? 'auto', (int) ($profile['decimals'][$key] ?? 0));
            }
        }

        foreach ($columns as $index => $key) {
            $letter = Coordinate::stringFromColumnIndex($index + 1);
            $sheet->getColumnDimension($letter)->setWidth(max(6, ((int) ($profile['widths'][$key] ?? 100)) / 7));
            $sheet->getStyle($letter.$headerRow.':'.$letter.$sheet->getHighestRow())->getAlignment()
                ->setHorizontal($profile['alignments'][$key] ?? 'left')->setVertical('center')->setWrapText(true);
        }

        if ((bool) ($hf['enabled'] ?? true)) {
            $footer = $sheet->getHighestRow() + 2;
            $sheet->mergeCells("A{$footer}:{$lastColumn}{$footer}");
            $location = trim(($hf['footer_location'] ?? '').' '.(($hf['footer_year'] ?? '') !== '' ? 'năm '.$hf['footer_year'] : ''));
            $sheet->setCellValue("A{$footer}", $location);
            $sheet->getStyle("A{$footer}")->getAlignment()->setHorizontal('right');
            $footer++;
            $sheet->mergeCells("A{$footer}:{$lastColumn}{$footer}");
            $sheet->setCellValue("A{$footer}", $hf['signatory_title'] ?? '');
            $sheet->getStyle("A{$footer}")->getFont()->setBold(true);
            $sheet->getStyle("A{$footer}")->getAlignment()->setHorizontal('right');
            $signatureRow = $footer + 1;
            $this->addDrawing($sheet, $profile['signature_path'] ?? null, 'Signature', $lastColumn.$signatureRow, (float) ($hf['signature_width_cm'] ?? 4), (float) ($hf['signature_height_cm'] ?? 2), true);
            if (! empty($profile['signature_path'])) $sheet->getRowDimension($signatureRow)->setRowHeight(max(22, (float) ($hf['signature_height_cm'] ?? 2) * 28.35));
            if (($hf['signatory_name'] ?? '') !== '') {
                $footer += 4;
                $sheet->mergeCells("A{$footer}:{$lastColumn}{$footer}");
                $sheet->setCellValue("A{$footer}", $hf['signatory_name']);
                $sheet->getStyle("A{$footer}")->getAlignment()->setHorizontal('right');
            }
        }
        ...
        $setup->setPaperSize(match ($page['paper_size'] ?? 'A4') {
            'A3' => PageSetup::PAPERSIZE_A3,
            'LETTER' => PageSetup::PAPERSIZE_LETTER,
            'LEGAL' => PageSetup::PAPERSIZE_LEGAL,
            default => PageSetup::PAPERSIZE_A4,
        });
        $setup->setOrientation(($page['orientation'] ?? 'landscape') === 'portrait' ? PageSetup::ORIENTATION_PORTRAIT : PageSetup::ORIENTATION_LANDSCAPE);
        if (($page['scaling'] ?? 'fit_width') !== 'none') {
            $setup->setFitToWidth((int) ($page['fit_width'] ?? 1));
            $setup->setFitToHeight(($page['scaling'] ?? 'fit_width') === 'fit_sheet' ? 1 : 0);
        }
        $margins = $sheet->getPageMargins();
        $margins->setLeft(((float) ($page['margin_left_cm'] ?? 0.3)) / 2.54)
            ->setRight(((float) ($page['margin_right_cm'] ?? 0.3)) / 2.54)
            ->setTop(((float) ($page['margin_top_cm'] ?? 0.8)) / 2.54)
            ->setBottom(((float) ($page['margin_bottom_cm'] ?? 0.8)) / 2.54);
        $setup->setHorizontalCentered((bool) ($page['center_horizontal'] ?? true))->setVerticalCentered((bool) ($page['center_vertical'] ?? false));
    ...
    private function writeConfiguredCell($sheet, string $coordinate, mixed $value, string $type, int $decimals): void
    {
        ...
        if ($type === 'number') {
            ...
            $sheet->setCellValue($coordinate, $numeric);
            $sheet->getStyle($coordinate)->getNumberFormat()->setFormatCode('#,##0'.($decimals > 0 ? '.'.str_repeat('0', min(6, $decimals)) : ''));
            return;
        }

        if ($type === 'date') {
            $date = $value instanceof DateTimeInterface ? $value : new DateTimeImmutable((string) $value);
            $sheet->setCellValue($coordinate, Date::PHPToExcel($date));
    ...
    private function addDrawing($sheet, ?string $path, string $name, string $coordinate, float $widthCm, float $heightCm, bool $right = false): void
    {
        ...
        if ($right) $drawing->setOffsetX(-max(0, (int) round($widthCm * 20)));
