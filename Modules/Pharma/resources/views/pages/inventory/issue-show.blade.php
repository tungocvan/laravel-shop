...
<div class="mx-auto w-full max-w-[1580px] space-y-6">
 ...
 <section class="rounded-2xl border border-slate-200 bg-white shadow-sm">
  ...
  <div class="grid gap-px bg-slate-100 md:grid-cols-4">
   ...
   <div class="bg-white p-5"><p class="text-[11px] font-bold uppercase tracking-wide text-slate-400">Kho xuất</p><p class="mt-2 font-bold">Kho chính</p></div>
  </div>
  <div class="grid gap-4 border-t border-slate-100 bg-slate-50/70 p-5 md:grid-cols-4">
   ...
   <div><p class="text-[11px] font-bold uppercase text-slate-400">Bảng giá áp dụng</p><p class="mt-1 font-semibold">{{ $issue->priceList?->code ?: '—' }}</p><p class="text-xs text-slate-500">{{ $issue->priceList?->name }}</p></div>
   <div><p class="text-[11px] font-bold uppercase text-slate-400">Ghi chú</p><p class="mt-1 text-sm">{{ $issue->notes ?: 'Không có ghi chú.' }}</p></div>
  </div>
 </section>
 <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
  ...
  <div class="overflow-x-auto"><table class="min-w-[1100px] w-full text-sm"><thead class="bg-slate-50 text-[11px] uppercase tracking-wide text-slate-500"><tr><th class="p-3 text-center">STT</th><th class="p-3 text-left">Mã thuốc</th><th class="p-3 text-left">Tên thuốc / Quy cách</th><th class="p-3 text-center">ĐVT</th><th class="p-3 text-left">Số lô</th><th class="p-3 text-center">Hạn dùng</th><th class="p-3 text-right">Số lượng</th><th class="p-3 text-right">Đơn giá xuất</th><th class="p-3 text-right">Thành tiền</th></tr></thead><tbody class="divide-y divide-slate-100">@foreach($issue->items as $item)<tr class="hover:bg-slate-50/60"><td class="p-3 text-center">{{ $loop->iteration }}</td><td class="p-3 font-mono text-xs font-semibold">{{ $item->medicine->medicine_code }}</td><td class="p-3"><b>{{ $item->medicine->name }}</b>@if($item->medicine->packaging_specification)<div class="mt-0.5 text-xs text-slate-500">{{ $item->medicine->packaging_specification }}</div>@endif</td><td class="p-3 text-center">{{ $item->medicine->unit ?: '—' }}</td><td class="p-3">{{ $item->batch_number }}</td><td class="p-3 text-center">{{ $item->expiry_date->format('d/m/Y') }}</td><td class="p-3 text-right font-semibold">{{ number_format((float)$item->quantity,3,',','.') }}</td><td class="p-3 text-right">{{ number_format((float)$item->unit_price,0,',','.') }} đ</td><td class="p-3 text-right font-bold">{{ number_format((float)$item->quantity*(float)$item->unit_price,0,',','.') }} đ</td></tr>@endforeach</tbody><tfoot class="border-t-2 border-slate-200 bg-slate-50"><tr><td colspan="6" class="p-4 text-right font-bold">Tổng cộng</td><td class="p-4 text-right font-bold">{{ number_format($totalQuantity,3,',','.') }}</td><td></td><td class="p-4 text-right text-base font-bold">{{ number_format($totalValue,0,',','.') }} đ</td></tr></tfoot></table></div>
 </section>
 <div class="grid gap-5 lg:grid-cols-[360px_minmax(0,1fr)]">
  <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 class="font-bold">Tóm tắt phiếu</h2><dl class="mt-4 space-y-3 text-sm"><div class="flex justify-between"><dt class="text-slate-500">Số mặt hàng</dt><dd class="font-bold">{{ $issue->items->count() }}</dd></div><div class="flex justify-between"><dt class="text-slate-500">Tổng số lượng</dt><dd class="font-bold">{{ number_format($totalQuantity,3,',','.') }}</dd></div><div class="border-t pt-3"><dt class="text-slate-500">Tổng giá trị</dt><dd class="mt-1 text-2xl font-bold text-indigo-700">{{ number_format($totalValue,0,',','.') }} đ</dd></div></dl></section>
  <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 class="font-bold">Người liên quan</h2><div class="mt-5 grid gap-6 text-center sm:grid-cols-3"><div><p class="font-semibold">Người lập phiếu</p><p class="mt-1 text-xs text-slate-400">Ký, ghi rõ họ tên</p><div class="h-16"></div></div><div><p class="font-semibold">Người giao hàng</p><p class="mt-1 text-xs text-slate-400">Ký, ghi rõ họ tên</p><div class="h-16"></div></div><div><p class="font-semibold">Người nhận hàng</p><p class="mt-1 text-xs text-slate-400">Ký, ghi rõ họ tên</p><div class="h-16"></div></div></div></section>
 </div>
</div>
@endsection
