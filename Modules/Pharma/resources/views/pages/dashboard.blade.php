    ...
    <div class="space-y-8">
        <header class="flex flex-col gap-4 border-b border-slate-200 pb-6 lg:flex-row lg:items-end lg:justify-between">
            <div class="min-w-0">
                ...
                <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                    Điểm vào tập trung cho các workspace Pharma, giúp kiểm tra nhanh dữ liệu và mở đúng chức năng mà không cần nhớ URL.
                </p>
            </div>

            @if ($capabilities['create'])
                <a href="{{ route('admin.pharma.price-lists.create') }}"
        ...
        <section aria-labelledby="pharma-summary-heading">
            ...
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <a href="{{ $capabilities['create'] ? route('admin.pharma.price-lists.create') : '#' }}"
                   @if (! $capabilities['create']) aria-disabled="true" @endif
                   class="rounded-2xl border p-5 shadow-sm {{ $priceList['ready'] ? 'border-emerald-200 bg-emerald-50/60' : 'border-amber-200 bg-amber-50/60' }} {{ $capabilities['create'] ? 'transition hover:shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-500' : 'cursor-default' }}">
                    <p class="text-sm font-medium text-slate-600">Workbook bảng giá</p>
                    <p class="mt-2 text-lg font-bold {{ $priceList['ready'] ? 'text-emerald-700' : 'text-amber-700' }}">
                        {{ $priceList['ready'] ? 'Sẵn sàng' : 'Thiếu file nguồn' }}
                    </p>
        ...
        <section aria-labelledby="pharma-workspaces-heading">
            <div class="mb-4">
                ...
                <p class="mt-1 max-w-3xl text-sm leading-6 text-slate-500">
                    Dashboard chỉ tổng hợp và điều hướng; thao tác nghiệp vụ vẫn thực hiện trong từng workspace chuyên trách.
                </p>
            </div>
            ...
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <a href="{{ route('admin.pharma.hssp.index') }}" class="rounded-2xl border border-indigo-200 bg-indigo-50/60 p-5 transition hover:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <p class="text-xs font-semibold uppercase tracking-wide text-indigo-600">Danh mục</p>
                    <h3 class="mt-2 text-lg font-bold text-slate-950">Thuốc / HSSP</h3>
                    <p class="mt-2 text-sm leading-6 text-slate-600">Tra cứu và quản lý hồ sơ sản phẩm Pharma.</p>
                </a>
                ...
                <a href="{{ route('admin.pharma.drug-bid-awards.index') }}" class="rounded-2xl border border-sky-200 bg-sky-50/60 p-5 transition hover:border-sky-400 focus:outline-none focus:ring-2 focus:ring-sky-500">
                    <p class="text-xs font-semibold uppercase tracking-wide text-sky-700">Đấu thầu</p>
                    <h3 class="mt-2 text-lg font-bold text-slate-950">Kết quả trúng thầu</h3>
                    <p class="mt-2 text-sm leading-6 text-slate-600">Mở workspace kết quả lựa chọn nhà thầu.</p>
                </a>
                ...
                <a href="{{ route('admin.pharma.supplier-trackings.index') }}" class="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-5 transition hover:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                    <p class="text-xs font-semibold uppercase tracking-wide text-emerald-700">Nhà cung cấp</p>
                    <h3 class="mt-2 text-lg font-bold text-slate-950">Theo dõi nhà cung cấp</h3>
                    <p class="mt-2 text-sm leading-6 text-slate-600">Theo dõi giá, cam kết và dữ liệu nhà cung cấp.</p>
                </a>
                ...
                @if ($capabilities['create'])
                    <a href="{{ route('admin.pharma.price-lists.create') }}" class="rounded-2xl border border-amber-200 bg-amber-50/60 p-5 transition hover:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-500">
                        <p class="text-xs font-semibold uppercase tracking-wide text-amber-700">Excel</p>
                        <h3 class="mt-2 text-lg font-bold text-slate-950">Tạo bảng giá</h3>
                        <p class="mt-2 text-sm leading-6 text-slate-600">Chọn sản phẩm và cột để xuất bảng giá từ workbook nguồn.</p>
                    </a>
                @endif
            </div>
        </section>
        ...
        @if ($capabilities['create'])
            <section aria-labelledby="pharma-quick-actions-heading">
                <div class="mb-4">
                    <h2 id="pharma-quick-actions-heading" class="text-lg font-semibold text-slate-900">Thao tác nhanh</h2>
                    <p class="mt-1 text-sm text-slate-500">Các hành động chỉ xuất hiện khi tài khoản có quyền tạo dữ liệu Pharma.</p>
                </div>
