    ...
    <div class="space-y-8">
        <header class="flex flex-col gap-4 border-b border-slate-200 pb-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
                ...
                <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Điểm vào tập trung cho Medicine Master, HSSP và các workspace Pharma chuyên trách.</p>
            </div>
            @if($capabilities['create'])
                <a href="{{ route('admin.pharma.medicines.create') }}" class="inline-flex min-h-11 items-center justify-center rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700">Thêm thuốc vào Medicine Master</a>
            @endif
        </header>
        ...
        <section>
            ...
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <a href="{{ route('admin.pharma.medicines.index') }}" class="rounded-2xl border border-indigo-200 bg-indigo-50/60 p-5 shadow-sm transition hover:border-indigo-400">
                    <p class="text-sm font-medium text-slate-600">Danh mục thuốc chuẩn</p><p class="mt-2 text-3xl font-bold text-slate-950">{{ $formatCount($metrics['medicines']) }}</p><p class="mt-2 text-xs text-slate-500">Medicine Master dùng chung toàn ERP</p>
                </a>
                <a href="{{ route('admin.pharma.hssp.index') }}" class="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-5 shadow-sm transition hover:border-emerald-400">
                    <p class="text-sm font-medium text-slate-600">HSSP thuốc</p><p class="mt-2 text-3xl font-bold text-slate-950">→</p><p class="mt-2 text-xs text-slate-500">Hồ sơ sản phẩm gắn với Medicine Master</p>
                </a>
                <a href="{{ route('admin.pharma.drug-bid-awards.index') }}" class="rounded-2xl border border-sky-200 bg-sky-50/60 p-5 shadow-sm transition hover:border-sky-400">
                    <p class="text-sm font-medium text-slate-600">Kết quả trúng thầu</p><p class="mt-2 text-3xl font-bold text-slate-950">{{ $formatCount($metrics['drug_bid_awards']) }}</p><p class="mt-2 text-xs text-slate-500">Bản ghi kết quả lựa chọn nhà thầu</p>
                </a>
                <a href="{{ $capabilities['create'] ? route('admin.pharma.price-lists.create') : '#' }}" class="rounded-2xl border p-5 shadow-sm {{ $priceList['ready'] ? 'border-emerald-200 bg-emerald-50/60' : 'border-amber-200 bg-amber-50/60' }}">
                    <p class="text-sm font-medium text-slate-600">Workbook bảng giá</p><p class="mt-2 text-lg font-bold">{{ $priceList['ready'] ? 'Sẵn sàng' : 'Thiếu file nguồn' }}</p><p class="mt-2 break-all text-xs text-slate-500">{{ $priceList['source'] }}</p>
                </a>
            </div>
        </section>
        ...
        <section>
            ...
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                ...
                @if($capabilities['create'])<a href="{{ route('admin.pharma.price-lists.create') }}" class="rounded-2xl border border-slate-200 bg-white p-5 hover:border-slate-400"><p class="text-xs font-semibold uppercase tracking-wide text-slate-600">Excel</p><h3 class="mt-2 text-lg font-bold text-slate-950">Tạo bảng giá</h3><p class="mt-2 text-sm leading-6 text-slate-600">Xuất bảng giá từ workbook nguồn.</p></a>@endif
            </div>
        </section>
    </div>
@endsection
