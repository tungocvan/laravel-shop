                        ...
                        <tr>
                            ...
                            <td class="px-3 py-3">
                                <div class="flex items-center gap-2">
                                    <div class="relative w-28"><input type="number" min="0" max="100" step="0.01" wire:model.blur="hospitalPolicyOverrides.{{ $product->id }}" wire:change="saveHospitalPolicyOverride({{ $product->id }})" class="w-full rounded-lg border border-slate-300 px-2.5 py-2 pr-7 text-right text-sm"><span class="pointer-events-none absolute right-2 top-2 text-xs text-slate-400">%</span></div>
                                    @if(($hospitalPolicyOverrides[$product->id] ?? '') !== '')<button type="button" wire:click="resetHospitalPolicyOverride({{ $product->id }})" class="text-xs font-semibold text-rose-600">Đặt lại</button>@endif
                                </div>
                            </td>
