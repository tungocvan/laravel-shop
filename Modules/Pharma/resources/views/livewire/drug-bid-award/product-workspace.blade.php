    ...
    <section class="rounded-2xl border border-indigo-200 bg-indigo-50/30 p-5 shadow-sm">
        ...
        <div class="mt-5 grid gap-4 xl:grid-cols-12">
            <div class="xl:col-span-4 space-y-4 rounded-xl border border-slate-200 bg-white p-4">
                <div><p class="text-xs font-bold uppercase tracking-wide text-indigo-600">Bước 1 · Phạm vi</p><label class="mt-2 block text-sm font-semibold text-slate-700">Tỉnh/Thành trúng thầu *</label><div class="mt-2 grid max-h-44 gap-2 overflow-y-auto rounded-xl border border-slate-200 p-3">@foreach ($provinceOptions as $province)<label wire:key="award-province-{{ md5($province) }}" class="flex items-center gap-2 text-sm text-slate-700"><input type="checkbox" wire:model.live="selectedProvinces" value="{{ $province }}" class="h-4 w-4 rounded border-slate-300 text-indigo-600"><span>{{ $province }}</span></label>@endforeach</div><p class="mt-1 text-xs text-slate-500">Có thể chọn nhiều tỉnh/thành. Nguồn: Kho dữ liệu cơ sở KCB nguồn.</p></div>
