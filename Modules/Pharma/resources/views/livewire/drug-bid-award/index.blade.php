        ...
        <div class="overflow-x-auto"><table class="min-w-[1060px] w-full divide-y divide-slate-200 text-left text-sm">
            ...
            @forelse ($awards as $award)
                <tr wire:key="award-group-{{ $award->id }}" class="align-top hover:bg-slate-50">
                    <td class="px-4 py-4 text-center"><input type="checkbox" wire:key="award-checkbox-{{ $award->id }}-{{ in_array((string) $award->id, array_map('strval', $selectedIds), true) ? '1' : '0' }}" x-data="{ selected: @js(in_array((string) $award->id, array_map('strval', $selectedIds), true)) }" x-model="selected" x-on:change="$wire.setAwardSelected({{ $award->id }}, selected)" aria-label="Chọn TBMT {{ $award->bidding_notice_code ?: $award->id }}" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"></td>
