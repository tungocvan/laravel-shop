...
<div class="space-y-6">
    ...
    <section class="rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        @if($showFilters)<div class="border-t border-slate-100 p-5">
            <div class="grid items-end gap-3 md:grid-cols-2 xl:grid-cols-[minmax(180px,0.9fr)_minmax(260px,1.55fr)_minmax(260px,1.55fr)_minmax(150px,0.75fr)_minmax(150px,0.75fr)_minmax(130px,0.65fr)_auto]">
                ...
                <label class="block text-xs font-semibold text-slate-700">Nguồn<select wire:model.live="filterSource" class="mt-1.5 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm"><option value="">Tất cả nguồn</option>@foreach ($sourceOptions as $value => $label)<option value="{{ $value }}">{{ $label }}</option>@endforeach</select></label>
