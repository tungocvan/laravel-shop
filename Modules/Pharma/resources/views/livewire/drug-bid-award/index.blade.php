        ...
        <div class="overflow-x-auto overflow-y-visible"><table class="min-w-[1060px] w-full divide-y divide-slate-200 text-left text-sm">
            ...
            @forelse ($awards as $award)
                <tr wire:key="award-group-{{ $award->representative_id }}" class="align-top hover:bg-slate-50">
                    ...
                    <td class="px-4 py-4 text-right"><div class="flex items-center justify-end gap-2">@if ($canViewAllocations)<a href="{{ route('admin.pharma.drug-bid-awards.allocations', $award->representative_id) }}" class="inline-flex min-h-10 items-center whitespace-nowrap rounded-xl bg-indigo-600 px-3.5 py-2 text-xs font-semibold text-white shadow-sm hover:bg-indigo-700">Sản phẩm / Phân bổ</a>@endif @if($canEdit || $canViewCommercialPolicies)<div x-data="{ open: false, top: 0, right: 0, place() { const r = this.$refs.trigger.getBoundingClientRect(); this.top = r.bottom + 8; this.right = Math.max(8, window.innerWidth - r.right); } }" x-on:keydown.escape.window="open = false" x-on:scroll.window="if (open) place()" x-on:resize.window="if (open) place()" class="relative z-40"><button x-ref="trigger" type="button" x-on:click="open = !open; if (open) $nextTick(() => place())" class="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-300 bg-white text-lg font-bold text-slate-600 hover:bg-slate-50" aria-label="Thêm thao tác" aria-haspopup="menu" x-bind:aria-expanded="open">⋯</button><template x-teleport="body"><div x-show="open" x-cloak x-on:click.outside="open = false" x-bind:style="`position: fixed; top: ${top}px; right: ${right}px;`" class="z-[100] w-52 overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 text-left shadow-xl" role="menu">@if($canEdit)<button type="button" wire:click="editResultGroup({{ $award->representative_id }})" x-on:click="open = false" class="block w-full rounded-lg px-3 py-2.5 text-left text-xs font-semibold text-slate-700 hover:bg-slate-50">Sửa hồ sơ</button>@endif @if($canViewCommercialPolicies)<a href="{{ route('admin.pharma.drug-bid-awards.commercial-policy', $award->representative_id) }}" class="block rounded-lg px-3 py-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">Chính sách kinh doanh</a>@endif @if($canDelete)<button type="button" wire:click="deleteResultGroup({{ $award->representative_id }})" wire:confirm="Xóa toàn bộ TBMT {{ $award->bidding_notice_code ?: '#' . $award->representative_id }} và các sản phẩm trúng thầu liên quan? Thao tác này không thể hoàn tác." x-on:click="open = false" class="block w-full rounded-lg px-3 py-2.5 text-left text-xs font-semibold text-rose-700 hover:bg-rose-50">Xóa TBMT</button>@endif</div></template></div>@endif</div></td>
                </tr>
            @empty
