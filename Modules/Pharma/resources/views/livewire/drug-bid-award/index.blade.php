...
<div class="space-y-6">
    ...
    <section class="rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        @if($showFilters)<div class="border-t border-slate-100 p-5">
            ...
            <div class="mt-3 grid gap-4 md:grid-cols-2 xl:grid-cols-12">
                <label class="block text-xs font-semibold text-slate-700 xl:col-span-2">Mã TBMT<x-select-search id="drug-award-filter-tbmt" wire:model="filterTbmt" placeholder="Tất cả Mã TBMT" class="mt-1.5"><option value="">Tất cả Mã TBMT</option>@foreach ($tbmtOptions as $value)<option value="{{ $value }}">{{ $value }}</option>@endforeach</x-select-search></label>
                <label class="block text-xs font-semibold text-slate-700 xl:col-span-4">Chủ đầu tư<x-select-search id="drug-award-filter-investor" wire:model="filterInvestor" placeholder="Tất cả chủ đầu tư" class="mt-1.5"><option value="">Tất cả chủ đầu tư</option>@foreach ($investorOptions as $value)<option value="{{ $value }}">{{ $value }}</option>@endforeach</x-select-search></label>
                <label class="block text-xs font-semibold text-slate-700 xl:col-span-4">Sản phẩm<x-select-search id="drug-award-filter-medicine" wire:model="search" placeholder="Tất cả sản phẩm" class="mt-1.5"><option value="">Tất cả sản phẩm</option>@foreach ($medicineOptions as $value)<option value="{{ $value }}">{{ $value }}</option>@endforeach</x-select-search></label>
                <label class="block text-xs font-semibold text-slate-700 xl:col-span-2">Giá trị<select wire:model.live="valueSort" class="mt-1.5 min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm"><option value="">Mặc định</option><option value="desc">Cao nhất → thấp nhất</option><option value="asc">Thấp nhất → cao nhất</option></select></label>
            </div>
            <div class="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-12">
                <label class="block text-xs font-semibold text-slate-700 xl:col-span-3">Nguồn<select wire:model.live="filterSource" class="mt-1.5 min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm"><option value="">Tất cả nguồn</option>@foreach ($sourceOptions as $value => $label)<option value="{{ $value }}">{{ $label }}</option>@endforeach</select></label>
                <label class="block text-xs font-semibold text-slate-700 xl:col-span-2">Hiển thị<select wire:model.live="perPage" class="mt-1.5 min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm">@foreach ($perPageOptions as $option)<option value="{{ $option }}">{{ $option }} kết quả / trang</option>@endforeach</select></label>
            </div>
        </div>@endif
    </section>
