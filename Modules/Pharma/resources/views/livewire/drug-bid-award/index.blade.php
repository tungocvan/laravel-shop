...
<div class="space-y-6">
    <header class="flex flex-col gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div><p class="text-xs font-semibold uppercase tracking-wide text-indigo-600">Pharma · Procurement Intelligence</p><h1 class="mt-1 text-2xl font-bold text-slate-950 sm:text-3xl">Kết quả trúng thầu thuốc</h1><p class="mt-2 max-w-4xl text-sm text-slate-600">Procurement snapshot luôn giữ theo nguồn. Bệnh viện nhận phân bổ và hợp đồng là nghiệp vụ Pharma riêng, không thay đổi Chủ đầu tư TBMT.</p></div>
        <div class="flex flex-wrap gap-2">@if ($canEdit)<button type="button" wire:click="syncMuasamcong" wire:loading.attr="disabled" wire:target="syncMuasamcong" class="min-h-11 rounded-xl border border-sky-300 bg-sky-50 px-4 py-2.5 text-sm font-semibold text-sky-800 disabled:opacity-60"><span wire:loading.remove wire:target="syncMuasamcong">{{ $syncAfterId ? 'Đồng bộ tiếp KQLCNT' : 'Đồng bộ KQLCNT' }}</span><span wire:loading wire:target="syncMuasamcong">Đang đồng bộ...</span></button>@if ($syncAfterId)<button type="button" wire:click="restartMuasamcongSync" class="min-h-11 rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold">Bắt đầu lại sync</button>@endif @endif @if ($canCreate)<a href="{{ route('admin.pharma.drug-bid-awards.create') }}" class="inline-flex min-h-11 items-center rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white">Thêm hồ sơ mới</a>@endif</div>
    </header>
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4"><div><h2 class="font-semibold text-slate-950">Drug Award Business Catalog</h2><p class="mt-1 text-xs text-slate-500">{{ number_format($awards->total()) }} bản ghi · Trang {{ $currentPage }}/{{ max(1, $lastPage) }}</p></div><div wire:loading wire:target="search,filterTbmt,filterInvestor,filterCompany,filterSource,filterMatchStatus,perPage,gotoPage,syncMuasamcong" class="text-sm font-semibold text-indigo-600">Đang tải...</div></div>
        <div class="overflow-x-auto"><table class="min-w-[1450px] w-full divide-y divide-slate-200 text-left text-sm"><thead class="bg-slate-50 text-xs font-semibold uppercase text-slate-600"><tr>@if ($canSelect)<th class="w-12 px-4 py-3"><input type="checkbox" wire:model.live="selectPage" class="rounded border-slate-300"></th>@endif<th class="w-[18%] px-4 py-3">Thuốc / HSSP</th><th class="w-[14%] px-4 py-3">TBMT / Lô</th><th class="w-[10%] px-4 py-3">Giá & SL</th><th class="w-[17%] px-4 py-3">Chủ đầu tư TBMT</th><th class="w-[13%] px-4 py-3">Nhà thầu</th><th class="w-[11%] px-4 py-3">Quyết định</th><th class="w-[7%] px-4 py-3">Nguồn</th>@if ($canEdit || $canDelete || $canViewAllocations)<th class="w-[10%] min-w-60 px-4 py-3 text-right">Thao tác</th>@endif</tr></thead><tbody class="divide-y divide-slate-100">
            @forelse ($awards as $award)
                @php
                    ...
                    $usesHssp = in_array('hssp', [$name['origin'], $ingredient['origin'], $strength['origin']], true);
                @endphp
                <tr class="align-top hover:bg-slate-50">@if ($canSelect)<td class="px-4 py-4"><input type="checkbox" wire:model.live="selectedIds" value="{{ $award->id }}" class="rounded border-slate-300"></td>@endif<td class="min-w-52 max-w-72 px-4 py-4"><div class="font-semibold text-slate-950">{{ $name['value'] ?: '—' }}</div><div class="mt-1 line-clamp-2 text-xs text-slate-500">{{ $ingredient['value'] ?: '—' }} · {{ $strength['value'] ?: '—' }}</div>@if ($usesHssp)<span class="mt-2 inline-flex rounded-full bg-violet-50 px-2 py-0.5 text-xs font-semibold text-violet-700">Bổ sung từ HSSP</span>@endif <span class="mt-2 inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold">{{ $matchStatusOptions[$award->medicine_match_status] ?? $award->medicine_match_status }}</span></td><td class="min-w-48 px-4 py-4"><div class="font-mono text-xs font-semibold">{{ $award->bidding_notice_code ?: '—' }}</div><div class="mt-1 line-clamp-2 text-xs text-slate-500">Lô {{ $award->lot_no ?: '—' }} · {{ $award->lot_name ?: '—' }}</div></td><td class="min-w-40 px-4 py-4"><div class="font-semibold text-indigo-700">{{ $price !== null ? number_format((float) $price, 0, ',', '.') . ' VNĐ' : '—' }}</div><div class="mt-1 text-xs text-slate-500">SL: {{ $award->quantity !== null ? rtrim(rtrim(number_format((float) $award->quantity, 4, ',', '.'), '0'), ',') : '—' }}</div></td><td class="min-w-56 px-4 py-4"><div class="font-medium">{{ $award->investor_name ?: '—' }}</div><div class="mt-1 text-xs text-slate-500">{{ $award->investor_code ?: '—' }}</div></td><td class="min-w-48 max-w-60 px-4 py-4"><div class="line-clamp-2 font-medium">{{ $award->winning_company_name ?: '—' }}</div><div class="mt-1 text-xs text-slate-500">{{ $award->contractor_code ?: '—' }}</div></td><td class="min-w-44 px-4 py-4"><div>QĐ {{ $award->decision_number ?: '—' }}</div><div class="mt-1 text-xs text-slate-500">{{ $award->decision_date?->format('d/m/Y') ?: '—' }}</div></td><td class="px-4 py-4"><span class="rounded-full bg-sky-50 px-2.5 py-1 text-xs font-semibold text-sky-700">{{ $award->source_type === \Modules\Pharma\Models\DrugBidAward::SOURCE_MUASAMCONG ? 'Mua sắm công' : 'Pharma' }}</span><div class="mt-2 text-xs text-slate-500">{{ $award->sources->count() }} lineage</div></td>
                @if ($canEdit || $canDelete || $canViewAllocations)<td class="min-w-60 px-4 py-4 text-right"><div class="flex flex-wrap justify-end gap-2">@if ($canViewAllocations)<a href="{{ route('admin.pharma.drug-bid-awards.allocations', $award->id) }}" class="rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-semibold text-indigo-700">Phân bổ</a>@endif @if ($canEdit)<a href="{{ route('admin.pharma.drug-bid-awards.edit', $award->id) }}" class="rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold">Sửa</a>@endif @if ($canDelete)<button type="button" wire:click="deleteAward({{ $award->id }})" wire:confirm="Xóa vĩnh viễn hồ sơ trúng thầu này?" class="rounded-lg border border-rose-200 px-3 py-2 text-xs font-semibold text-rose-700">Xóa</button>@endif</div></td>@endif</tr>
            @empty<tr><td colspan="10" class="px-6 py-12 text-center text-slate-500">Không có kết quả phù hợp.</td></tr>@endforelse
        </tbody></table></div>
        @if ($lastPage > 1)<nav class="flex flex-col gap-3 border-t border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><p class="text-sm text-slate-500">Hiển thị {{ $awards->firstItem() }}–{{ $awards->lastItem() }} / {{ $awards->total() }}</p><div class="flex gap-2"><button type="button" wire:click="gotoPage({{ max(1, $currentPage - 1) }})" @disabled($awards->onFirstPage()) class="min-h-10 rounded-xl border border-slate-300 px-3 py-2 text-sm font-semibold disabled:opacity-40">Trước</button><button type="button" wire:click="gotoPage({{ min($lastPage, $currentPage + 1) }})" @disabled(!$awards->hasMorePages()) class="min-h-10 rounded-xl border border-slate-300 px-3 py-2 text-sm font-semibold disabled:opacity-40">Sau</button></div></nav>@endif
    </section>

    @if ($showBulkDeleteModal)<div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4"><div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl"><h2 class="text-lg font-semibold">Xóa các hồ sơ đã chọn?</h2><p class="mt-2 text-sm text-slate-600">Thao tác này xóa {{ count($selectedIds) }} bản ghi trên trang hiện tại.</p><div class="mt-5 flex justify-end gap-2"><button type="button" wire:click="cancelBulkDelete" class="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold">Hủy</button><button type="button" wire:click="deleteSelected" class="rounded-xl bg-rose-600 px-4 py-2 text-sm font-semibold text-white">Xóa</button></div></div></div>@endif
</div>
