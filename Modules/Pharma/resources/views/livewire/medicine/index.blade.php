<div class="space-y-6">
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5" aria-labelledby="medicine-filters-heading">
        ...
        <div class="grid gap-4 xl:grid-cols-12">
            <div class="xl:col-span-4"><label class="block text-sm font-medium text-slate-700">Tìm kiếm</label><input type="search" wire:model.live.debounce.300ms="search" placeholder="MED-..., SKU, tên thuốc, GPLH..." class="mt-1 min-h-11 w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm"></div>
            ...
            <div class="xl:col-span-2"><label class="block text-sm font-medium text-slate-700">Mỗi trang</label><select wire:model.live="perPage" class="mt-1 min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm">@foreach($perPageOptions as $option)<option value="{{ $option }}">{{ $option }} bản ghi</option>@endforeach</select></div>
        </div>
        <div class="mt-4 flex justify-end border-t border-slate-100 pt-4"><button type="button" wire:click="resetFilters" class="inline-flex min-h-10 items-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Xóa bộ lọc</button></div>
    </section>
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div class="hidden overflow-x-auto lg:block">
            <table class="min-w-[1580px] w-full divide-y divide-slate-200 text-left text-sm">
                <thead class="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-600"><tr>@if($canSelect)<th class="w-12 px-4 py-3 text-center"><input type="checkbox" wire:model.live="selectPage" class="rounded border-slate-300 text-indigo-600"></th>@endif<th class="px-4 py-3">Thuốc</th><th class="px-4 py-3">Mã thuốc</th><th class="px-4 py-3">GPLH</th><th class="px-4 py-3">Hoạt chất / Hàm lượng</th><th class="px-4 py-3">Variant / SKU</th><th class="px-4 py-3">Quy cách</th><th class="px-4 py-3">HSSP</th><th class="px-4 py-3">Nguồn</th><th class="px-4 py-3 text-right">Thao tác</th></tr></thead>
        ...
        <div class="divide-y divide-slate-100 lg:hidden">
            @forelse($medicines as $medicine)
                <article class="space-y-4 p-4 {{ in_array((string)$medicine->id, $selectedIds, true) ? 'bg-indigo-50/60' : '' }}">
                    ...
                    <div class="rounded-xl bg-slate-50 p-3">@if($medicine->variants->isNotEmpty()) @foreach($medicine->variants->take(3) as $variant)<div class="mb-1 last:mb-0"><span class="font-mono text-xs font-semibold text-slate-800">{{ $variant->sku }}</span><span class="ml-2 text-xs text-slate-500">{{ $variant->strength_text }}</span></div>@endforeach @if($medicine->variants_count > 3)<div class="mt-1 text-xs text-slate-500">+{{ $medicine->variants_count - 3 }} variant</div>@endif @else <span class="text-xs text-amber-700">Chưa có variant/SKU</span> @endif</div>
