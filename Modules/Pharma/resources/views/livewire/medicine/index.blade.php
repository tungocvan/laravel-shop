<div class="space-y-6">
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div class="hidden overflow-x-auto lg:block">
            <table class="min-w-[1280px] w-full divide-y divide-slate-200 text-left text-sm">
                <thead class="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-600"><tr>@if($canSelect)<th class="w-12 px-4 py-3 text-center"><input type="checkbox" wire:model.live="selectPage" class="rounded border-slate-300 text-indigo-600"></th>@endif<th class="px-4 py-3">Thuốc</th><th class="px-4 py-3">Mã thuốc</th><th class="px-4 py-3">GPLH</th><th class="px-4 py-3">Nhóm thuốc</th><th class="px-4 py-3">Hoạt chất / Hàm lượng</th><th class="px-4 py-3">Quy cách</th><th class="px-4 py-3">Nhà cung cấp</th><th class="px-4 py-3">HSSP</th><th class="px-4 py-3">Nguồn</th><th class="sticky right-0 z-20 border-l border-slate-200 bg-slate-50 px-4 py-3 text-right shadow-[-8px_0_16px_-16px_rgba(15,23,42,0.45)]">Thao tác</th></tr></thead>
                ...
                @forelse($medicines as $medicine)
                    <tr class="align-top hover:bg-slate-50 {{ in_array((string)$medicine->id, $selectedIds, true) ? 'bg-indigo-50/60' : '' }}">
                        ...
                        <td class="sticky right-0 z-10 min-w-56 border-l border-slate-100 bg-white px-4 py-4 text-right shadow-[-8px_0_16px_-16px_rgba(15,23,42,0.35)]"><div class="grid grid-cols-2 gap-2">@if($medicine->currentProfile)<a href="{{ route('admin.pharma.hssp.edit', [$medicine->id, $medicine->currentProfile->id]) }}" class="inline-flex items-center justify-center rounded-lg border border-emerald-200 px-3 py-2 text-xs font-semibold text-emerald-700 hover:bg-emerald-50">HSSP</a>@elseif($canCreate)<a href="{{ route('admin.pharma.hssp.create', $medicine->id) }}" class="inline-flex items-center justify-center rounded-lg border border-amber-200 px-3 py-2 text-xs font-semibold text-amber-700 hover:bg-amber-50">Tạo HSSP</a>@endif @if($canEdit)<a href="{{ route('admin.pharma.supplier-trackings.create', ['medicine_id' => $medicine->id]) }}" class="inline-flex items-center justify-center rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-semibold text-indigo-700 hover:bg-indigo-100">Cập nhật NCC</a><a href="{{ route('admin.pharma.medicines.edit', $medicine->id) }}" class="inline-flex items-center justify-center rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">Sửa thuốc</a>@endif @if($canDelete)<button type="button" @if($medicine->profiles_count > 0) disabled title="Không thể xóa vì thuốc đã có HSSP" @else wire:click="confirmDelete({{ $medicine->id }})" @endif class="inline-flex items-center justify-center rounded-lg border px-3 py-2 text-xs font-semibold {{ $medicine->profiles_count > 0 ? 'cursor-not-allowed border-slate-200 bg-slate-100 text-slate-400 opacity-60' : 'border-rose-200 text-rose-700 hover:bg-rose-50' }}">Xóa</button>@endif</div></td>
                    </tr>
                @empty
