            ...
            <table class="min-w-[1180px] w-full divide-y divide-slate-200 text-left text-sm">
                ...
                @forelse($medicines as $medicine)
                    <tr class="align-top hover:bg-slate-50 {{ in_array((string)$medicine->id, $selectedIds, true) ? 'bg-indigo-50/60' : '' }}">
                        ...
                        <td class="sticky right-0 z-10 w-40 border-l border-slate-100 bg-white px-3 py-4 text-right shadow-[-8px_0_16px_-16px_rgba(15,23,42,0.35)]"><details class="relative inline-block text-left"><summary class="inline-flex min-h-9 cursor-pointer list-none items-center justify-between gap-3 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">Thao tác <span aria-hidden="true">⌄</span></summary><div class="absolute right-0 z-40 mt-1 w-44 overflow-hidden rounded-xl border border-slate-200 bg-white p-1 text-left shadow-xl">@if($medicine->currentProfile)<a href="{{ route('admin.pharma.hssp.edit', [$medicine->id, $medicine->currentProfile->id]) }}" class="block rounded-lg px-3 py-2 text-xs font-semibold text-emerald-700 hover:bg-emerald-50">Mở HSSP</a>@elseif($canCreate)<a href="{{ route('admin.pharma.hssp.create', $medicine->id) }}" class="block rounded-lg px-3 py-2 text-xs font-semibold text-amber-700 hover:bg-amber-50">Tạo HSSP</a>@endif @if($canEdit)<a href="{{ route('admin.pharma.supplier-trackings.create', ['medicine_id' => $medicine->id]) }}" class="block rounded-lg px-3 py-2 text-xs font-semibold text-indigo-700 hover:bg-indigo-50">Điều kiện NCC</a><a href="{{ route('admin.pharma.medicines.edit', $medicine->id) }}" class="block rounded-lg px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">Sửa thuốc</a>@endif @if($canDelete)<button type="button" @if($medicine->profiles_count > 0 || $medicine->drug_bid_awards_count > 0 || $medicine->supplierTrackings->isNotEmpty()) disabled title="Thuốc đang có dữ liệu tham chiếu" @else wire:click="confirmDelete({{ $medicine->id }})" @endif class="block w-full rounded-lg px-3 py-2 text-left text-xs font-semibold {{ ($medicine->profiles_count > 0 || $medicine->drug_bid_awards_count > 0 || $medicine->supplierTrackings->isNotEmpty()) ? 'cursor-not-allowed text-slate-400' : 'text-rose-700 hover:bg-rose-50' }}">Xóa thuốc</button>@endif</div></details></td>
                    </tr>
                @empty
