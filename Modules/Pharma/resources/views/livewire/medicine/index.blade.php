    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-[1450px] w-full divide-y divide-slate-200 text-left text-sm">
                <thead class="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-600"><tr>@if($canSelect)<th class="w-12 px-4 py-3 text-center"><input type="checkbox" wire:model.live="selectPage" class="rounded border-slate-300 text-indigo-600"></th>@endif<th class="px-4 py-3">Thuốc</th><th class="px-4 py-3">Mã / GPLH</th><th class="px-4 py-3">Variant / SKU</th><th class="px-4 py-3">Hoạt chất</th><th class="px-4 py-3">Quy cách</th><th class="px-4 py-3">HSSP</th><th class="px-4 py-3">Nguồn</th><th class="px-4 py-3 text-right">Thao tác</th></tr></thead>
                ...
                @forelse($medicines as $medicine)
                    <tr class="align-top hover:bg-slate-50 {{ in_array((string)$medicine->id, $selectedIds, true) ? 'bg-indigo-50/60' : '' }}">
                        ...
                        <td class="min-w-48 px-4 py-4"><div class="font-mono text-xs font-semibold text-indigo-700">{{ $medicine->medicine_code ?: 'Chưa cấp mã' }}</div><div class="mt-2 font-mono text-xs text-slate-600">{{ $medicine->registration_number ?: 'Chưa có GPLH' }}</div></td>
                        ...
                        <td class="min-w-52 px-4 py-4"><div class="font-medium">{{ $medicine->active_ingredients ?: '—' }}</div><div class="mt-1 text-xs text-slate-500">{{ $medicine->concentration ?: '—' }}</div></td>
