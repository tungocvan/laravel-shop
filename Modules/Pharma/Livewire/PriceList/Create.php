...
class Create extends Component
{
    ...
    public function updatedSpecialControl(): void
    {
        $this->resetProductPage();
    }
    ...
    public function goToStep(int $step): void
    {
        ...
        if ($step >= 2 && ! $this->headerIsValid()) {
            return;
        }

        if ($step >= 3 && $this->selectedRows === []) {
            $this->addError('selectedRows', 'Vui lòng chọn ít nhất một SKU/quy cách trước khi thiết lập giá.');
            $this->step = 2;
    ...
    {
        $keys = $this->productQuery()->get()->map(function ($row): string {
            return $this->rowKey((int) $row->variant_id, $row->package_id ? (int) $row->package_id : null);
        })->all();
    ...
    {
        if ($this->type !== PriceList::TYPE_CUSTOMER) {
            $this->addError('sourceGlobalPriceListId', 'Chỉ bảng giá khách hàng mới có thể khởi tạo từ bảng giá chung.');

            return;
        }

        if (! $this->sourceGlobalPriceListId) {
            $this->addError('sourceGlobalPriceListId', 'Vui lòng chọn bảng giá chung nguồn.');
        ...
        $source = PriceList::query()
            ->with('items')
            ->whereKey($this->sourceGlobalPriceListId)
            ->where('type', PriceList::TYPE_GLOBAL)
            ->where('status', PriceList::STATUS_ACTIVE)
            ->first();

        if (! $source) {
            $this->addError('sourceGlobalPriceListId', 'Bảng giá chung nguồn không còn ở trạng thái ACTIVE.');
        ...
        $this->successMessage = 'Đã khởi tạo '.count($this->selectedRows).' SKU/quy cách từ '.$source->name.'. Bạn chỉ cần điều chỉnh các giá ngoại lệ của khách hàng.';
        $this->step = 3;
    }
    ...
    public function render()
    {
        ...
        $globalPriceLists = PriceList::query()
            ->where('type', PriceList::TYPE_GLOBAL)
            ->where('status', PriceList::STATUS_ACTIVE)
        ...
        $declared = $this->declaredPricesForSelected();
        $missingSale = collect($this->selectedRows)
            ->filter(fn (string $key): bool => ($this->prices[$key]['company'] ?? '') === '')
            ->count();
        $overCeiling = collect($this->selectedRows)->filter(function (string $key) use ($declared): bool {
    ...
    private function headerIsValid(): bool
    {
        ...
        $this->validate([
            ...
            'partnerId' => [$this->type === PriceList::TYPE_CUSTOMER ? 'required' : 'nullable', 'integer'],
    ...
    {
        $rows = $this->productQuery()->get()->map(function ($row) {
    ...
    {
        return DB::table('pharma_medicine_variants as v')
            ...
            ->select([
                ...
                'm.packaging_specification',
                'm.declared_price',
                'm.catalog_status',
                'm.is_special_control',
                'p.id as package_id',
                'p.packaging_text',
                'p.package_code',
            ])
            ->when($this->catalogStatus !== 'all', fn ($query) => $query->where('m.catalog_status', $this->catalogStatus))
            ->when($this->specialControl !== 'all', fn ($query) => $query->where('m.is_special_control', $this->specialControl === 'yes'))
    ...
    private function selectedProductRows(): Collection
    {
        ...
        $selected = array_fill_keys($this->selectedRows, true);
        $variantIds = collect($this->selectedRows)->map(fn (string $key): int => $this->parseRowKey($key)[0])->unique()->values();
        ...
        return DB::table('pharma_medicine_variants as v')
            ...
            ->select([
                'v.id as variant_id', 'v.sku', 'v.strength_text',
                'm.medicine_code', 'm.name', 'm.active_ingredients', 'm.concentration', 'm.registration_number', 'm.declared_price',
                'p.id as package_id', 'p.packaging_text', 'm.packaging_specification',
            ])
            ->orderBy('m.name')
            ->orderBy('v.sku')
            ->get()
            ...
            ->filter(fn ($row): bool => isset($selected[$row->key]))
            ->values();
    }

    private function declaredPricesForSelected(): array
    {
        return $this->selectedProductRows()
            ->mapWithKeys(fn ($row): array => [$row->key => $row->declared_price])
            ->all();
    }
