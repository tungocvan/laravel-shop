...
class Create extends Component
{
    ...
    public function goToStep(int $step): void
    {
        ...
        if ($step >= 4 && $this->includedRows === []) {
            $this->addError('includedRows', 'Vui lòng giữ lại ít nhất một SKU trong bảng giá.');
            $this->step = 3;
    ...
    public function loadFromGlobalPriceList(): void
    {
        ...
        if (! $this->sourceGlobalPriceListId) {
            $this->addError('sourceGlobalPriceListId', 'Vui lòng chọn bảng giá chung ACTIVE làm nguồn.');
    ...
    {
        if (! in_array($key, $this->selectedRows, true) || ! in_array($field, ['company', 'receivable', 'invoice'], true)) {
            return;
        }
    ...
    public function applyDiscount(): void
    {
        ...
        foreach ($this->declaredPricesForKeys($this->includedRows) as $key => $declared) {
            if ($declared !== null) {
                $this->prices[$key]['receivable'] = round((float) $declared * (100 - $discount) / 100, 2);
            }
        }
    }
    ...
    public function saveDraft(): void
    {
        ...
        if ($this->includedRows === []) {
            $this->addError('includedRows', 'Bảng giá phải có ít nhất một SKU/quy cách.');
            $this->step = 3;
    ...
    {
        return $this->redirectRoute('admin.pharma.price-lists.index', navigate: true);
    }
    ...
    private function headerIsValid(): bool
    {
        ...
        try {
            $this->manager->validateHeader($this->headerPayload());
        } catch (Throwable $exception) {
            $this->errorMessage = $exception->getMessage();
    ...
    {
        return ['name' => $this->name, 'code' => $this->code, 'type' => $this->type, 'partner_id' => $this->partnerId, 'effective_from' => $this->effectiveFrom ?: null, 'effective_to' => $this->effectiveTo ?: null, 'currency' => strtoupper($this->currency), 'priority' => $this->priority, 'notes' => $this->notes ?: null];
    }
    ...
    private function selectedProductRows(): Collection
    {
        ...
        return $this->productQuery(false)->get()->map(function ($row) {
            $row->key = $this->rowKey((int) $row->variant_id, $row->package_id ? (int) $row->package_id : null);

            return $row;
        })->filter(fn ($row): bool => isset($selected[$row->key]))->values();
    }
    ...
    private function initializeSelectedPrices(array $keys): void
    {
        ...
        foreach ($keys as $key) {
            if (! in_array($key, $this->selectedRows, true) || isset($this->prices[$key])) {
                continue;
