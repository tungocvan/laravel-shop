...
class ExportConfigurator extends Component
{
    ...
    public function moveColumnToGroup(string $key, string $group, ?string $beforeKey = null): void
    {
        ...
        if ($targetIndex === false) {
            ...
            if ($groupKeys === []) {
                ...
                $lastIndex = array_search($lastGroupKey, $order, true);
                array_splice($order, $lastIndex + 1, 0, [$key]);
            }
        } else {
            array_splice($order, $targetIndex, 0, [$key]);
        }
    ...
    public function save(): void
    {
        ...
        $saved=app(PriceListExportProfileService::class)->save((int)auth('admin')->id(),['name'=>$this->profileName,'is_default'=>$this->isDefault,'column_order'=>$this->columnOrder,'column_groups'=>$this->columnGroupsMap,'selected_columns'=>$selected,'headers'=>$this->headers,'alignments'=>$this->alignments,'widths'=>$this->widths,'data_types'=>$this->dataTypes,'decimals'=>$this->decimals,'header_footer'=>$this->headerFooter,'page_setup'=>$this->pageSetup,'logo_path'=>$this->storeMedia($this->logoUpload,'logo',$this->logoPath),'signature_path'=>$this->storeMedia($this->signatureUpload,'signature',$this->signaturePath)],$this->profileId);
