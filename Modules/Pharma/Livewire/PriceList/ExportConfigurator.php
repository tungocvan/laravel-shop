...
class ExportConfigurator extends Component
{
    ...
    public function addColumn(string $key):void{if(!isset(PriceListExportProfileService::COLUMNS[$key]))return;$this->selectedColumns[$key]=true;$this->compactSelectedColumns();$this->activeColumnKey=$key;}
    public function removeColumn(string $key):void{if(!isset(PriceListExportProfileService::COLUMNS[$key]))return;$this->selectedColumns[$key]=false;$this->compactSelectedColumns();if($this->activeColumnKey===$key)$this->activeColumnKey=$this->selectedOrder()[0]??null;}
    public function moveColumn(string $key,int $offset):void{$selected=$this->selectedOrder();$index=array_search($key,$selected,true);if($index===false)return;$target=max(0,min(count($selected)-1,$index+$offset));if($target===$index)return;array_splice($selected,$index,1);array_splice($selected,$target,0,[$key]);$this->replaceSelectedOrder($selected);$this->activeColumnKey=$key;}
    public function setColumnPosition(string $key,int $position):void{if(!isset(PriceListExportProfileService::COLUMNS[$key])||!($this->selectedColumns[$key]??false))return;$selected=array_values(array_filter($this->selectedOrder(),fn($columnKey)=>$columnKey!==$key));$position=max(1,min(count($selected)+1,$position));array_splice($selected,$position-1,0,[$key]);$this->replaceSelectedOrder($selected);$this->activeColumnKey=$key;}
    ...
    private function apply(array $p):void{$this->profileId=$p['profile_id'];$this->profileName=$p['profile_name'];$this->isDefault=$p['is_default'];$this->columnOrder=$p['column_order'];$this->columnGroupsMap=[];foreach(array_keys(PriceListExportProfileService::COLUMNS)as$k)$this->columnGroupsMap[$k]=PriceListExportProfileService::COLUMNS[$k]['group'];$lookup=array_fill_keys($p['selected_columns'],true);$this->selectedColumns=[];foreach($this->columnOrder as$k)$this->selectedColumns[$k]=isset($lookup[$k]);$this->compactSelectedColumns();$this->headers=$p['headers'];$this->alignments=$p['alignments'];$this->widths=$p['widths'];$this->dataTypes=$p['data_types'];$this->decimals=$p['decimals'];$this->headerFooter=$p['header_footer'];$this->pageSetup=$p['page_setup'];$this->pageSetup['table_border']=$this->pageSetup['table_border']??'thin';$this->pageSetup['header_background']=$this->pageSetup['header_background']??'#E8EEF9';$this->pageSetup['header_text_color']=$this->pageSetup['header_text_color']??'#111827';$this->logoPath=$p['logo_path'];$this->signaturePath=$p['signature_path'];$this->originalLogoPath=$this->logoPath;$this->originalSignaturePath=$this->signaturePath;$this->removeLogoRequested=false;$this->removeSignatureRequested=false;$this->activeColumnKey=$this->activeColumnKey&&isset(PriceListExportProfileService::COLUMNS[$this->activeColumnKey])?$this->activeColumnKey:($this->selectedOrder()[0]??$this->columnOrder[0]??null);}
