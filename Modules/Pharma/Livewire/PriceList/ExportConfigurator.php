...
class ExportConfigurator extends Component
{
    ...
    public function setColumnWidthPreset(string $preset):void{if(!$this->activeColumnKey)return;$width=match($preset){'xs'=>10,'s'=>14,'m'=>20,'l'=>28,'xl'=>36,default=>PriceListExportProfileService::COLUMNS[$this->activeColumnKey]['width']??18};$this->columnDraft['width']=$width;$this->commitColumnDraft();}
    ...
    private function apply(array $p):void{$this->profileId=$p['profile_id'];$this->profileName=$p['profile_name'];$this->isDefault=$p['is_default'];$this->columnOrder=$p['column_order'];$this->columnGroupsMap=[];foreach(array_keys(PriceListExportProfileService::COLUMNS)as$k)$this->columnGroupsMap[$k]=PriceListExportProfileService::COLUMNS[$k]['group'];$lookup=array_fill_keys($p['selected_columns'],true);$this->selectedColumns=[];foreach($this->columnOrder as$k)$this->selectedColumns[$k]=isset($lookup[$k]);$this->compactSelectedColumns();$this->headers=$p['headers'];$this->alignments=$p['alignments'];$this->widths=$p['widths'];$this->dataTypes=$p['data_types'];$this->decimals=$p['decimals'];$this->headerFooter=$p['header_footer'];$this->pageSetup=$p['page_setup'];$this->pageSetup['table_border']=$this->pageSetup['table_border']??'thin';$this->pageSetup['header_background']=$this->pageSetup['header_background']??'#E8EEF9';$this->pageSetup['header_text_color']=$this->pageSetup['header_text_color']??'#111827';$this->logoPath=$p['logo_path'];$this->signaturePath=$p['signature_path'];$this->originalLogoPath=$this->logoPath;$this->originalSignaturePath=$this->signaturePath;$this->removeLogoRequested=false;$this->removeSignatureRequested=false;$this->activeColumnKey=$this->activeColumnKey&&isset(PriceListExportProfileService::COLUMNS[$this->activeColumnKey])?$this->activeColumnKey:($this->selectedOrder()[0]??$this->columnOrder[0]??null);$this->loadColumnDraft($this->activeColumnKey);}
