...
class TableList extends Component
{
    ...
    {
        if ($value) {
            $this->selectedTables = array_column($this->service->getAllTables($this->search, $this->moduleFilter), 'name');
    ...
    public function openModuleRestoreModal(string $reference, ModuleSnapshotService $snapshots): void
    {
        ...
        try {
            ...
            } $validated = $snapshots->validatePackage($snapshot['absolute_path'], $module, enforceSchema: true);
            $this->selectedModulePreflight = (array) ($validated['compatibility_report'] ?? ['status' => $validated['compatibility'], 'issues' => []]);
    ...
    public function importTable(): void
    {
        ...
        try {
            $this->service->importTableFromFile($tableName, $this->importFile->getRealPath());
    ...
    public function dropTable(string $tableName): void
    {
        ...
        try {
            ...
            $this->selectedTables = array_values(array_filter($this->selectedTables, static fn (string $selected): bool => $selected !== $tableName));
        } catch (\Throwable $e) {
