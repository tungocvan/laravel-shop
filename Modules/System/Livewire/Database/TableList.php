...
class TableList extends Component
{
    ...
    public function mount(string $moduleFilter = ''): void
    {
        $this->moduleFilter = trim($moduleFilter);
        $this->refreshModuleSnapshots();
    }

    public function updatedSearch(): void { $this->resetVisibleSelectionState(); }
    public function updatedModuleFilter(): void { $this->resetVisibleSelectionState(); $this->resetModuleSnapshotState(); $this->refreshModuleSnapshots(); }
    public function updatedSelectAll(bool $value): void
    {
        if ($value) { $this->selectedTables = array_column($this->service->getAllTables($this->search, $this->moduleFilter), 'name'); return; }
        $this->selectedTables = [];
    }
    ...
    public function refreshModuleSnapshots(): void
    {
        if ($this->moduleFilter === '' || $this->moduleFilter === 'Unknown') {
            $this->moduleLocalSnapshots = []; $this->moduleRemoteSnapshots = []; $this->moduleDependencies = [];
            $this->moduleDriveConnected = false; $this->moduleCloudUnavailable = false; return;
        }
        ...
        try {
            $this->moduleDriveConnected = (bool) (app(GoogleDriveConnectionService::class)->status()['connected'] ?? false);
        } catch (\Throwable $e) {
            $this->moduleDriveConnected = false;
            $this->moduleCloudUnavailable = true;
            $this->reportOperationError('Module snapshot Google Drive status failed.', $e, ['module' => $this->moduleFilter]);
        }

        try {
            $this->moduleDependencies = app(ModuleDependencyService::class)->dependenciesFor($this->moduleFilter);
            $this->moduleLocalSnapshots = app(ModuleSnapshotService::class)->listLocal($this->moduleFilter, 30);
        } catch (\Throwable $e) {
            $this->moduleLocalSnapshots = []; $this->moduleDependencies = [];
            $this->reportOperationError('Module snapshot local catalog refresh failed.', $e, ['module' => $this->moduleFilter]);
        }
    ...
    public function restoreModuleSnapshot(ModuleSnapshotService $snapshots): void
    {
        ...
        try { $result = $snapshots->restore($this->selectedModuleSnapshotReference, $module); $this->showModuleRestoreModal = false; $this->selectedModuleSnapshotReference = null; $this->refreshModuleSnapshots(); $this->notify('success', "Restore Module {$module} thành công. Safety Snapshot: {$result['safety_snapshot']['name']}."); }
        catch (\Throwable $e) { $this->reportOperationError('Module snapshot restore failed.', $e, ['module' => $module, 'snapshot' => $this->selectedModuleSnapshotReference]); $this->notify('error', 'Restore Module thất bại. Hệ thống đã cố gắng rollback an toàn; vui lòng kiểm tra log.'); }
        finally { $this->isModuleRestoring = false; }
    }

    public function backupFull(): void { $this->authorizePermission('database.backup'); try { $this->service->backupFullDatabase(); $this->notify('success', 'Backup toàn bộ dữ liệu thành công!'); } catch (\Throwable $e) { $this->reportOperationError('Full database backup failed.', $e); $this->notify('error', 'Backup database thất bại. Vui lòng kiểm tra log hệ thống.'); } }
    public function exportTable(string $tableName): void { $this->authorizePermission('database.backup'); try { $this->service->backupTable($tableName); $this->notify('success', "Export bảng {$tableName} thành công!"); } catch (\Throwable $e) { $this->reportOperationError('Table export failed.', $e, ['table' => $tableName]); $this->notify('error', 'Export bảng thất bại. Vui lòng kiểm tra log hệ thống.'); } }
    public function exportSelected(): void
    {
        $this->authorizePermission('database.backup'); if ($this->selectedTables === []) { $this->notify('error', 'Vui lòng chọn ít nhất một bảng để export.'); return; }
        try { $fileName = $this->service->backupTablesAsZip($this->selectedTables); $this->selectedExportFile = $this->service->getBackupReference($fileName, ['zip']); if ($this->selectedExportFile === null) throw new \RuntimeException('Không thể tạo download reference cho file ZIP.'); $this->notify('success', 'Đã export các bảng đã chọn thành file ZIP!'); }
    ...
    public function restoreTable(string $tableName): void { $this->authorizePermission('database.restore'); try { if (! $this->service->restoreTable($tableName)) { $this->notify('error', 'Không tìm thấy file backup của bảng đã chọn.'); return; } $this->notify('success', "Restore bảng {$tableName} thành công!"); } catch (\Throwable $e) { $this->reportOperationError('Table restore failed.', $e, ['table' => $tableName]); $this->notify('error', 'Restore bảng thất bại. Vui lòng kiểm tra log hệ thống.'); } }
    public function openImportModal(string $tableName): void { $this->authorizePermission('database.restore'); try { $this->service->assertAllowedTable($tableName); $this->resetValidation('importFile'); $this->importFile = null; $this->importTargetTable = $tableName; $this->showImportModal = true; } catch (\Throwable $e) { $this->reportOperationError('Open table import rejected.', $e, ['table' => $tableName]); $this->notify('error', 'Không thể import vào bảng đã chọn.'); } }
    public function closeImportModal(): void { if ($this->isImporting) return; $this->showImportModal = false; $this->importTargetTable = null; $this->importFile = null; $this->resetValidation('importFile'); }
    public function importTable(): void
    {
        $this->authorizePermission('database.restore'); if ($this->isImporting || ! $this->importTargetTable) return;
        ...
        try { $this->service->importTableFromFile($tableName, $this->importFile->getRealPath()); $this->notify('success', "Import bảng {$tableName} thành công!"); $this->closeImportStateAfterSuccess(); }
    ...
    public function truncateTable(string $tableName): void { $this->authorizePermission('database.destroy'); try { $this->service->truncateTable($tableName); $this->notify('success', "Đã làm sạch dữ liệu bảng {$tableName}"); } catch (\Throwable $e) { $this->reportOperationError('Table truncate failed.', $e, ['table' => $tableName]); $this->notify('error', 'Không thể làm sạch bảng đã chọn.'); } }
    public function dropTable(string $tableName): void { $this->authorizePermission('database.destroy'); try { $this->service->dropTable($tableName); $this->notify('success', "Đã xóa bảng {$tableName}"); $this->selectedTables = array_values(array_filter($this->selectedTables, static fn (string $selected): bool => $selected !== $tableName)); } catch (\Throwable $e) { $this->reportOperationError('Table drop failed.', $e, ['table' => $tableName]); $this->notify('error', 'Không thể xóa bảng đã chọn.'); } }
    public function openRestoreModal(): void { $this->authorizePermission('database.restore'); $this->backupFiles = array_values(array_filter($this->service->getAllBackupFiles(), static fn (array $file): bool => $file['is_full'])); $this->showRestoreModal = true; }
    public function closeRestoreModal(): void { if ($this->isRestoring) return; $this->showRestoreModal = false; $this->selectedBackupFile = null; }
    public function restoreDatabase(): void
    {
        $this->authorizePermission('database.restore'); if ($this->isRestoring) return; if (! $this->selectedBackupFile) { $this->notify('error', 'Vui lòng chọn file backup.'); return; }
    ...
    {
        return view('System::livewire.database.table-list-with-dependencies', ['tables' => $this->service->getAllTables($this->search, $this->moduleFilter), 'modules' => $this->service->getModuleOptions(), 'canBackup' => (bool) auth('admin')->user()?->can('database.backup'), 'canDownload' => (bool) auth('admin')->user()?->can('database.download'), 'canRestore' => (bool) auth('admin')->user()?->can('database.restore'), 'canDestroy' => (bool) auth('admin')->user()?->can('database.destroy')]);
    }
