...
class SystemMailConfigService
{
    ...
    public function publicConfig(): array
    {
        ...
        $result['MAIL_ENCRYPTION'] = $this->normalizeEncryption($result['MAIL_ENCRYPTION'] ?? null);
    ...
    public function test(array $form, string $recipient, ?int $actorId = null): array
    {
        ...
        $lockKey = 'system:mail-config:test:'.($actorId ?? 'guest');
        $lock = Cache::lock($lockKey, 5);

        if (! $lock->get()) {
            return [
                'success' => false,
                'message' => 'Vui lòng chờ vài giây trước khi gửi lại email kiểm tra.',
            ];
        }
    ...
    public function save(array $form, ?int $actorId = null): array
    {
        ...
        if (! $lock->get()) {
            return [
                'success' => false,
                'message' => 'Một thao tác cập nhật cấu hình Email khác đang được thực hiện.',
            ];
        }

        try {
            $updated = $this->envManager->update($candidate);
            if (! $updated) {
            ...
            Log::error('Mail configuration save failed.', [
                'actor_id' => $actorId,
                'exception' => $e::class,
            ]);
    ...
    {
        $unknown = array_diff(array_keys($form), self::WRITABLE_KEYS);
        if ($unknown !== []) {
