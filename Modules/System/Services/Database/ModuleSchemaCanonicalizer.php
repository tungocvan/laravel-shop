...
final class ModuleSchemaCanonicalizer
{
    public function legacySqlMatchesCurrent(string $sql, array $tables): bool
    {
        ...
        $current = $this->current($tables);

        return $snapshot !== [] && $current !== [] && $this->schemasEquivalent($snapshot, $current, $tables);
    }

    public function schemasEquivalent(array $expected, array $current, array $tables): bool
    {
        foreach ($tables as $table) {
            $left = (array) ($expected[$table] ?? []);
            $right = (array) ($current[$table] ?? []);
            if ($left === [] || $right === []) {
                return false;
            }
            ...
            if (! $this->foreignKeysEquivalent((array) ($left['foreign_keys'] ?? []), (array) ($right['foreign_keys'] ?? []))) {
                return false;
            }
            if (! $this->indexesEquivalent((array) ($left['indexes'] ?? []), (array) ($right['indexes'] ?? []), (array) ($left['foreign_keys'] ?? []), (array) ($right['foreign_keys'] ?? []))) {
                return false;
            }
        }
    ...
    public function current(array $tables): array
    {
        ...
        foreach ($tables as $table) {
            ...
            try {
                ...
                $indexRows = DB::select('SHOW INDEX FROM '.$quoted);
            } catch (\Throwable) {
                return [];
            }
            $columns = [];
            foreach ($columnRows as $row) {
                $data = (array) $row;
                $columns[(string) $data['Field']] = $this->column(['type' => (string) $data['Type'], 'null' => (string) $data['Null'], 'default' => $data['Default'], 'extra' => (string) $data['Extra']]);
            }
            ...
            $schema[(string) $table] = ['columns' => $columns, 'indexes' => $this->named($indexes, true), 'foreign_keys' => $this->named($foreignKeys, false)];
        }
        ...
        foreach ($tables as $table) {
            ...
            foreach (preg_split('/\R/', (string) $match[1]) ?: [] as $rawLine) {
                ...
                if (preg_match('/^`([^`]+)`\s+([^\s]+)(.*)$/i', $line, $columnMatch)) {
                    ...
                    $columns[$columnMatch[1]] = $this->column(['type' => $type, 'null' => stripos($tail, 'NOT NULL') !== false ? 'NO' : 'YES', 'default' => $this->sqlDefault($tail), 'extra' => $this->sqlExtra($tail)]);
                    continue;
                }
                ...
                if (preg_match('/^CONSTRAINT `([^`]+)` FOREIGN KEY \(`([^`]+)`\) REFERENCES `([^`]+)` \(`([^`]+)`\)/i', $line, $fkMatch)) {
                    $foreignKeys[$fkMatch[1]][] = ['column' => $fkMatch[2], 'referenced_table' => $fkMatch[3], 'referenced_column' => $fkMatch[4]];
                }
            }
            ksort($columns, SORT_STRING);
            $schema[(string) $table] = ['columns' => $columns, 'indexes' => $this->named($indexes, true), 'foreign_keys' => $this->named($foreignKeys, false)];
    ...
    private function foreignKeysEquivalent(array $expected, array $current): bool
    {
        return $this->structureSignatures($expected, false) === $this->structureSignatures($current, false);
    }

    private function indexesEquivalent(array $expected, array $current, array $expectedFks, array $currentFks): bool
    {
        $expectedSignatures = $this->structureSignatures($expected, true);
        $currentSignatures = $this->structureSignatures($current, true);
        if ($expectedSignatures === $currentSignatures) {
            return true;
        }

        $fkPrefixes = array_unique(array_merge($this->foreignKeyColumnPrefixes($expectedFks), $this->foreignKeyColumnPrefixes($currentFks)));
        $filter = static fn (array $signatures): array => array_values(array_filter($signatures, static function (string $signature) use ($fkPrefixes): bool {
            [$unique, $columns] = array_pad(explode('|', $signature, 2), 2, '');
            if ($unique === '1') {
                return true;
            }
            foreach ($fkPrefixes as $prefix) {
                if ($columns === $prefix || str_starts_with($columns, $prefix.',')) {
                    return false;
                }
            }
            return true;
        ...
        return $filter($expectedSignatures) === $filter($currentSignatures)
            && $this->allForeignKeysSupported($expectedFks, $expected)
            && $this->allForeignKeysSupported($currentFks, $current);
    }

    private function allForeignKeysSupported(array $foreignKeys, array $indexes): bool
    {
        $indexColumns = array_map(fn (array $parts): array => array_column($parts, 'column'), $indexes);
        foreach ($foreignKeys as $parts) {
            $required = array_column((array) $parts, 'column');
            $supported = false;
    ...
    private function foreignKeyColumnPrefixes(array $foreignKeys): array
    {
        $result = [];
        foreach ($foreignKeys as $parts) {
            $result[] = implode(',', array_column((array) $parts, 'column'));
        }
        sort($result, SORT_STRING);
        return $result;
    }

    private function structureSignatures(array $structures, bool $index): array
    {
        $signatures = [];
        foreach ($structures as $parts) {
            if ($index) {
                $signatures[] = ((bool) ($parts[0]['unique'] ?? false) ? '1' : '0').'|'.implode(',', array_column((array) $parts, 'column'));
            } else {
                $signatures[] = implode(',', array_map(static fn (array $part): string => ($part['column'] ?? '').'>'.($part['referenced_table'] ?? '').'.'.($part['referenced_column'] ?? ''), (array) $parts));
            }
        }
    ...
    private function column(array $definition): array
    {
        ...
        return ['type' => $type, 'null' => strtoupper((string) ($definition['null'] ?? '')), 'default' => $definition['default'] ?? null, 'extra' => strtolower(trim(preg_replace('/\s+/', ' ', (string) ($definition['extra'] ?? '')) ?? ''))];
    }
    ...
    private function named(array $structures, bool $index): array
    {
        ...
        foreach ($structures as $name => $parts) {
            ...
            usort($items, static fn (array $a, array $b): int => $index ? [$a['sequence'], $a['column']] <=> [$b['sequence'], $b['column']] : [$a['column'], $a['referenced_table'], $a['referenced_column']] <=> [$b['column'], $b['referenced_table'], $b['referenced_column']]);
