        ...
        <div class="fixed inset-0 z-[10020] flex items-center justify-center bg-gray-950/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true">
            <div class="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
                <div class="flex items-start justify-between gap-4 border-b border-gray-100 px-5 py-4 sm:px-6">
                    <div><p class="text-xs font-bold uppercase tracking-wider text-indigo-600">Schema Doctor · {{ $module }}</p><h2 class="mt-1 text-lg font-bold text-gray-950">Kết quả chẩn đoán</h2></div>
                ...
                <div class="space-y-4 p-5 sm:p-6">
                    ...
                    <div class="rounded-xl border p-4 {{ $verdict === 'SAFE' ? 'border-emerald-200 bg-emerald-50' : ($verdict === 'REVIEW' ? 'border-amber-200 bg-amber-50' : 'border-red-200 bg-red-50') }}">
                        <div class="flex flex-wrap items-center gap-2"><span class="rounded-full px-2.5 py-1 text-xs font-bold {{ $verdict === 'SAFE' ? 'bg-emerald-600 text-white' : ($verdict === 'REVIEW' ? 'bg-amber-600 text-white' : 'bg-red-600 text-white') }}">{{ $verdict }}</span><span class="text-sm font-semibold text-gray-900">{{ $report['summary'] }}</span></div>
                    </div>
                        ...
                        <div class="space-y-3">
                            @foreach ($report['issues'] as $issue)
                                <article class="rounded-xl border border-gray-200 bg-white p-4">
                                    <div class="flex flex-wrap items-center gap-2"><span class="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-bold text-gray-700">{{ $issue['risk'] ?? 'REVIEW' }}</span><span class="text-xs font-semibold uppercase tracking-wide text-gray-500">{{ str_replace('_', ' ', $issue['type'] ?? 'schema') }}</span></div>
                                    ...
                                    @if (($issue['differences'] ?? []) !== [])
                                        <div class="mt-3 overflow-hidden rounded-xl border border-gray-200">
                                            <div class="grid grid-cols-[minmax(90px,0.8fr)_minmax(120px,1fr)_minmax(120px,1fr)] bg-gray-50 px-3 py-2 text-xs font-bold uppercase tracking-wide text-gray-500"><span>Thuộc tính</span><span>Snapshot</span><span>Hiện tại</span></div>
                                            @foreach ($issue['differences'] as $attribute => $difference)
                                                <div class="grid grid-cols-[minmax(90px,0.8fr)_minmax(120px,1fr)_minmax(120px,1fr)] gap-2 border-t border-gray-100 px-3 py-2 text-xs sm:text-sm"><code class="font-bold text-gray-700">{{ $attribute }}</code><code class="break-all text-amber-800">{{ var_export($difference['snapshot'] ?? null, true) }}</code><code class="break-all text-indigo-800">{{ var_export($difference['current'] ?? null, true) }}</code></div>
                                            @endforeach
                                        </div>
                                    @endif
                    ...
                    <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm leading-6 text-gray-700">
                        <div class="flex flex-wrap items-center justify-between gap-2">
                            ...
                            @if (($repairPlan['all_safe'] ?? false))<span class="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-bold text-emerald-800">PLAN SAFE</span>@else<span class="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-bold text-amber-800">CẦN REVIEW</span>@endif
                        </div>
                        @if (($repairPlan['steps'] ?? []) !== [])
                            <div class="mt-3 space-y-2">
                                @foreach ($repairPlan['steps'] as $step)
                        ...
                        @else
                            <p class="mt-2">Không có remediation step tự động từ evidence hiện tại.</p>
                        @endif
                        <p class="mt-3 border-t border-gray-200 pt-3 text-xs text-gray-600">Execution vẫn bị khóa ở batch này. Chỉ khi toàn bộ plan được chứng minh SAFE mới triển khai executor có Safety Snapshot → xác nhận → sửa metadata/schema → chẩn đoán lại → kiểm tra fingerprint. Không DROP cột/bảng và không ép Restore.</p>
                    </div>
                </div>

                <div class="flex justify-end border-t border-gray-100 bg-gray-50 px-5 py-4 sm:px-6"><button type="button" wire:click="close" class="rounded-xl bg-gray-900 px-5 py-2.5 text-sm font-bold text-white hover:bg-gray-800">Đóng</button></div>
            </div>
        </div>
    @endif
</div>
