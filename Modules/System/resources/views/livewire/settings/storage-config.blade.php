        ...
        <div class="xl:col-span-2 space-y-5">
            ...
            <details class="group rounded-xl border border-indigo-200 bg-indigo-50/40 p-5">
                <summary class="cursor-pointer list-none flex items-center justify-between gap-4">
                    <div><h4 class="text-sm font-black text-indigo-950 uppercase tracking-wide">Hướng dẫn kết nối Google Drive / thêm Test User</h4><p class="mt-1 text-xs text-indigo-700">Dùng khi triển khai website mới, đổi Google Cloud Project, chạy VPS/Docker hoặc kết nối tài khoản Google khác.</p></div>
                ...
                <div class="mt-5 space-y-5 text-sm text-gray-700">
                    ...
                    <div class="rounded-xl border border-sky-200 bg-white p-4">
                        <div class="font-bold text-sky-950">E. Triển khai trên VPS / Docker</div>
                        <ol class="mt-2 list-decimal pl-5 space-y-2 text-xs leading-5">
                            <li>Redirect URI phải dùng <strong>domain public HTTPS</strong> mà trình duyệt truy cập, không dùng tên container như <code>app</code>, <code>nginx</code>, IP nội bộ Docker hoặc <code>localhost</code>. Ví dụ: <code>https://example.com/admin/system/settings/cloud/google/callback</code>.</li>
                            <li>Đảm bảo <code>APP_URL</code> của production là URL HTTPS public đúng domain. Nếu SSL kết thúc ở Nginx/Traefik/Cloudflare reverse proxy, Laravel vẫn phải nhận đúng scheme HTTPS qua trusted proxy/forwarded headers để tránh sinh callback <code>http://</code>.</li>
                            <li>Khai báo các biến <code>GOOGLE_DRIVE_CLIENT_ID</code>, <code>GOOGLE_DRIVE_CLIENT_SECRET</code>, <code>GOOGLE_DRIVE_REDIRECT_URI</code>, <code>GOOGLE_DRIVE_FOLDER_NAME</code> trong nguồn environment thực sự của container. Nếu Docker Compose dùng <code>env_file: .env</code> thì chỉnh file đó trên VPS; nếu dùng <code>environment:</code> hoặc secret manager thì chỉnh tại nơi tương ứng. Không chỉ sửa một file .env nằm ngoài container nếu Compose không nạp file đó.</li>
                            <li>Sau khi thay đổi environment, chạy <code>php artisan optimize:clear</code> bên trong container Laravel. Nếu biến environment được Docker inject khi tạo container, cần <strong>recreate/restart container</strong> để nhận giá trị mới.</li>
                            <li>Kiểm tra route callback tồn tại trong container bằng <code>php artisan route:list --path=admin/system</code>, sau đó mở tab Lưu trữ Cloud và xác nhận ô Authorized Redirect URI đúng domain HTTPS.</li>
                            <li>Trong Google Cloud OAuth Client, Authorized redirect URI phải khớp <strong>100%</strong> với production: scheme, domain, port (nếu có) và path. Redirect URI local và production có thể cùng tồn tại trong một OAuth Client.</li>
                            <li>Container PHP cần truy cập Internet qua HTTPS tới <code>accounts.google.com</code>, <code>oauth2.googleapis.com</code> và Google APIs. Nếu VPS/firewall/proxy outbound chặn HTTPS, OAuth hoặc upload Drive sẽ thất bại.</li>
                            <li>Đảm bảo giờ hệ thống/container tương đối chính xác (NTP trên VPS), vì OAuth token có thời hạn. Sai lệch thời gian lớn có thể gây lỗi token khó chẩn đoán.</li>
                            <li>Khi Phase backup queue được bật, worker queue phải dùng cùng environment và storage/database với web container. Sau khi đổi cấu hình deployment, restart worker để worker không giữ config cũ.</li>
                            <li>Không bake Client Secret/token vào Docker image hoặc commit vào Git. Production secret nên nằm trong environment/secret của VPS; access token và refresh token runtime vẫn được hệ thống mã hóa bằng <code>APP_KEY</code> trước khi lưu database.</li>
                        </ol>
                        <div class="mt-3 rounded-lg bg-sky-50 px-3 py-2 text-xs leading-5 text-sky-900"><strong>Kiểm tra nhanh trong container:</strong> <code>php artisan optimize:clear</code> → <code>php artisan route:list --path=admin/system</code> → mở <code>/admin/system/settings/env?tab=storage</code> → Kiểm tra kết nối.</div>
                    </div>
