<div class="mt-6 rounded-xl border border-emerald-200 bg-white p-6 shadow-sm">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        ...
        <button wire:click="runAutomationNow" wire:loading.attr="disabled" wire:target="runAutomationNow" @disabled(!$canManageAutomation)
    ...
    @unless ($canManageAutomation)
        <div class="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            Quản lý lịch cần đồng thời các quyền <code>system.env.update</code>, <code>database.backup</code> và <code>database.destroy</code> vì một lần chạy có thể tạo backup và áp dụng retention.
        </div>
    @endunless
    ...
    <div class="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
        <label class="rounded-lg border border-gray-200 p-4"><span class="block text-xs font-bold uppercase text-gray-500">Trạng thái lịch</span><span class="mt-3 flex items-center gap-2 text-sm font-semibold text-gray-800"><input type="checkbox" wire:model="automation.enabled" @disabled(!$canManageAutomation) class="rounded border-gray-300">Bật backup hằng ngày</span></label>
        <div class="rounded-lg border border-gray-200 p-4"><label class="block text-xs font-bold uppercase text-gray-500">Giờ chạy</label><input type="time" wire:model="automation.time" @disabled(!$canManageAutomation) class="mt-2 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm disabled:bg-gray-100">@error('automation.time')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
        <div class="rounded-lg border border-gray-200 p-4"><label class="block text-xs font-bold uppercase text-gray-500">Giữ local</label><input type="number" min="1" max="365" wire:model="automation.local_retention" @disabled(!$canManageAutomation) class="mt-2 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm disabled:bg-gray-100"><p class="mt-1 text-xs text-gray-400">Số full backup mới nhất.</p></div>
        <div class="rounded-lg border border-gray-200 p-4"><label class="block text-xs font-bold uppercase text-gray-500">Giữ Google Drive</label><input type="number" min="1" max="365" wire:model="automation.drive_retention" @disabled(!$canManageAutomation) class="mt-2 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm disabled:bg-gray-100"><p class="mt-1 text-xs text-gray-400">Số backup remote mới nhất.</p></div>
    </div>

    <label class="mt-4 flex items-center gap-2 text-sm text-gray-700"><input type="checkbox" wire:model="automation.upload_drive" @disabled(!$canManageAutomation) class="rounded border-gray-300">Tự động đưa backup mới vào queue upload Google Drive</label>

    <div class="mt-5 flex flex-wrap items-center gap-3 border-t border-gray-100 pt-5">
        <button wire:click="saveAutomation" wire:loading.attr="disabled" wire:target="saveAutomation" @disabled(!$canManageAutomation) class="rounded-lg bg-gray-900 px-5 py-2.5 text-xs font-bold text-white hover:bg-black disabled:opacity-50"><span wire:loading.remove wire:target="saveAutomation">LƯU / CẬP NHẬT LỊCH</span><span wire:loading wire:target="saveAutomation">ĐANG LƯU...</span></button>
