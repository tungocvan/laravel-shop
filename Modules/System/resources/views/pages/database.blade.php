...
@section('content')
    <div class="space-y-6" x-data="{ loadingModule: false, showFullDatabase: false }">
        ...
        <section class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
            <div class="border-b border-gray-100 bg-gray-50 px-5 py-4 sm:px-6"><div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><p class="text-xs font-bold uppercase tracking-wider text-indigo-600">Bước 1 · Phạm vi thao tác</p><h2 class="mt-1 text-lg font-bold text-gray-900">Chọn Module cần Backup / Restore</h2><p class="mt-1 text-sm text-gray-500">Hệ thống chỉ tải snapshot, Google Drive và danh sách bảng sau khi bạn chọn Module.</p></div><form method="GET" action="{{ route('admin.system.database.index') }}" class="w-full lg:w-80" x-ref="moduleForm"><label for="database-module" class="sr-only">Chọn Module</label><select id="database-module" name="module" @change="loadingModule = true; $refs.moduleForm.submit()" class="min-h-11 w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-900 shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"><option value="">— Chọn Module —</option>@foreach ($modules as $module)<option value="{{ $module }}" @selected($selectedModule === $module)>{{ $module }}</option>@endforeach</select></form></div></div>
            @if ($selectedModule === '')
