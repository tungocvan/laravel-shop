...
@section('content')
    <div class="space-y-6" x-data="{ loadingModule: false, showFullDatabase: false }">
            ...
            <div class="border-b border-gray-100 bg-gray-50 px-5 py-4 sm:px-6">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <form method="GET" action="{{ route('admin.system.database.index') }}" class="w-full lg:w-80" x-ref="moduleForm">
                        ...
                        <select id="database-module" name="module" @change="loadingModule = true; $refs.moduleForm.submit()" class="min-h-11 w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-900 shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100">
                            <option value="">— Chọn Module —</option>
                            @foreach ($modules as $module)<option value="{{ $module }}" @selected($selectedModule === $module)>{{ $module }}</option>@endforeach
                        </select>
                    </form>
                </div>
            </div>
        ...
        <section class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
            <button type="button" @click="showFullDatabase = !showFullDatabase" class="flex w-full items-center justify-between gap-4 px-5 py-4 text-left sm:px-6">
                <div><p class="text-xs font-bold uppercase tracking-wider text-gray-500">Phạm vi nâng cao</p><h2 class="mt-1 text-base font-bold text-gray-900">Toàn bộ Database · Backup / Restore · Google Drive</h2><p class="mt-1 text-sm text-gray-500">Giữ đầy đủ chức năng backup SQL và đồng bộ Google Drive hai chiều của giao diện cũ.</p></div>
                <span class="shrink-0 rounded-xl border border-gray-300 bg-gray-50 px-3 py-2 text-sm font-semibold text-gray-700" x-text="showFullDatabase ? 'Thu gọn' : 'Mở quản lý'"></span>
            </button>
            <div x-cloak x-show="showFullDatabase" class="border-t border-gray-200 bg-gray-50/40 p-4 sm:p-6">
