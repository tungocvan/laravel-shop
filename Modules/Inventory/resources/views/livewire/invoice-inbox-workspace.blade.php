                            ...
                            <article class="rounded-2xl border border-slate-200 p-5">
                                ...
                                @if($packageUom && $packageQuantity)
                                    <div class="mt-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
                                        ...
                                        1 {{ $packageUom }} = {{ rtrim(rtrim(number_format((float) $packageQuantity, 8, '.', ''), '0'), '.') }} {{ $packageBaseUom }}
                                    </div>
                                @endif
                    ...
                    <div class="rounded-2xl border {{ $isConfirmed ? 'border-emerald-200 bg-emerald-50' : 'border-indigo-200 bg-indigo-50' }} p-5">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            <div>
                                ...
                                @unless($isConfirmed)<p class="mt-2 text-sm font-medium text-amber-700">Phiếu nháp chưa làm thay đổi tồn kho. Hãy xem lại toàn bộ phiếu trước khi xác nhận.</p>@endunless
                            </div>
                            ...
                            @foreach($selected->receipt->lines as $line)
                                ...
                                <div class="p-4 sm:p-5">
                                    ...
                                    <div class="mt-3 grid gap-2 text-sm text-slate-600 sm:grid-cols-3">
                                        <div><span class="font-semibold text-slate-800">Lô:</span> {{ $line->lot_number ?: '—' }}</div>
                                        <div><span class="font-semibold text-slate-800">HSD:</span> {{ $line->expiry_date?->format('d/m/Y') ?: '—' }}</div>
                                        <div><span class="font-semibold text-slate-800">NSX:</span> {{ $line->manufacture_date?->format('d/m/Y') ?: '—' }}</div>
                                    </div>
                                    @if($reviewPackageUom && $reviewPackageQuantity)
                                        <div class="mt-3 rounded-xl bg-indigo-50 px-3 py-2 text-xs text-indigo-800"><span class="font-semibold">Quy cách đóng gói:</span> 1 {{ $reviewPackageUom }} = {{ rtrim(rtrim(number_format((float) $reviewPackageQuantity, 8, '.', ''), '0'), '.') }} {{ $reviewPackageBaseUom }}</div>
                                    @endif
                                </div>
                            @endforeach
                        </div>
