<div class="space-y-5">
    ...
    @else
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                            ...
                            <article class="rounded-2xl border {{ $line->classification === 'NON_STOCK' || $line->inventory_item_id ? 'border-emerald-200 bg-emerald-50/30' : 'border-amber-200 bg-amber-50/30' }} p-5">
                                ...
                                @if($canManageReceipt)
                                    <div class="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center">
                                        ...
                                        @if($canManageItem && !$line->inventory_item_id)<button type="button" wire:click="beginCreateItem({{ $line->id }})" class="min-h-11 rounded-xl border border-indigo-200 bg-indigo-50 px-4 text-sm font-semibold text-indigo-700">+ Tạo mặt hàng mới</button>@endif
            ...
            @elseif(!$receipt)
                <div class="p-5 sm:p-6">
                    <div class="mb-5"><h3 class="text-lg font-bold text-slate-900">Thông tin nhập kho</h3><p class="mt-1 text-sm text-slate-500">Số lô và hạn sử dụng được điền sẵn khi dữ liệu hóa đơn có cung cấp. Bạn có thể kiểm tra hoặc sửa trước khi lập phiếu.</p></div>
                            ...
                            <article class="rounded-2xl border border-slate-200 p-5">
                                ...
                                <div class="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                                    ...
                                    <div class="mt-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
                                        ...
                                        1 {{ $packageUom }}
                                        =
                                        {{ rtrim(rtrim(number_format((float) $packageQuantity, 8, '.', ''), '0'), '.') }}
                                        {{ $packageBaseUom }}
                                    </div>
                                @endif
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-3 sm:p-6">
            <div class="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
                ...
                <div class="overflow-y-auto p-5 sm:p-6">
                    <section class="rounded-2xl border border-slate-200 p-4 sm:p-5">
                                    ...
                                    <label class="text-sm font-medium text-slate-700">
                                        ...
                                        <div class="mt-1 flex min-h-11 items-center rounded-xl border border-gray-300 bg-white px-3">
                                            ...
                                            <input
                                                ...
                                                min="0"
                                                step="any"
                                                wire:model.live.debounce.300ms="itemForm.conversion_factor"
                                                placeholder="VD: 30"
                                                class="mx-2 min-w-0 flex-1 border-0 p-0 text-center focus:ring-0"
                                            >
                                            <span class="shrink-0 text-slate-500">{{ $modalBaseUom }}</span>
                                        </div>
                                        @error('itemForm.conversion_factor')
                        ...
                        <div class="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <div class="mb-4">
                                <h4 class="font-bold text-slate-900">Thông tin quy cách sản phẩm</h4>
                                <p class="mt-1 text-xs text-slate-600">
                                    Chỉ lưu thông tin đóng gói của sản phẩm, không làm thay đổi số lượng nhập kho.
                                </p>
                            </div>
                            ...
                            <div class="grid gap-4 md:grid-cols-3">
                                <label class="text-sm font-medium text-slate-700">
                                    ...
                                    <input
                                        type="text"
                                        wire:model.live.debounce.300ms="itemForm.package_uom"
                                        placeholder="VD: Hộp"
                                        class="mt-1 min-h-11 w-full rounded-xl border border-gray-300 bg-white px-4 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-100"
                                    >
                                    @error('itemForm.package_uom')
                                        <span class="mt-1 block text-xs text-red-600">{{ $message }}</span>
                                    @enderror
                                </label>

                                <label class="text-sm font-medium text-slate-700">
                                    Số lượng trong 1 đơn vị
                                    <input
                                        type="number"
                                        min="0"
                                        step="any"
                                        wire:model.live.debounce.300ms="itemForm.package_quantity"
                                        placeholder="VD: 60"
                                        class="mt-1 min-h-11 w-full rounded-xl border border-gray-300 bg-white px-4 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-100"
                                    >
                                    @error('itemForm.package_quantity')
                                        <span class="mt-1 block text-xs text-red-600">{{ $message }}</span>
                                    @enderror
                                </label>

                                <div class="rounded-xl border border-indigo-200 bg-indigo-50 p-3">
                                    <div class="text-xs font-semibold uppercase tracking-wide text-indigo-700">Quy cách đóng gói</div>
                                    <div class="mt-1 font-bold text-slate-900">
                                        @if(filled(data_get($itemForm, 'package_uom')) && filled(data_get($itemForm, 'package_quantity')))
                                            1 {{ data_get($itemForm, 'package_uom') }}
                                            =
                                            {{ rtrim(rtrim(number_format((float) data_get($itemForm, 'package_quantity'), 8, '.', ''), '0'), '.') }}
                                            {{ data_get($itemForm, 'base_uom') ?: 'đơn vị cơ sở' }}
                                        @else
                                            Chưa khai báo
                                        @endif
                                    </div>
                    ...
                    <section class="mt-4 rounded-2xl border border-indigo-200 bg-indigo-50/40 p-4 sm:p-5">
                        <div class="mb-4"><h4 class="font-bold text-slate-900">Thông tin lô của lần nhập này</h4><p class="mt-1 text-xs text-slate-600">Hệ thống điền sẵn từ hóa đơn nếu có. Có thể sửa hoặc nhập tay trước khi tạo phiếu nhập nháp.</p></div>
                        <div class="grid gap-4 md:grid-cols-2">
                            <label class="text-sm font-medium text-slate-700">Số lô<input type="text" wire:model="itemForm.lot_number" placeholder="Nhập số lô nếu có" class="mt-1 min-h-11 w-full rounded-xl border border-gray-300 bg-white px-4 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-100">@error('itemForm.lot_number')<span class="mt-1 block text-xs text-red-600">{{ $message }}</span>@enderror</label>
                            <label class="text-sm font-medium text-slate-700">Hạn sử dụng<input type="date" wire:model="itemForm.expiry_date" class="mt-1 min-h-11 w-full rounded-xl border border-gray-300 bg-white px-4 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-100">@error('itemForm.expiry_date')<span class="mt-1 block text-xs text-red-600">{{ $message }}</span>@enderror</label>
                            <div class="md:col-span-2 rounded-xl border border-slate-200 bg-white p-3"><label class="flex cursor-pointer items-center gap-2 text-sm font-medium text-slate-700"><input type="checkbox" wire:model.live="itemForm.include_manufacture_date" class="rounded border-gray-300"> Nhập ngày sản xuất</label>@if(data_get($itemForm, 'include_manufacture_date'))<div class="mt-3 max-w-sm"><input type="date" wire:model="itemForm.manufacture_date" class="min-h-11 w-full rounded-xl border border-gray-300 bg-white px-4 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-100">@error('itemForm.manufacture_date')<span class="mt-1 block text-xs text-red-600">{{ $message }}</span>@enderror</div>@endif<p class="mt-2 text-xs text-slate-500">Ngày sản xuất là tùy chọn. Không dùng thông tin “NSX: Việt Nam” làm ngày sản xuất.</p></div>
                        </div>
                    </section>
                ...
                <div class="flex flex-col-reverse gap-2 border-t border-slate-200 bg-white p-4 sm:flex-row sm:justify-end">
                    <button type="button" wire:click="closeCreateItem" class="min-h-11 rounded-xl border border-slate-300 bg-white px-5 text-sm font-semibold text-slate-700">Hủy</button>
                    <button type="button" wire:click="saveStandaloneItem" wire:loading.attr="disabled" wire:target="saveStandaloneItem" class="min-h-11 rounded-xl bg-indigo-600 px-5 text-sm font-semibold text-white disabled:opacity-50">{{ $editingItemId ? 'Lưu thay đổi' : 'Tạo mặt hàng và tiếp tục' }}</button>
                </div>
            </div>
        </div>
    @endif
