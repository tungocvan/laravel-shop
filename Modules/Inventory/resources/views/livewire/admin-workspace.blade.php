<div class="space-y-6">
    <header class="flex flex-col gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div><a href="{{ route('admin.inventory.dashboard') }}" class="text-xs font-semibold text-indigo-700 hover:text-indigo-900">← Inventory Dashboard</a><h1 class="mt-2 text-2xl font-bold text-slate-950">{{ $title }}</h1><p class="mt-1 text-sm text-slate-500">Workspace vận hành với tìm kiếm, bộ lọc và phân trang có giới hạn.</p></div>
        @if($canManage)<button type="button" wire:click="create" class="inline-flex min-h-11 items-center justify-center rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700">Tạo mới</button>@endif
    </header>

    @if($errorMessage)<div role="alert" class="rounded-xl border border-red-300 bg-red-50 px-4 py-3 text-sm text-red-800">{{ $errorMessage }}</div>@endif

    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            <div class="xl:col-span-2"><label class="text-xs font-semibold text-slate-700">Tìm kiếm</label><input wire:model.live.debounce.300ms="search" type="search" placeholder="Mã, tên, số chứng từ..." class="mt-1 w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-100"></div>
            @if(in_array($workspace, ['receipts','issues','transfers','stocktakes']))<div><label class="text-xs font-semibold text-slate-700">Trạng thái</label><select wire:model.live="status" class="mt-1 w-full rounded-xl border border-gray-300 bg-white px-3 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"><option value="all">Tất cả</option><option>DRAFT</option><option>COUNTED</option><option>CONFIRMED</option><option>CANCELLED</option></select></div>@elseif(in_array($workspace, ['warehouses','items']))<div><label class="text-xs font-semibold text-slate-700">Trạng thái</label><select wire:model.live="status" class="mt-1 w-full rounded-xl border border-gray-300 bg-white px-3 py-2.5 text-sm"><option value="all">Tất cả</option><option value="active">Đang dùng</option><option value="inactive">Ngừng dùng</option></select></div>@elseif($workspace==='stock')<div><label class="text-xs font-semibold text-slate-700">Tồn kho</label><select wire:model.live="status" class="mt-1 w-full rounded-xl border border-gray-300 bg-white px-3 py-2.5 text-sm"><option value="all">Tất cả</option><option value="low">Tồn thấp</option><option value="positive">Còn hàng</option><option value="zero">Hết hàng</option></select></div>@elseif($workspace==='lots')<div><label class="text-xs font-semibold text-slate-700">HSD</label><select wire:model.live="status" class="mt-1 w-full rounded-xl border border-gray-300 bg-white px-3 py-2.5 text-sm"><option value="all">Tất cả</option><option value="expired">Đã hết hạn</option><option value="expiring">90 ngày tới</option><option value="valid">Còn hạn</option></select></div>@elseif($workspace==='movements')<div><label class="text-xs font-semibold text-slate-700">Loại</label><select wire:model.live="status" class="mt-1 w-full rounded-xl border border-gray-300 bg-white px-3 py-2.5 text-sm"><option value="all">Tất cả</option><option>RECEIPT</option><option>ISSUE</option><option>TRANSFER</option><option>STOCKTAKE</option><option>REVERSAL</option></select></div>@endif
            @if(in_array($workspace, ['receipts','issues','transfers','stocktakes','stock','movements']))<div><label class="text-xs font-semibold text-slate-700">Kho</label><select wire:model.live="warehouseFilter" class="mt-1 w-full rounded-xl border border-gray-300 bg-white px-3 py-2.5 text-sm"><option value="all">Tất cả kho</option>@foreach($warehouses as $warehouse)<option value="{{ $warehouse->id }}">{{ $warehouse->code }} — {{ $warehouse->name }}</option>@endforeach</select></div>@endif
            <div><label class="text-xs font-semibold text-slate-700">Số dòng</label><select wire:model.live="perPage" class="mt-1 w-full rounded-xl border border-gray-300 bg-white px-3 py-2.5 text-sm"><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select></div>
        </div>
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        @forelse($rows as $row)<tr class="align-top hover:bg-slate-50/70">
            ...
            @elseif($workspace==='lots')<td class="px-4 py-3"><span class="font-semibold">{{ $row->sku }}</span><br><span class="text-xs text-slate-500">{{ $row->display_name }}</span></td><td class="px-4 py-3 font-medium">{{ $row->lot_number }}</td><td class="px-4 py-3">{{ $row->expiry_date ? \Illuminate\Support\Carbon::parse($row->expiry_date)->format('d/m/Y') : '—' }}</td><td class="px-4 py-3">{{ $row->status }}</td>
            @else<td class="px-4 py-3 whitespace-nowrap">{{ \Illuminate\Support\Carbon::parse($row->occurred_at)->format('d/m/Y H:i') }}</td><td class="px-4 py-3">{{ $row->warehouse_code }}</td><td class="px-4 py-3"><span class="font-semibold">{{ $row->sku }}</span><br><span class="text-xs text-slate-500">{{ $row->display_name }}</span></td><td class="px-4 py-3">{{ $row->movement_type }}</td><td class="px-4 py-3 text-right font-semibold">{{ $row->quantity_delta }} {{ $row->base_uom }}</td><td class="px-4 py-3 text-xs">{{ $row->document_type }} #{{ $row->document_id }}</td>@endif
            @if($canManage || $canConfirm)<td class="px-4 py-3 text-right whitespace-nowrap">@if($canManage && (!isset($row->status) || $row->status==='DRAFT'))<button type="button" wire:click="edit({{ $row->id }})" class="text-xs font-semibold text-indigo-700 hover:text-indigo-900">Sửa</button>@endif @if($canConfirm && isset($row->status) && in_array($row->status,['DRAFT','COUNTED']))<button type="button" wire:click="askConfirm({{ $row->id }})" class="ml-3 text-xs font-semibold text-emerald-700 hover:text-emerald-900">Xác nhận</button>@endif</td>@endif
        </tr>@empty<tr><td colspan="8" class="px-4 py-12 text-center text-slate-500">Không có dữ liệu phù hợp.</td></tr>@endforelse
        </tbody></table></div>
        <div class="border-t border-slate-200 px-4 py-4">{{ $rows->links('Inventory::vendor.pagination.admin-inventory') }}</div>
    </section>

    @if($formOpen)
    <div class="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/45 p-4" role="dialog" aria-modal="true"><div class="max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
        <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4"><h2 class="text-lg font-bold text-slate-950">{{ $editingId ? 'Cập nhật' : 'Tạo mới' }} — {{ $title }}</h2><button type="button" wire:click="$set('formOpen', false)" class="rounded-lg px-3 py-2 text-slate-500 hover:bg-slate-100">✕</button></div>
        <form wire:submit="save" class="space-y-5 p-5">
            ...
            <div class="flex justify-end gap-3 border-t border-slate-200 pt-4"><button type="button" wire:click="$set('formOpen', false)" class="min-h-11 rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold">Hủy</button><button type="submit" wire:loading.attr="disabled" wire:target="save" class="min-h-11 rounded-xl bg-indigo-600 px-5 py-2 text-sm font-semibold text-white disabled:opacity-60">Lưu nháp</button></div>
        </form>
    </div></div>
    @endif

    @if($confirmingId)
    <div class="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/45 p-4" role="dialog" aria-modal="true"><div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl"><h2 class="text-lg font-bold text-slate-950">Xác nhận ghi sổ tồn kho?</h2><p class="mt-2 text-sm leading-6 text-slate-600">Thao tác này sẽ gọi posting service, tạo stock movements và cập nhật balance. Chứng từ sau khi xác nhận là bất biến.</p><div class="mt-5 flex justify-end gap-3"><button type="button" wire:click="$set('confirmingId', null)" class="min-h-11 rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold">Hủy</button><button type="button" wire:click="confirmDocument" wire:loading.attr="disabled" wire:target="confirmDocument" class="min-h-11 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">Xác nhận nhập sổ</button></div></div></div>
    @endif
</div>
