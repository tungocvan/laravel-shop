    ...
    <div x-show="tab === 'builder'" x-cloak class="space-y-8 rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
                        ...
                        @forelse($builderSlots[$slotKey] ?? [] as $index => $item)
                            ...
                            <div class="rounded-lg border bg-white p-3 shadow-sm {{ $enabled ? 'border-gray-200' : 'border-dashed border-gray-300 opacity-60' }}" wire:key="builder-item-{{ str_replace('.', '-', $slotKey) }}-{{ $index }}-{{ $item['type'] ?? 'unknown' }}">
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    <button type="button" wire:click="toggleComponent('{{ $slotKey }}', {{ $index }})" class="rounded-md px-2 py-1 text-xs font-semibold {{ $enabled ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600' }}">{{ $enabled ? 'Đang bật' : 'Đang tắt' }}</button>
                                </div>
        ...
        <div class="border-t border-gray-100 pt-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                ...
                <label class="flex items-center gap-3 rounded-lg border border-gray-200 px-4 py-3 text-sm font-medium text-gray-700"><input type="checkbox" wire:model="presentation.sticky" class="rounded border-gray-300 text-blue-600"> Sticky Header</label>
                <label class="text-sm font-medium text-gray-700">Shadow
                    <select wire:model="presentation.shadow" class="mt-1 w-full rounded-lg border-gray-300 text-sm"><option value="none">None</option><option value="soft">Soft</option><option value="medium">Medium</option></select>
                </label>
            </div>

            <div class="mt-5 rounded-xl border border-gray-200 p-4">
                <label class="flex items-center gap-3 text-sm font-semibold text-gray-700"><input type="checkbox" wire:model="presentation.inherit_colors" class="rounded border-gray-300 text-blue-600"> Kế thừa màu Global Website Theme</label>
                <div class="mt-4 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
                    @foreach(['background' => 'Background','foreground' => 'Text','accent' => 'Accent','border' => 'Border','topbar_background' => 'Topbar BG','topbar_foreground' => 'Topbar Text'] as $colorKey => $colorLabel)
                        <label class="text-xs font-medium text-gray-600">{{ $colorLabel }}<input type="color" wire:model="presentation.colors.{{ $colorKey }}" class="mt-1 h-10 w-full rounded border border-gray-200 bg-white p-1"></label>
                    @endforeach
                </div>
            </div>
                ...
                <div class="mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 rounded-xl border border-amber-200 bg-amber-50/40 p-4">
                    @foreach(['container_width' => ['Container max width', 960, 1920],'desktop_height' => ['Desktop height', 52, 120],'tablet_height' => ['Tablet height', 52, 120],'mobile_height' => ['Mobile height', 52, 120],'topbar_height' => ['Topbar height', 24, 56],'logo_max_height' => ['Logo max height', 24, 72],'search_max_width' => ['Search max width', 320, 900]] as $field => [$label, $min, $max])
                        <label class="text-sm font-medium text-gray-700">{{ $label }}
                            <div class="relative mt-1"><input type="number" min="{{ $min }}" max="{{ $max }}" wire:model="presentation.custom.{{ $field }}" class="w-full rounded-lg border-gray-300 pr-10 text-sm"><span class="absolute right-3 top-2.5 text-xs text-gray-400">px</span></div>
                        </label>
                    @endforeach
                </div>
            @endif
