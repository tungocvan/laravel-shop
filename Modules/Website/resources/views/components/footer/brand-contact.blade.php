<div class="lg:col-span-4 space-y-6">
    <a href="/" class="flex items-center gap-2 group">
        @if(!empty($footerSettings['logo']))
            <img src="{{ str_starts_with($footerSettings['logo'], 'http') ? $footerSettings['logo'] : asset('storage/'.$footerSettings['logo']) }}" alt="{{ $footerSettings['brand_name'] }}" class="w-auto max-w-40 object-contain" style="max-height: var(--footer-logo-max);">
        @else
            <div class="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-gray-900 font-black text-2xl transition-colors" style="color: var(--footer-background);">{{ mb_substr($footerSettings['brand_name'] ?? 'F', 0, 1) }}</div>
        @endif
