@if(($mode ?? 'desktop') === 'mobile')
    <div x-show="mobileSearchOpen" x-transition class="lg:hidden pb-4" style="display:none">
        <form action="{{ Route::has('product.list') ? route('product.list') : '#' }}" method="GET">
            <input type="search" name="search" value="{{ request('search') }}" placeholder="Tìm kiếm sản phẩm..." class="w-full bg-gray-100 border-none rounded-xl py-3 px-4 focus:ring-2 focus:ring-blue-500">
        </form>
    </div>
@else
    <div class="hidden lg:flex flex-1 max-w-xl relative">
        <form action="{{ Route::has('product.list') ? route('product.list') : '#' }}" method="GET" class="w-full relative">
            ...
            <button type="submit" aria-label="Tìm kiếm" class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 hover:text-blue-600">
