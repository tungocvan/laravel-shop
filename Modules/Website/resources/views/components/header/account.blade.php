@php
    ...
    $guestActions = $accountActions['guest'] ?? [];
    $authenticatedActions = $accountActions['authenticated'] ?? [];
@endphp
<div class="hidden lg:block relative">
    @auth
        <button @click="userDropdownOpen=!userDropdownOpen" class="flex items-center gap-2 hover:bg-gray-50 p-1.5 rounded-full border border-transparent hover:border-gray-200">
            @if($authenticatedActions['show_avatar'] ?? true)
            ...
            @endif
            @if($authenticatedActions['show_name'] ?? true)
        ...
        <div x-show="userDropdownOpen" @click.outside="userDropdownOpen=false" x-transition class="absolute right-0 mt-3 w-64 bg-white rounded-xl shadow-2xl py-2 ring-1 ring-black/5 z-50 overflow-hidden" style="display:none">
            ...
            @endif
            @if($authenticatedActions['logout_enabled'] ?? true)
                <div class="border-t py-1">
                    <form method="POST" action="{{ route('logout') }}">@csrf
                        <button type="submit" class="block w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50">{{ $authenticatedActions['logout_label'] ?? 'Đăng xuất' }}</button>
                    </form>
                </div>
            @endif
        </div>
