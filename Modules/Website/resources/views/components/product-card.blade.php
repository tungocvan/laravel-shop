...
<div class="group relative flex flex-col h-full">
    ...
    <div class="flex-1 flex flex-col">
        ...
        <div class="flex items-center gap-1 mb-2" aria-label="{{ $reviewCount > 0 ? $rating . ' trên 5 sao, ' . $reviewCount . ' đánh giá' : 'Chưa có đánh giá' }}">
            <div class="flex text-yellow-400 text-xs">
                @for($i=1; $i<=5; $i++)
                    <svg class="w-3 h-3 {{ $i <= round($rating) ? 'text-yellow-400' : 'text-gray-200' }}" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                @endfor
            </div>
            <span class="text-[10px] text-gray-400">{{ $reviewCount > 0 ? '(' . $rating . ' · ' . $reviewCount . ')' : '(Chưa có)' }}</span>
        </div>
