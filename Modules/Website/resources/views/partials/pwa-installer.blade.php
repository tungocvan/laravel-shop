    ...
    <button type="button" data-pwa-install-button
        ...
        <div class="flex items-center gap-3">
            ...
            <span class="min-w-0 flex-1">
                <strong data-pwa-install-title class="block text-sm font-bold text-white">Cài ứng dụng FlexBiz</strong>
                <span data-pwa-install-subtitle class="mt-1 block text-xs leading-5 text-gray-400">Truy cập nhanh từ màn hình chính · Không cần App Store</span>
            </span>
...
<div data-pwa-install-modal class="fixed inset-0 z-[100] hidden items-end justify-center bg-slate-950/70 p-0 backdrop-blur-sm sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="pwa-install-heading">
    <div data-pwa-install-panel class="w-full max-w-md rounded-t-[2rem] bg-white p-5 text-slate-900 shadow-2xl sm:rounded-[2rem] sm:p-6">
        ...
        <div class="flex items-start justify-between gap-4">
            <div><p class="text-xs font-bold uppercase tracking-[0.18em] text-blue-600">Progressive Web App</p><h2 id="pwa-install-heading" data-pwa-modal-title class="mt-1 text-2xl font-black tracking-tight">Cài FlexBiz</h2></div>
        ...
        <div data-pwa-ios-guide class="mt-5 hidden">
            ...
            <div class="mt-5 space-y-3">
                ...
                <div class="flex gap-3 rounded-2xl bg-slate-50 p-4"><span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-950 text-sm font-bold text-white">3</span><div><strong class="text-sm">Nhấn “Thêm”</strong><p class="mt-1 text-xs leading-5 text-slate-500">FlexBiz sẽ xuất hiện như một ứng dụng trên màn hình chính.</p></div></div>
            </div>
        </div>
...
(() => {
    ...
    if (isIOS) {
        title.textContent = 'Cài FlexBiz trên iPhone/iPad';
        subtitle.textContent = isSafari ? '3 bước nhanh trong Safari' : 'Mở bằng Safari để cài PWA';
    } else {
        title.textContent = 'Cài ứng dụng FlexBiz';
        subtitle.textContent = 'Truy cập nhanh từ màn hình chính · Không cần App Store';
    }
