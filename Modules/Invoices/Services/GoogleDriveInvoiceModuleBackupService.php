...
final class GoogleDriveInvoiceModuleBackupService
{
    ...
    public function files(): array
    {
        ...
        return collect($response->json('files') ?: [])->filter(fn (array $file): bool => $this->validArchiveName((string) ($file['name'] ?? '')))->map(fn (array $file): array => [
            ...
            'checksum' => (string) ($file['appProperties']['sha256'] ?? ''),
    ...
    public function downloadSnapshot(string $fileId, string $fileName): array
    {
        ...
        if (! $metadata->successful() || (bool) $metadata->json('trashed') || (string) $metadata->json('name') !== $fileName || ! is_array($parents) || ! in_array($folderId, $parents, true)) {
        ...
        return $this->extractArchive($download->body(), $fileName);
    }
    ...
    private function buildArchive(string $directory): array
    {
        ...
        return ['path' => $temp, 'checksum' => hash_file('sha256', $temp)];
    }
    ...
    {
        return (bool) preg_match('/\AInvoices-Module-[A-Za-z0-9_-]+\.zip\z/', basename($name));
    }
