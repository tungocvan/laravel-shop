...
final class GoogleDriveInvoiceModuleBackupService
{
    ...
    public function uploadSnapshot(string $directory): array
    {
        ...
        try {
            ...
            if (! $create->successful() || $fileId === '') throw new RuntimeException('Không thể tạo module backup trên Google Drive. HTTP '.$create->status().'.');
            $body = fopen($archive['path'], 'rb');
            if ($body === false) throw new RuntimeException('Không thể đọc archive module backup.');
            try {
    ...
    public function files(): array
    {
        ...
        if ($folderId !== null) {
            ...
            if (! $response->successful()) throw new RuntimeException('Không thể đọc module backup trên Google Drive. HTTP '.$response->status().'.');
        ...
        if ($files === []) {
            ...
            if (! $fallback->successful()) throw new RuntimeException('Không thể dò module backup Invoices trên Google Drive. HTTP '.$fallback->status().'.');
    ...
    {
        if (! preg_match('/\A[A-Za-z0-9_-]+\z/', $fileId) || ! $this->validArchiveName($fileName)) throw new RuntimeException('Module backup Google Drive không hợp lệ.');
        [$token, $folderId] = $this->driveContext(false);
        if ($folderId === null) throw new RuntimeException('Không tìm thấy thư mục Module-Backups trên Google Drive.');
        ...
        if (! $metadata->successful() || (bool) $metadata->json('trashed') || $this->canonicalArchiveName($remoteName) !== $this->canonicalArchiveName($fileName) || ! is_array($parents) || ! in_array($folderId, $parents, true)) throw new RuntimeException('Module backup không còn thuộc thư mục Google Drive được bảo vệ.');
        $size = (int) ($metadata->json('size') ?? 0);
        if ($size <= 0 || $size > self::MAX_DOWNLOAD_BYTES) throw new RuntimeException('Kích thước module backup Google Drive không hợp lệ.');
        $download = Http::withToken($token)->timeout(300)->get('https://www.googleapis.com/drive/v3/files/'.rawurlencode($fileId), ['alt' => 'media']);
        if (! $download->successful() || $download->body() === '' || strlen($download->body()) > self::MAX_DOWNLOAD_BYTES) throw new RuntimeException('Không thể tải module backup từ Google Drive.');
        $expected = (string) ($metadata->json('appProperties.sha256') ?? '');
        if ($expected === '' || ! hash_equals($expected, hash('sha256', $download->body()))) throw new RuntimeException('Checksum module backup trên Google Drive không hợp lệ.');
    ...
    {
        if (! class_exists(ZipArchive::class)) throw new RuntimeException('PHP ZipArchive chưa sẵn sàng để đóng gói module backup.');
        $relative = trim($directory, '/');
        if (! preg_match('#^invoices/module-backups/[A-Za-z0-9_-]+$#', $relative)) throw new RuntimeException('Đường dẫn module snapshot không hợp lệ.');
        $disk = Storage::disk('local');
        foreach (['manifest.json', 'database/invoices.json', 'database/invoice_files.json'] as $required) if (! $disk->exists($relative.'/'.$required)) throw new RuntimeException('Snapshot thiếu file bắt buộc: '.$required);
        ...
        $zip = new ZipArchive;
        if ($zip->open($temp, ZipArchive::CREATE | ZipArchive::OVERWRITE) !== true) throw new RuntimeException('Không thể tạo ZIP module backup.');
        foreach (['manifest.json', 'database/invoices.json', 'database/invoice_files.json'] as $file) $zip->addFromString($file, $disk->get($relative.'/'.$file));
        $zip->close();
    ...
    {
        if (! class_exists(ZipArchive::class)) throw new RuntimeException('PHP ZipArchive chưa sẵn sàng để đọc module backup.');
        $temp = storage_path('app/invoices/module-backups/.download-'.bin2hex(random_bytes(6)).'.zip');
        if (! is_dir(dirname($temp))) mkdir(dirname($temp), 0775, true);
        if (file_put_contents($temp, $body, LOCK_EX) === false) throw new RuntimeException('Không thể ghi module backup tạm thời.');
        $zip = new ZipArchive;
        if ($zip->open($temp) !== true) { @unlink($temp); throw new RuntimeException('ZIP module backup không đọc được.'); }
        $allowed = ['manifest.json', 'database/invoices.json', 'database/invoice_files.json'];
        for ($i = 0; $i < $zip->numFiles; $i++) if (! in_array($zip->getNameIndex($i), $allowed, true)) { $zip->close(); @unlink($temp); throw new RuntimeException('ZIP module backup chứa file ngoài contract.'); }
        $manifest = json_decode((string) $zip->getFromName('manifest.json'), true);
        if (! is_array($manifest) || ($manifest['module'] ?? null) !== 'Invoices') { $zip->close(); @unlink($temp); throw new RuntimeException('Manifest module backup không hợp lệ.'); }
        $directory = 'invoices/module-backups/'.preg_replace('/\.zip$/i', '', str_replace('Invoices-Module-', '', $this->canonicalArchiveName($fileName)));
        $disk = Storage::disk('local');
        if ($disk->exists($directory.'/manifest.json')) { $zip->close(); @unlink($temp); throw new RuntimeException('Snapshot đã tồn tại ở local; không tải đè.'); }
        foreach ($allowed as $file) {
            $contents = $zip->getFromName($file);
            if ($contents === false) { $zip->close(); @unlink($temp); throw new RuntimeException('ZIP module backup thiếu file bắt buộc.'); }
    ...
    {
        $status = $this->drive->testConnection(); $rootId = trim((string) ($status['folder_id'] ?? ''));
        if ($rootId === '') throw new RuntimeException('Không xác định được thư mục Laravel-Backup trên Google Drive.');
        $token = $this->drive->accessToken(); $invoices = $this->folder($token, $rootId, self::INVOICES_FOLDER, $create);
        if ($invoices === null) return [$token, null];
    ...
    private function folder(string $token, string $parentId, string $name, bool $create): ?string
    {
        ...
        if (! $list->successful()) throw new RuntimeException('Không thể tìm thư mục '.$name.' trên Google Drive. HTTP '.$list->status().'.');
        $files = $list->json('files');
        if (is_array($files) && isset($files[0]['id'])) return (string) $files[0]['id'];
        ...
        if (! $response->successful() || $id === '') throw new RuntimeException('Không thể tạo thư mục '.$name.' trên Google Drive. HTTP '.$response->status().'.');
        return $id;
    }
    ...
    private function findFile(string $token, string $parentId, string $name): ?array
    {
        ...
        if (! $response->successful()) throw new RuntimeException('Không thể kiểm tra module backup trên Google Drive. HTTP '.$response->status().'.');
        $files = $response->json('files');
