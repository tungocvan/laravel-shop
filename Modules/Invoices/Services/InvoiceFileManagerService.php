...
class InvoiceFileManagerService
{
    ...
    public function errorDetails(array $filters,int $limit=10):array{$base=$filters;$base['pdf_status']='all';$ids=$this->invoiceService->filteredBuilder($base)->select('id');return InvoiceFile::query()->with('invoice:id,invoice_number,symbol,tax_code,name,invoice_type,issued_date')->whereIn('invoice_id',$ids)->where('status','error')->latest('updated_at')->limit(max(1,min($limit,50)))->get()->map(fn($f)=>['invoice_id'=>$f->invoice_id,'invoice_number'=>$f->invoice?->invoice_number,'partner'=>$f->invoice?->name,'provider'=>$f->provider,'error'=>$f->last_error,'updated_at'=>$f->updated_at?->format('d/m/Y H:i')])->all();}
    public function reconcile(array $filters,int $limit=1000):array{$base=$filters;$base['pdf_status']='all';$invoices=$this->invoiceService->filteredBuilder($base)->orderByDesc('issued_date')->limit(max(1,min($limit,5000)))->get();$available=0;$missing=0;foreach($invoices as $invoice){$file=InvoiceFile::query()->where('invoice_id',$invoice->getKey())->first();if($this->fileService->existsForInvoice($invoice)){$path=$this->fileService->pdfPathForInvoice($invoice);$detected=str_contains(str_replace('\\','/',$path),'/hoadon_temp/')?'legacy':'local';$this->recordAvailable($invoice,$path,$file?->provider?:$detected);$available++;continue;}if($file?->status==='available')$file->update(['status'=>'missing','path'=>null,'size'=>null,'downloaded_at'=>null]);$missing++;}return['scanned'=>$invoices->count(),'available'=>$available,'missing'=>$missing];}
