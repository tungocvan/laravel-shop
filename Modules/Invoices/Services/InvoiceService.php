...
class InvoiceService
{
    ...
    private function filteredQuery(array $filters, bool $includeTaxRate = true): Builder
    {
        ...
        if ($pdfStatus === 'available') {
            $query->whereHas('file', fn ($file) => $file->where('status', 'available'));
        } elseif ($pdfStatus === 'error') {
            $query->whereHas('file', fn ($file) => $file->where('status', 'error'));
        } elseif ($pdfStatus === 'missing') {
            $query->where(function (Builder $query) {
                $query->whereDoesntHave('file')
