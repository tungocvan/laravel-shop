<div class="space-y-6" @if ($syncId && in_array($syncState, ['queued', 'processing'], true)) wire:poll.2s="pollStatus" @endif>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm">
        ...
        <div class="flex flex-wrap items-center gap-3 border-t border-gray-200 px-6 py-4">
            ...
            <button wire:click="run" wire:loading.attr="disabled" class="h-11 rounded-xl bg-emerald-600 px-5 text-sm font-semibold text-white disabled:opacity-50"><span wire:loading.remove wire:target="run">Chạy đồng bộ</span><span wire:loading wire:target="run">Đang xử lý…</span></button>
        </div>
    </div>

    <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div><p class="text-sm font-semibold text-gray-900">Trạng thái đồng bộ</p><p class="mt-1 text-sm text-gray-600">Queue chỉ là trạng thái đã xếp hàng; khối này tự cập nhật đến khi hoàn tất, hoàn tất một phần hoặc thất bại.</p></div>
        ...
        @if($syncMessage)<p class="mt-3 text-sm {{ $syncState==='partial'?'font-medium text-amber-800':'text-gray-700' }}">{{ $syncMessage }}</p>@endif
    ...
    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div class="border-b border-gray-200 p-6">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div><h3 class="text-base font-semibold text-gray-900">Kho file hóa đơn</h3><p class="mt-1 text-sm text-gray-500">File Excel/CSV dùng cho export, backup và import thủ công. Xóa file tại đây không xóa dữ liệu hóa đơn hoặc RAW canonical.</p></div>
        ...
        <div class="grid gap-6 p-6 xl:grid-cols-[1.4fr_1fr]">
            <div>
                <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <p class="text-sm font-semibold text-gray-900">File đã đồng bộ</p>
                        <p class="mt-1 text-xs text-gray-500">vat_in_* = Mua vào · vat_out_* = Bán ra.</p>
                    </div>
                    <div class="flex flex-wrap items-center gap-2">
                        <span class="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-600">{{ count($availableFiles) }} file</span>
                        <span class="rounded-full bg-indigo-50 px-3 py-1 text-xs font-semibold text-indigo-700">Đã chọn {{ count($selectedFiles) }}</span>
                        <button wire:click="selectAllAvailableFiles" @disabled(count($availableFiles)===0) class="h-9 rounded-lg border border-gray-300 px-3 text-xs font-semibold text-gray-700 disabled:opacity-40">Chọn tất cả</button>
                        <button wire:click="clearFileSelection" @disabled(count($selectedFiles)===0) class="h-9 rounded-lg border border-gray-300 px-3 text-xs font-semibold text-gray-600 disabled:opacity-40">Bỏ chọn</button>
                    </div>
                </div>
                ...
                <div class="mt-4 rounded-xl border border-indigo-100 bg-indigo-50/60 p-4">
                    <p class="text-xs font-semibold uppercase tracking-wide text-indigo-700">Quản lý file</p>
                    <p class="mt-1 text-sm text-gray-700">File tạo trực tiếp từ GDT đã được ghi header và RAW canonical vào CSDL trong lúc đồng bộ, vì vậy <strong>không cần bấm Đồng bộ vào CSDL lần nữa</strong>. Chức năng import CSDL chỉ dành cho file upload thủ công, Google Drive hoặc dữ liệu legacy.</p>
                    <div class="mt-3 flex flex-wrap gap-2">
                        <button wire:click="importSelectedFile" wire:confirm="Chỉ tiếp tục nếu đây là file upload thủ công, Google Drive hoặc dữ liệu legacy. File tạo trực tiếp từ GDT đã được ghi vào CSDL và không cần import lần nữa. Bạn có muốn tiếp tục?" wire:loading.attr="disabled" @disabled(count($selectedFiles)!==1) class="h-11 rounded-xl bg-indigo-600 px-5 text-sm font-semibold text-white disabled:opacity-40"><span wire:loading.remove wire:target="importSelectedFile">Import CSDL (1 file)</span><span wire:loading wire:target="importSelectedFile">Đang import…</span></button>
                ...
                <div class="mt-5 rounded-2xl border border-violet-100 bg-violet-50/60 p-5">
                    <div class="flex flex-wrap items-start justify-between gap-3"><div><p class="text-sm font-semibold text-gray-900">Backup File đã đồng bộ qua Email</p><p class="mt-1 text-xs text-gray-600">Gửi bản sao toàn bộ file vat_in_* / vat_out_* dưới dạng ZIP. File gốc trên server không bị xóa.</p></div><span class="rounded-full bg-white px-2.5 py-1 text-[11px] font-semibold text-violet-700">Tự chia email nếu lớn</span></div>
                    <div class="mt-4 flex flex-col gap-3 sm:flex-row"><input type="email" wire:model="backupEmail" placeholder="email-nhan-backup@example.com" class="h-11 min-w-0 flex-1 rounded-xl border border-violet-200 bg-white px-4 text-sm"><button wire:click="backupFilesToEmail" wire:loading.attr="disabled" @disabled(trim($backupEmail)==='' || count($availableFiles)===0) class="h-11 rounded-xl bg-violet-600 px-5 text-sm font-semibold text-white hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-40"><span wire:loading.remove wire:target="backupFilesToEmail">Gửi Backup qua Email</span><span wire:loading wire:target="backupFilesToEmail">Đang gửi…</span></button></div>
            ...
            <div class="space-y-4">
                <div class="rounded-2xl border border-gray-200 bg-gray-50 p-5">
                    <p class="text-sm font-semibold text-gray-900">Upload từ máy tính</p>
                    <p class="mt-1 text-xs text-gray-500">XLSX/CSV tối đa 20 MB. Upload chỉ đưa file vào kho; sau đó chọn đúng 1 file để import thủ công vào CSDL.</p>
                    <div class="mt-3 rounded-xl border border-gray-200 bg-white px-3 py-2 text-xs text-gray-600"><strong>Quy ước:</strong> <code>vat_in_*</code> = Mua vào · <code>vat_out_*</code> = Bán ra. Tên khác sẽ bị từ chối để tránh đồng bộ nhầm loại.</div>
                    ...
                    @error('uploadFile')<p class="mt-2 text-sm font-medium text-red-600">{{ $message }}</p>@enderror
                ...
                <div class="rounded-2xl border border-blue-100 bg-blue-50/50 p-5">
                    ...
                    <p class="mt-1 text-xs text-gray-500">Dán link chia sẻ công khai. Hệ thống lấy tên file gốc và tự nhận dạng loại hóa đơn trước khi lưu vào kho.</p>
                    <div class="mt-3 rounded-xl border border-blue-100 bg-white px-3 py-2 text-xs text-blue-800"><strong>Tên bắt buộc:</strong> <code>vat_in_*</code> hoặc <code>vat_out_*</code>. Nếu sai, hãy đổi tên file trên Google Drive rồi thử lại.</div>
                    ...
                    @error('googleDriveUrl')<p class="mt-2 text-sm font-medium text-red-600">{{ $message }}</p>@enderror
                    <button wire:click="stageGoogleDriveFile" wire:loading.attr="disabled" @disabled(trim($googleDriveUrl)==='') class="mt-4 h-11 rounded-xl bg-blue-600 px-5 text-sm font-semibold text-white disabled:opacity-40"><span wire:loading.remove wire:target="stageGoogleDriveFile">Tải từ Drive vào kho</span><span wire:loading wire:target="stageGoogleDriveFile">Đang kiểm tra & tải…</span></button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-gray-950 p-5 font-mono text-xs text-gray-200 shadow-sm">
        ...
        <div class="max-h-80 space-y-1 overflow-y-auto">@forelse($logs as $line)<div>{{ $line }}</div>@empty<div class="text-gray-400">Chưa có tác vụ.</div>@endforelse</div>
    </div>
</div>
