<div class="space-y-6">
                ...
                <div class="p-6">
                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        <div class="rounded-2xl border border-sky-100 bg-sky-50/70 p-4">
                            <p class="text-xs font-semibold uppercase text-sky-700">Bán ra · đã VAT</p>
                            <p class="mt-2 text-xl font-bold text-sky-900">{{ number_format($partnerDetail['sold_total']) }} ₫</p>
                            <p class="mt-1 text-xs text-sky-700">{{ number_format($partnerDetail['sold_count']) }} hóa đơn bán ra</p>
                        </div>
                        <div class="rounded-2xl border border-amber-100 bg-amber-50/70 p-4">
                            <p class="text-xs font-semibold uppercase text-amber-700">Mua vào · đã VAT</p>
                            <p class="mt-2 text-xl font-bold text-amber-900">{{ number_format($partnerDetail['purchase_total']) }} ₫</p>
                            <p class="mt-1 text-xs text-amber-700">{{ number_format($partnerDetail['purchase_count']) }} hóa đơn mua vào</p>
                        </div>
                        <div class="rounded-2xl border border-indigo-100 bg-indigo-50/70 p-4">
                            <p class="text-xs font-semibold uppercase text-indigo-700">VAT đầu ra</p>
                            <p class="mt-2 text-xl font-bold text-indigo-900">{{ number_format($partnerDetail['sold_vat']) }} ₫</p>
                        </div>
                        <div class="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4">
                            <p class="text-xs font-semibold uppercase text-emerald-700">VAT đầu vào</p>
                            <p class="mt-2 text-xl font-bold text-emerald-900">{{ number_format($partnerDetail['purchase_vat']) }} ₫</p>
                        </div>
                    </div>

                    <div class="mt-5 grid gap-4 sm:grid-cols-2">
                        <div class="rounded-2xl border border-gray-200 bg-gray-50 p-5">
                            <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Chênh lệch tổng tiền</p>
                            <p class="mt-2 text-2xl font-bold {{ $partnerDetail['total_difference'] >= 0 ? 'text-emerald-700' : 'text-red-600' }}">{{ $partnerDetail['total_difference'] >= 0 ? '+' : '' }}{{ number_format($partnerDetail['total_difference']) }} ₫</p>
                            <p class="mt-2 text-xs text-gray-500">Bán ra đã VAT − Mua vào đã VAT. Chỉ là số đối chiếu theo đối tác, không phải lợi nhuận.</p>
                        </div>
                        <div class="rounded-2xl border border-gray-200 bg-gray-50 p-5">
                            <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Chênh lệch VAT</p>
                            <p class="mt-2 text-2xl font-bold {{ $partnerDetail['vat_difference'] >= 0 ? 'text-indigo-700' : 'text-amber-700' }}">{{ $partnerDetail['vat_difference'] >= 0 ? '+' : '' }}{{ number_format($partnerDetail['vat_difference']) }} ₫</p>
                            <p class="mt-2 text-xs text-gray-500">VAT đầu ra − VAT đầu vào của riêng đối tác trong kỳ đang lọc.</p>
                        </div>
                    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div class="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <div class="flex items-end justify-end gap-2"><button type="button" wire:click="resetFilters" class="h-11 rounded-xl border border-gray-300 bg-white px-4 text-sm font-semibold text-gray-700 hover:bg-gray-50">Đặt lại</button>@if(auth('admin')->user()?->can('invoices-export'))<button type="button" wire:click="exportExcel" class="h-11 rounded-xl bg-indigo-600 px-4 text-sm font-semibold text-white hover:bg-indigo-700">Xuất Excel báo cáo</button>@endif</div>
        </div>
    </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div class="overflow-x-auto"><table class="min-w-full divide-y divide-gray-200 text-sm"><thead class="bg-gray-50 text-left text-xs uppercase text-gray-500"><tr><th class="px-4 py-3">Đối tác</th><th class="px-4 py-3">MST</th><th class="px-4 py-3 text-right">Số HĐ</th><th class="px-4 py-3 text-right">Bán ra <span class="block normal-case text-[10px] font-medium text-gray-400">Đã có VAT</span></th><th class="px-4 py-3 text-right">Mua vào <span class="block normal-case text-[10px] font-medium text-gray-400">Đã có VAT</span></th><th class="px-4 py-3 text-right">Thao tác</th></tr></thead><tbody class="divide-y divide-gray-100">
