<div class="space-y-6">
    ...
    <section class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($records as $record)
                        ...
                        <tr wire:key="source-data-desktop-row-{{ $record->id }}" class="align-top transition hover:bg-gray-50/70">
                            <td class="px-4 py-4">
                            ...
                            <td class="px-4 py-4"><select wire:model="businessClassifications.{{ $record->id }}" class="{{ $controlClass }}">@foreach ($classificationOptions as $option)<option value="{{ $option }}">{{ $classificationLabels[$option] }}</option>@endforeach</select><label wire:key="source-data-desktop-supplier-label-{{ $record->id }}" class="mt-3 flex items-start gap-2 text-xs font-medium text-slate-700"><input wire:key="source-data-desktop-supplier-checkbox-{{ $record->id }}" id="source-data-desktop-supplier-checkbox-{{ $record->id }}" type="checkbox" wire:model.live="applySameTaxCode.{{ $record->id }}" class="mt-0.5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"><span>Áp dụng cho toàn bộ nhà cung cấp<span class="mt-0.5 block font-normal text-slate-500">MST {{ $invoice?->tax_code ?: '—' }} · các hóa đơn cùng loại</span></span></label></td>
        ...
        <div class="divide-y divide-gray-200 lg:hidden">
            @forelse ($records as $record)
                @php($invoice = $record->invoice)
                <article wire:key="source-data-mobile-row-{{ $record->id }}" class="space-y-4 p-4 sm:p-5">
                    ...
                    <label wire:key="source-data-mobile-supplier-label-{{ $record->id }}" class="flex items-start gap-2 text-xs font-medium text-slate-700"><input wire:key="source-data-mobile-supplier-checkbox-{{ $record->id }}" id="source-data-mobile-supplier-checkbox-{{ $record->id }}" type="checkbox" wire:model.live="applySameTaxCode.{{ $record->id }}" class="mt-0.5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"><span>Áp dụng cho toàn bộ nhà cung cấp<span class="mt-0.5 block font-normal text-slate-500">MST {{ $invoice?->tax_code ?: '—' }} · các hóa đơn cùng loại</span></span></label>
