...
class ModuleBackupRestore extends Component
{
    ...
    {
        $this->driveConnected = $drive->isConnected();
    }

    public function backupNow(InvoiceModuleSnapshotService $snapshots, GoogleDriveInvoiceModuleBackupService $drive): void
    {
        $this->resetFeedback();
        try {
            ...
            if ($drive->isConnected()) {
                $drive->uploadSnapshot($snapshot['directory']);
                $this->message = 'Đã tạo MANUAL snapshot và bảo vệ trên Google Drive.';
            } else {
                $this->message = 'Đã tạo MANUAL snapshot tại Local. Google Drive chưa kết nối nên backup chưa được bảo vệ ngoài server.';
            }
            $this->checkRestore($snapshots, app(InvoiceRestoreReadinessService::class), app(InvoiceRestoreImpactService::class));
        } catch (Throwable $exception) {
            report($exception);
            $this->error = 'Không thể tạo/bảo vệ backup module Invoices: '.$exception->getMessage();
        }
    }

    public function refreshDriveBackups(GoogleDriveInvoiceModuleBackupService $drive): void
    {
        $this->resetFeedback();
        try {
            $this->driveConnected = $drive->isConnected();
            $this->driveBackups = $this->driveConnected ? $drive->files() : [];
            $this->message = $this->driveConnected ? 'Đã tải danh sách module backup từ Google Drive.' : 'Google Drive chưa kết nối.';
        } catch (Throwable $exception) {
            report($exception);
            $this->error = 'Không thể đọc backup Google Drive: '.$exception->getMessage();
        }
    }

    public function protectSnapshot(string $directory, GoogleDriveInvoiceModuleBackupService $drive): void
    {
        $this->resetFeedback();
        try {
            $drive->uploadSnapshot($directory);
            $this->message = 'Snapshot đã được bảo vệ trên Google Drive.';
            $this->driveBackups = $drive->files();
        } catch (Throwable $exception) {
            report($exception);
            $this->error = 'Không thể đồng bộ snapshot lên Google Drive: '.$exception->getMessage();
        }
    }

    public function downloadDriveBackup(string $fileId, string $fileName, GoogleDriveInvoiceModuleBackupService $drive): void
    {
        $this->resetFeedback();
        try {
            $result = $drive->downloadSnapshot($fileId, $fileName);
            $this->selectedSnapshot = $result['directory'];
            $this->message = 'Đã tải module backup từ Google Drive và xác minh checksum archive.';
            $this->checkRestore(app(InvoiceModuleSnapshotService::class), app(InvoiceRestoreReadinessService::class), app(InvoiceRestoreImpactService::class));
        } catch (Throwable $exception) {
            report($exception);
            $this->error = 'Không thể tải module backup từ Google Drive: '.$exception->getMessage();
        }
    }
    ...
    public function checkRestore(InvoiceModuleSnapshotService $snapshots, InvoiceRestoreReadinessService $readiness, InvoiceRestoreImpactService $impact): void
    {
        $this->resetFeedback();
        ...
        try {
            ...
            $snapshotInvoices = $inspection['checksum_valid'] && $inspection['version_supported'] ? $snapshots->readTable($this->selectedSnapshot, 'invoices') : [];
            $snapshotFiles = $inspection['checksum_valid'] && $inspection['version_supported'] ? $snapshots->readTable($this->selectedSnapshot, 'invoice_files') : [];
            $this->readiness = $readiness->inspect($inspection);
            $this->impact = $this->readiness['status'] !== InvoiceRestoreReadinessService::BLOCKED ? $impact->preview($snapshotInvoices, $snapshotFiles) : null;
        } catch (Throwable $exception) {
            report($exception);
            $this->readiness = ['status' => InvoiceRestoreReadinessService::BLOCKED, 'checks' => [], 'warnings' => [], 'blockers' => [$exception->getMessage()]];
            $this->impact = null;
        }
    }
    ...
    {
        return view('Invoices::livewire.module-backup-restore', ['snapshots' => $this->snapshots()]);
    }

    private function setLastRestore(array $result): void
    {
        $safetyBackup = $result['safety_backup'] ?? null;
        $this->lastRestore = ['mode' => (string) ($result['mode'] ?? 'merge'), 'safety_backup_directory' => is_array($safetyBackup) ? (string) ($safetyBackup['directory'] ?? '') : (is_string($safetyBackup) ? $safetyBackup : ''), 'inserted_invoices' => (int) ($result['result']['insertedInvoices'] ?? 0), 'preserved_invoices' => (int) ($result['result']['preservedInvoices'] ?? 0), 'inserted_files' => (int) ($result['result']['insertedFiles'] ?? 0), 'preserved_files' => (int) ($result['result']['preservedFiles'] ?? 0), 'verification_passed' => (bool) ($result['verification']['passed'] ?? false), 'partner_master_changes' => (int) ($result['partner_master_changes'] ?? 0)];
    }
