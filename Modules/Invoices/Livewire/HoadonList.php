...
class HoadonList extends Component
{
    ...
    public ?string $downloadStatus=null; public ?string $pdfNotice=null; public ?string $pdfError=null; public ?int $pdfProcessingId=null;
    public ?string $type=null; public string $name=''; public string $tax_code=''; public string $from_date=''; public string $to_date=''; public string $taxRateFilter='all'; public string $pdfStatusFilter='all';
    public string $year=''; public string $month=''; public string $sort='date_desc'; public array $yearOptions=[]; public array $nameList=[]; public array $taxCodeList=[]; public array $selected=[]; public int $perPage=10; public array $perPageOptions=[10,25,50,100];
    ...
    public function updatedYear():void{if($this->year===''){$this->month='';$this->from_date='';$this->to_date='';$this->resetListState(true);return;}$this->applyPeriodSelection();}
    public function updatedMonth():void{if($this->month!==''&&$this->year==='')$this->year=(string)now()->year;$this->applyPeriodSelection();}
    public function updatedFromDate():void{$this->clearPeriodPreset();$this->resetListState(true);} public function updatedToDate():void{$this->clearPeriodPreset();$this->resetListState(true);}
    public function updatedPerPage(mixed $value):void{$this->perPage=$this->normalizedPerPage($value);$this->selected=[];$this->resetPage();}

    public function resetFilters():void{$this->type=null;$this->name='';$this->tax_code='';$this->year=(string)now()->year;$this->month='';$this->from_date=Carbon::now()->startOfYear()->format('Y-m-d');$this->to_date=Carbon::now()->format('Y-m-d');$this->taxRateFilter='all';$this->pdfStatusFilter='all';$this->sort='date_desc';$this->selected=[];$this->pdfNotice=null;$this->pdfError=null;$this->refreshOptions();$this->resetPage();$this->dispatch('filters-reset');}
    ...
    public function downloadMissingPdfs():void{$this->authorizePermission('invoices-download');$this->clearPdfMessages();try{$this->fileManager->reconcile($this->filters());$ids=$this->fileManager->missingInvoiceIds($this->filters(),25);if($ids===[]){$this->pdfNotice='Không còn PDF nào thiếu trong bộ lọc hiện tại.';return;}$r=$this->pdfService->downloadSelected($ids);$remaining=$this->fileManager->summary($this->filters())['missing'];$message="Batch PDF: mới {$r['downloaded']} · đã có {$r['existing']} · lỗi {$r['failed']} · còn thiếu {$remaining}.";$this->applyResultMessage($r,$message);}catch(\Throwable $e){$this->pdfError=$e->getMessage();}}
    public function retryPdfErrors():void{$this->authorizePermission('invoices-download');$this->clearPdfMessages();try{$ids=$this->fileManager->errorInvoiceIds($this->filters(),25);if($ids===[]){$this->pdfNotice='Không có PDF lỗi cần thử lại trong bộ lọc hiện tại.';return;}$r=$this->pdfService->downloadSelected($ids,true);$message="Retry PDF: thành công {$r['downloaded']} · lỗi {$r['failed']}.";$this->applyResultMessage($r,$message);}catch(\Throwable $e){$this->pdfError=$e->getMessage();}}
    public function deleteFilteredPdfs():void{$this->authorizePermission('invoices-download');$this->clearPdfMessages();try{$r=$this->fileManager->deleteFiles($this->filters());$this->pdfNotice="Đã xóa {$r['deleted']} PDF trong bộ lọc hiện tại".($r['failed']?" · {$r['failed']} file không xóa được.":'.');}catch(\Throwable $e){$this->pdfError=$e->getMessage();}}
    public function downloadPdfZip(){ $this->authorizePermission('invoices-download');$this->clearPdfMessages();try{$this->fileManager->reconcile($this->filters());$archive=$this->fileManager->createZip($this->filters());$this->pdfNotice="Đã đóng gói {$archive['count']} PDF.";return response()->download($archive['path'],$archive['filename']);}catch(\Throwable $e){$this->pdfError=$e->getMessage();return null;}}

    public function render(){ $filters=$this->filters();$dashboard=$this->invoiceService->dashboard();$filterStats=$this->invoiceService->statistics($filters);$fileSummary=$this->fileManager->summary($filters);$invoices=$this->invoiceService->paginate($filters,$this->perPage);$pdfStatuses=collect($invoices->items())->mapWithKeys(fn($invoice)=>[$invoice->id=>$this->pdfService->statusForInvoice($invoice)])->all();return view('Invoices::livewire.hoadon-list',['invoices'=>$invoices,'pdfStatuses'=>$pdfStatuses,'filterStats'=>$filterStats,'fileSummary'=>$fileSummary,'pdfErrors'=>$this->fileManager->errorDetails($filters),'storageBreakdown'=>$this->fileManager->storageBreakdown($filters),'totalSoldAmount'=>$dashboard['sold_amount'],'totalPurchaseAmount'=>$dashboard['purchase_amount'],'totalSoldCustomers'=>$dashboard['sold_customers'],'totalPurchaseCustomers'=>$dashboard['purchase_customers'],'yearlyRevenue'=>$dashboard['yearly']]);}
    ...
    private function resetListState(bool $refreshOptions=false):void{$this->selected=[];if($refreshOptions)$this->refreshOptions();$this->resetPage();}
    private function refreshOptions():void{$options=$this->invoiceService->filterOptions($this->filters());$this->nameList=$options['names'];$this->taxCodeList=$options['tax_codes'];}
    private function filters():array{return ['invoice_type'=>$this->type,'name'=>$this->name,'tax_code'=>$this->tax_code,'issued_date_from'=>$this->from_date,'issued_date_to'=>$this->to_date,'tax_rate'=>$this->taxRateFilter,'pdf_status'=>$this->pdfStatusFilter,'sort'=>$this->sort];}
    private function normalizedPerPage(mixed $value):int{$value=(int)$value;return in_array($value,$this->perPageOptions,true)?$value:10;}
    ...
    private function clearPdfMessages():void{$this->pdfNotice=null;$this->pdfError=null;}
    private function setBatchMessage(array $result,string $prefix):void{$message="{$prefix}: {$result['downloaded']} · Đã có: {$result['existing']} · Lỗi: {$result['failed']}";$this->applyResultMessage($result,$message);}
    private function applyResultMessage(array $result,string $message):void{if(($result['failed']??0)>0){$detail=$result['errors'][0]??null;$this->pdfError=$detail?$message.' · '.$detail:$message;}else{$this->pdfNotice=$message;}}
}
