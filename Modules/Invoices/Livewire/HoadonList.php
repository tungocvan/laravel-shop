...
class HoadonList extends Component
{
    ...
    public function updatedPerPage(mixed $value): void { $this->perPage=$this->normalizedPerPage($value);$this->clearSelection();$this->resetPage(); }
    public function updatedSelected(): void { $this->selectAllFiltered=false; }
    ...
    private function resetListState(bool $refreshOptions=false): void { $this->clearSelection();if($refreshOptions)$this->refreshOptions();$this->resetPage(); }
    private function refreshOptions(): void { $o=$this->invoiceService->filterOptions($this->filters());$this->nameList=$o['names'];$this->taxCodeList=$o['tax_codes']; }
    ...
    private function setBatchMessage(array $result,string $prefix): void { $this->applyResultMessage($result,"{$prefix}: {$result['downloaded']} · Đã có: {$result['existing']} · Lỗi: {$result['failed']}"); }
