...
class HoadonList extends Component
{
    ...
    protected $queryString = [
        ...
        'pdfStatusFilter', 'year', 'month', 'sort', 'perPage',
    ];

    public function boot(InvoiceService $invoiceService, InvoicePdfService $pdfService, InvoiceFileManagerService $fileManager): void
    {
    ...
    public function updatedFromDate(): void { $this->clearPeriodPreset(); $this->resetListState(true); }
    public function updatedToDate(): void { $this->clearPeriodPreset(); $this->resetListState(true); }
    ...
    public function downloadMissingPdfs(): void
    {
        ...
        try {
            ...
            $this->applyResultMessage($result, $message);
        } catch (\Throwable $exception) {
            $this->pdfError = $exception->getMessage();
        }
    }
    ...
    public function retryPdfErrors(): void
    {
        ...
        try {
            ...
            $this->applyResultMessage($result, "Retry PDF: thành công {$result['downloaded']} · lỗi {$result['failed']}.");
        } catch (\Throwable $exception) {
            $this->pdfError = $exception->getMessage();
        }
    }
