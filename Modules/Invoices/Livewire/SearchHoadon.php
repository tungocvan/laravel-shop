...
class SearchHoadon extends Component
{
    ...
    public function boot(
        ...
        InvoiceFilesEmailBackupService $emailBackupService,
    ): void {
    ...
    public function run(): void
    {
        ...
        try {
            ...
            if ($file === null) {
                ...
                $this->syncMessage = 'Không có hóa đơn trong khoảng thời gian đã chọn. Hệ thống không tạo file Excel.';
                $this->syncFile = null;
                $this->log('ℹ Không có dữ liệu hóa đơn; không tạo file Excel.');
                $this->refreshAvailableFiles();
    ...
    public function pollStatus(): void
    {
        ...
        if (in_array($this->syncState, ['completed', 'partial', 'failed'], true)) {
            $this->refreshAvailableFiles();
        }
    }
    ...
    public function refreshAvailableFiles(): void
    {
        ...
        $this->availableFiles = array_map(function (array $file) {
            unset($file['mtime']);

            return $file;
        }, array_slice($files, 0, 50));

        $availableTokens = array_column($this->availableFiles, 'token');
        $this->selectedFiles = array_values(array_intersect($this->selectedFiles, $availableTokens));
    }
    ...
    public function selectAllAvailableFiles(): void
    {
        $this->selectedFiles = array_values(array_column($this->availableFiles, 'token'));
    }
    ...
    {
        $this->authorizePermission('invoices-create');
        [$direction, , $path] = $this->resolveSingleSelectedFile();
    ...
    {
        $this->authorizePermission('invoices-create');
        [, $filename, $path] = $this->resolveSingleSelectedFile();
        abort_unless(is_readable($path), 404);
    ...
    {
        $this->authorizePermission('invoices-create');
        $this->validate(['selectedFiles' => ['required', 'array', 'min:1', 'max:50']]);

        $deleted = 0;
        $failed = [];

        foreach (array_values(array_unique($this->selectedFiles)) as $token) {
            try {
                [, $filename, $path] = $this->resolveFileToken($token);
            } catch (\Throwable) {
                $failed[] = (string) $token;
                continue;
            }
        ...
        if ($deleted > 0) {
            $this->log("🗑️ Đã xóa {$deleted} file Excel/CSV khỏi kho file. Dữ liệu hóa đơn và RAW canonical không bị xóa.");
        }

        if ($failed !== []) {
            $this->log('⚠ Không thể xóa '.count($failed).' file. Kiểm tra quyền ghi hoặc log hệ thống.');
        }
    }
    ...
    {
        $this->authorizePermission('invoices-create');
        $this->validate(['uploadFile' => ['required', 'file', 'mimes:xlsx,csv', 'max:20480']]);
        $originalName = $this->uploadFile->getClientOriginalName();
        $direction = $this->directionFromFilename($originalName);

        if ($direction === null) {
            $this->addError('uploadFile', $this->invalidFilenameMessage($originalName));

            return;
        }

        $folder = $this->ensureSyncFolder($direction);
        $filename = $this->uniqueFilename($folder, $this->sanitizeFilename($originalName));
        ...
        $this->refreshAvailableFiles();
        $this->selectedFiles = [$direction.'|'.$filename];
        $this->log('📥 Đã nhận dạng '.($direction === 'vat_in' ? 'Mua vào' : 'Bán ra').' và đưa file vào File đã đồng bộ: '.$filename);
    }
    ...
    public function stageGoogleDriveFile(): void
    {
        ...
        $response = Http::connectTimeout(10)->timeout(60)->withOptions(['allow_redirects' => true])->get(
        ...
        if ($direction === null) {
            $this->addError('googleDriveUrl', $this->invalidFilenameMessage($filename).' Hãy đổi tên file trên Google Drive rồi thử lại.');
        ...
        $this->refreshAvailableFiles();
        $this->selectedFiles = [$direction.'|'.$filename];
        $this->log('☁️ Đã nhận dạng '.($direction === 'vat_in' ? 'Mua vào' : 'Bán ra').' và đưa file Google Drive vào File đã đồng bộ: '.$filename);
    }
    ...
    private function runImport(string $path, string $invoiceType): void
    {
        ...
        try {
            $count = $this->importService->import($path, $invoiceType, fn ($message) => $this->log($message));
    ...
    private function resolveSingleSelectedFile(): array
    {
        $this->validate(['selectedFiles' => ['required', 'array', 'size:1']]);

        return $this->resolveFileToken((string) $this->selectedFiles[0]);
    }

    private function resolveFileToken(string $token): array
    {
        [$direction, $filename] = array_pad(explode('|', $token, 2), 2, null);
        ...
        abort_unless($this->directionFromFilename($filename) === $direction, 422, 'Tên file không khớp loại hóa đơn.');
    ...
    private function filenameFromGoogleResponse(?string $contentDisposition, ?string $contentType, string $fileId): string
    {
        ...
        if (! is_string($filename) || $filename === '') {
            ...
            $extension = str_contains($type, 'csv') ? 'csv' : (str_contains($type, 'spreadsheetml') || str_contains($type, 'excel') ? 'xlsx' : null);
            if ($extension === null) {
                throw new RuntimeException('Google Drive không trả về tên hoặc định dạng XLSX/CSV hợp lệ.');
            }
            throw new RuntimeException('Google Drive không trả về tên file gốc nên không thể xác định vat_in_ hoặc vat_out_. Hãy bảo đảm file có tên đúng quy ước và được chia sẻ công khai.');
        }
    ...
    private function authorizePermission(string $permission): void
    {
        ...
        abort_unless($user && method_exists($user, 'can') && $user->can($permission), 403);
    }
