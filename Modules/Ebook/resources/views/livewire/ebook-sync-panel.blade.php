<div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
    <div class="flex flex-col gap-4 border-b border-gray-100 px-5 py-4 xl:flex-row xl:items-center xl:justify-between dark:border-gray-700">
        <div>
            <p class="text-[11px] font-bold uppercase tracking-[0.14em] text-indigo-500">Filesystem Reconciliation</p>
            <h2 class="mt-1 text-base font-bold text-gray-900 dark:text-gray-100">Scan & Sync</h2>
            <p class="mt-1 max-w-3xl text-sm text-gray-500 dark:text-gray-400">Quét filesystem, xem preview và chỉ áp dụng những thay đổi an toàn đã xác nhận.</p>
        </div>
    ...
    <div class="p-5">
        @if (session()->has('ebook_sync_success'))
            <div class="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-300">{{ session('ebook_sync_success') }}</div>
        @endif
        @error('selected') <div class="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900 dark:bg-red-950/30 dark:text-red-300">{{ $message }}</div> @enderror

        @if (! $plan)
            <div class="rounded-2xl border border-dashed border-gray-300 bg-gray-50 px-6 py-12 text-center dark:border-gray-700 dark:bg-gray-900/30">
                <div class="mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-full bg-white text-xl text-gray-500 shadow-sm dark:bg-gray-800 dark:text-gray-300">↻</div>
        ...
        @else
            <div class="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-5">
                @foreach ([['New', $plan['summary']['new'], 'indigo'], ['Changed', $plan['summary']['changed'], 'amber'], ['Missing', $plan['summary']['missing'], 'red'], ['Moved', $plan['summary']['moves'], 'emerald'], ['Ambiguous', $plan['summary']['ambiguous'], 'rose']] as [$label, $count, $tone])
                    <div class="rounded-xl border border-gray-200 bg-gray-50 px-3 py-3 dark:border-gray-700 dark:bg-gray-900/40">
                        <div class="text-[11px] font-bold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ $label }}</div>
                        <div class="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $count }}</div>
                    </div>
                @endforeach
            </div>

            <div class="mb-5 rounded-xl border border-indigo-100 bg-indigo-50/60 px-4 py-3 text-xs leading-5 text-indigo-800 dark:border-indigo-900 dark:bg-indigo-950/20 dark:text-indigo-300">
                <strong>Quy tắc an toàn:</strong> Scan chỉ tạo preview. Apply sẽ re-scan trước khi mutation. Missing và Ambiguous không bị xóa/di chuyển tự động.
            </div>
            ...
            <div class="space-y-5">
                ...
                @foreach ($safeGroups as $group)
                    @if ($group['items'] !== [])
                        <section class="overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700">
                            <div class="border-b border-gray-100 bg-gray-50 px-4 py-3 dark:border-gray-700 dark:bg-gray-900/50">
                            ...
                            <div class="divide-y divide-gray-100 dark:divide-gray-700">
                                @foreach ($group['items'] as $item)
                                    <label class="flex cursor-pointer items-start gap-3 px-4 py-3 transition hover:bg-gray-50 dark:hover:bg-gray-700/40">
                                        ...
                                        <span class="min-w-0 text-sm text-gray-700 dark:text-gray-200">
                                            @if (isset($item['from']))
                                                <span class="block break-all"><span class="font-medium text-gray-500">Từ:</span> {{ $item['from'] }}</span>
                                                <span class="mt-1 block break-all"><span class="font-medium text-gray-500">Đến:</span> {{ $item['to'] }}</span>
                                            @else
                ...
                @if ($plan['missing_folders'] !== [] || $plan['missing_files'] !== [])
                    <section class="rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900 dark:bg-amber-950/20">
                ...
                @if ($plan['ambiguous'] !== [])
                    <section class="rounded-xl border border-red-200 bg-red-50 p-4 dark:border-red-900 dark:bg-red-950/20">
                        <div class="flex items-start gap-3">
                            <span class="mt-0.5">!</span>
                            <div>
                ...
                @if (array_sum($plan['summary']) === 0)
                    <div class="rounded-2xl border border-dashed border-emerald-300 bg-emerald-50 px-6 py-10 text-center dark:border-emerald-900 dark:bg-emerald-950/20">
                        <div class="text-2xl">✓</div>
                        <p class="mt-2 text-sm font-semibold text-emerald-800 dark:text-emerald-300">Filesystem và metadata đang đồng bộ.</p>
                    </div>
                @endif
            </div>
        @endif
