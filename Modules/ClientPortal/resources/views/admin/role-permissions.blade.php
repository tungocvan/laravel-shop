...
@section('title', 'Quyền ứng dụng Role')
@section('content')
<div class="mx-auto max-w-4xl space-y-6">
    <div><a href="{{ route('admin.client-apps.index') }}" class="text-sm font-medium text-blue-600">← Ứng dụng Client</a><h1 class="mt-2 text-2xl font-bold text-gray-900">Phân quyền Application cho Role {{ $role->name }}</h1><p class="mt-1 text-sm text-gray-500">Guard: {{ $role->guard_name }} · Chỉ cập nhật permission namespace client.*</p></div>
    @if(session('success'))<div class="rounded-lg bg-emerald-50 px-4 py-3 text-sm text-emerald-800">{{ session('success') }}</div>@endif
    <form method="POST" action="{{ route('admin.client-apps.roles.update', $role) }}" class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">@csrf @method('PUT')
        ...
        @endforeach
        <div class="mt-6 flex justify-end"><button class="rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white">Lưu phân quyền Role</button></div>
    </form>
</div>
@endsection
