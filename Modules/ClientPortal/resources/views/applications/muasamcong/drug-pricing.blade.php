    ...
    @if($sourceMeta)
        <div class="mt-4 flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2.5 shadow-sm sm:px-4">
            <div class="flex min-w-0 items-center gap-2">
                <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-bold ring-1 ring-inset {{ $sourceMeta['class'] }}"><span class="h-1.5 w-1.5 rounded-full bg-current"></span>{{ $sourceMeta['label'] }}</span>
                <span class="hidden text-xs text-slate-500 sm:inline">Ưu tiên dữ liệu nội bộ để tải nhanh và giảm gọi API.</span>
            </div>
            @if(($dataSource ?? '') !== 'api')
                <a href="{{ route('client.muasamcong.drug-pricing', $refreshQuery) }}" class="inline-flex items-center gap-1.5 rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50" title="Bỏ qua dữ liệu nội bộ và gọi API Mua sắm công"><svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 7h-5V2"/><path d="M4 17h5v5"/><path d="M5.5 9A7 7 0 0 1 17 5l3 2M18.5 15A7 7 0 0 1 7 19l-3-2"/></svg><span class="hidden sm:inline">Tra cứu dữ liệu mới nhất</span><span class="sm:hidden">Mới nhất</span></a>
            @endif
        </div>
    @endif
...
<form id="sync-form" method="POST" action="{{ route('client.muasamcong.drug-pricing.sync') }}" class="mt-4 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
    ...
    <div class="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-3 sm:px-5"><div class="min-w-0"><h2 class="truncate font-bold">Kết quả “{{ $keyword }}”</h2><p class="text-xs text-slate-500">20 dòng/trang</p></div><div class="flex items-center gap-2">@if($canWishlist)<a href="{{ route('client.muasamcong.wishlist') }}" class="rounded-xl border border-rose-200 px-3 py-2 text-xs font-bold text-rose-600">♥ Quan tâm</a>@endif @if($canSync)<button type="submit" class="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-bold text-white sm:text-sm">Đồng bộ đã chọn</button>@endif</div></div>
