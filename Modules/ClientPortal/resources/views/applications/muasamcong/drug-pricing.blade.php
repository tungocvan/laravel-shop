...
<section class="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200 sm:p-7">
    ...
    <form method="GET" action="{{ route('client.muasamcong.drug-pricing') }}" class="mt-6 space-y-4">
        ...
        @if($keyword !== '')
            ...
            <div class="flex flex-wrap gap-2"><button type="submit" class="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold hover:bg-slate-50">Áp dụng bộ lọc</button><a href="{{ route('client.muasamcong.drug-pricing', ['keyword' => $keyword]) }}" class="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-500">Xóa bộ lọc</a></div>
        @endif
        @error('keyword')<p class="text-sm font-medium text-red-600">{{ $message }}</p>@enderror
    </form>
</section>
@if(session('status'))<div class="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800">{{ session('status') }}</div>@endif
@if($keyword !== '')
    @if(!($result['success'] ?? false))
        <div class="mt-6 rounded-3xl border border-amber-200 bg-amber-50 p-5 text-amber-900"><h2 class="font-bold">Chưa thể lấy dữ liệu</h2><p class="mt-1 text-sm">{{ $result['message'] ?? 'Dịch vụ Mua sắm công đang tạm thời không phản hồi.' }}</p></div>
    @else
        ...
        <form id="sync-form" method="POST" action="{{ route('client.muasamcong.drug-pricing.sync') }}" class="mt-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            ...
            <div class="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div><h2 class="font-bold">Kết quả cho “{{ $keyword }}”</h2><p class="mt-1 text-xs text-slate-500">20 dòng/trang · Có thể xem chi tiết từng thuốc trước khi sử dụng hoặc đồng bộ.</p></div>@if($canSync)<button type="submit" class="rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-emerald-700">Đồng bộ đã chọn qua Queue</button>@endif</div>
            @if($items->isEmpty())
                <div class="p-8 text-center text-sm text-slate-500">Không tìm thấy kết quả phù hợp với bộ lọc.</div>
            @else
                <div class="hidden overflow-x-auto lg:block">
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-slate-50 text-left text-xs uppercase tracking-wider text-slate-500"><tr>@if($canSync)<th class="w-10 px-4 py-3"><span class="sr-only">Chọn</span></th>@endif<th class="px-5 py-3">Thuốc</th><th class="px-5 py-3">Hoạt chất / Hàm lượng</th><th class="px-5 py-3">Giá trúng thầu</th><th class="px-5 py-3">Đơn vị trúng thầu</th><th class="px-5 py-3">Mã TBMT</th><th class="px-5 py-3 text-right">Thao tác</th></tr></thead>
                ...
                <div class="divide-y divide-slate-100 lg:hidden">
                    @foreach($items as $item)
                        @php $sourceId=(string)($item['id']??''); $isSynced=in_array($sourceId,$syncedSourceIds,true); @endphp
                        <article class="p-5"><div class="flex items-start gap-3">@if($canSync && $sourceId && !$isSynced)<input type="checkbox" name="selected_ids[]" value="{{ $sourceId }}" class="mt-1 h-4 w-4 shrink-0 rounded border-slate-300">@elseif($canSync && $isSynced)<span class="mt-0.5 text-emerald-600">✓</span>@endif<div class="min-w-0 flex-1"><div class="flex items-start justify-between gap-3"><div><h3 class="font-bold text-slate-950">{{ $item['tenThuoc'] ?? 'Không có tên thuốc' }}</h3><p class="mt-1 text-sm text-slate-600">{{ $item['tenHoatChat'] ?? '—' }} · {{ $item['nongDo'] ?? '—' }}</p></div><div class="shrink-0 text-right font-bold">{{ is_numeric($item['donGia'] ?? null) ? number_format((float)$item['donGia'], 0, ',', '.') . ' đ' : '—' }}</div></div><dl class="mt-4 grid gap-3 text-sm"><div><dt class="text-xs font-semibold uppercase text-slate-400">Đơn vị trúng thầu</dt><dd class="mt-1">{{ implode('; ', array_map('strval', (array)($item['winningName'] ?? []))) ?: '—' }}</dd></div><div class="grid grid-cols-2 gap-3"><div><dt class="text-xs font-semibold uppercase text-slate-400">Mã TBMT</dt><dd class="mt-1">{{ $item['maTbmt'] ?? '—' }}</dd></div><div><dt class="text-xs font-semibold uppercase text-slate-400">Nhóm thuốc</dt><dd class="mt-1">{{ $item['nhomThuoc'] ?? '—' }}</dd></div></div></dl>@if($sourceId)<a href="{{ route('client.muasamcong.drug-pricing.detail', ['sourceId'=>$sourceId,'keyword'=>$keyword]) }}" class="mt-4 inline-flex rounded-xl border border-slate-300 px-3 py-2 text-xs font-bold">Xem chi tiết</a>@endif</div></div></article>
                    @endforeach
                </div>
            @endif
            @if($items->hasPages())<div class="border-t border-slate-200 px-5 py-4">{{ $items->onEachSide(1)->links() }}</div>@endif
        </form>
    @endif
@endif
@endsection
