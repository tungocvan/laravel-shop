...
<div class="min-h-screen pb-20 sm:pb-0">
    <header class="sticky top-0 z-30 border-b border-slate-200/80 bg-white/95 backdrop-blur">
        <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
            <a href="{{ route('client.apps.index') }}" class="flex items-center gap-3">
                ...
                <div><div class="font-bold leading-tight">INAFO</div><div class="text-xs text-slate-500">Client Portal</div></div>
            </a>
            <div class="flex items-center gap-2">
                ...
                <form method="POST" action="{{ route('logout') }}">@csrf<button type="submit" class="rounded-xl bg-slate-900 px-3 py-2 text-sm font-semibold text-white shadow-sm">Đăng xuất</button></form>
            </div>
        </div>
    </header>
    <main class="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
        ...
        @if($applications->isEmpty())
            <div class="rounded-3xl border border-dashed border-slate-300 bg-white p-8 text-center shadow-sm sm:p-12"><h2 class="text-lg font-semibold">Chưa có ứng dụng được cấp</h2><p class="mt-2 text-sm text-slate-500">Quản trị viên cần cấp quyền ứng dụng cho tài khoản này.</p></div>
        @else
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                @foreach($applications as $application)
                    <a href="{{ route($application['route']) }}" class="group rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                        ...
                        <div class="mt-6 flex items-center justify-between text-sm font-semibold"><span>Mở ứng dụng</span><span>→</span></div>
                    </a>
                @endforeach
            </div>
        @endif
