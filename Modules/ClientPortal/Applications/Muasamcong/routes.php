...
use Modules\ClientPortal\Applications\Muasamcong\Http\Controllers\MuasamcongWishlistController;
...
if ((bool) config('modules.registry.Muasamcong.enabled', false)) {
    Route::middleware('web')->group(function () {
        Route::get('/share/muasamcong/drug/{token}', [PublicDrugShareController::class, 'show'])->where('token', '[A-Za-z0-9]{64}')->name('public.muasamcong.drug-share');
    });
    ...
    Route::middleware(['web', 'auth:web', 'client.application:muasamcong'])->prefix('apps/muasamcong')->name('client.muasamcong.')->group(function () {
        ...
        Route::middleware('client.feature:muasamcong,drug-pricing')->group(function () {
            ...
            Route::get('/drug-pricing/sync/{syncRequest}/status', [MuasamcongApplicationController::class, 'drugPricingSyncStatus'])->whereUuid('syncRequest')->name('drug-pricing.sync-status');
            Route::get('/drug-pricing/{sourceId}', [MuasamcongApplicationController::class, 'drugPricingDetail'])->whereUuid('sourceId')->name('drug-pricing.detail');
