...
final class DeleteCompletedRequest
{
    ...
    public function handle(string $publicId, int $actorId): void
    {
        $files = DB::transaction(function () use ($publicId, $actorId): array {
            ...
            if (! in_array($request->status, [RequestStatus::Approved, RequestStatus::Rejected, RequestStatus::Cancelled], true)) {
                throw ValidationException::withMessages(['request' => 'Chỉ có thể xóa đề nghị đã kết thúc.']);
            }

            $id = $request->id;
            $runIds = DB::table('request_runs')->where('request_instance_id', $id)->pluck('id');
            $taskIds = DB::table('request_tasks')->whereIn('request_run_id', $runIds)->pluck('id');
            $files = DB::table('request_attachments')
                ->where('request_instance_id', $id)
                ->get(['storage_disk', 'storage_path'])
                ->map(fn ($file) => [$file->storage_disk, $file->storage_path])
                ->all();

            DB::table('request_instances')->where('id', $id)->update([
                'current_run_id' => null,
            ...
            DB::table('request_attachments')->where('request_instance_id', $id)->delete();
            DB::table('request_comments')->where('request_instance_id', $id)->delete();
            DB::table('request_runs')->whereIn('id', $runIds)->delete();
            DB::table('request_payload_revisions')->where('request_instance_id', $id)->delete();
            DB::table('request_instances')->where('id', $id)->delete();
        ...
        foreach ($files as [$disk, $path]) {
            if ($disk !== 'public' && str_starts_with($path, 'request/attachments/') && ! str_contains($path, '..')) {
