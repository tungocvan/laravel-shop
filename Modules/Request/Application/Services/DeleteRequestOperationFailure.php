...
final class DeleteRequestOperationFailure
{
    ...
    public function handle(string $kind, string $publicId, int $actorId): void
    {
        $stored = DB::transaction(function () use ($kind, $publicId, $actorId): ?array {
            if ($kind === 'outbox_dispatch') {
                ...
                if ($record->failed_at === null) {
                    throw ValidationException::withMessages(['operation' => 'Chỉ xóa được outbox đã thất bại.']);
                }
                ...
                if ($record->status !== ExportStatus::Failed) {
                    throw ValidationException::withMessages(['operation' => 'Chỉ xóa được tác vụ xuất đã thất bại.']);
                }

                $stored = $record->storage_disk && $record->storage_path
                    ? [$record->storage_disk, $record->storage_path]
                    : null;
                $record->delete();
            } else {
                throw ValidationException::withMessages(['operation' => 'Lỗi kích hoạt luồng phải được phục hồi, không được xóa riêng.']);
            }

            $this->audit->append(
                'request_operation',
                $publicId,
                'request.operation.deleted.v1',
                $actorId,
                (string) Str::uuid(),
                ['kind' => $kind],
            );
        ...
        if ($stored && ! str_contains($stored[1], '..')) {
