...
final class RequestNotificationPlanner
{
    ...
    {
        [$request, $recipients, $template] = match ($outbox->event_key) {
    ...
    private function stageActivated(RequestOutboxMessage $outbox): array
    {
        ...
        return [$run->requestInstance, $recipients, 'approval_action_required'];
    }

    private function taskReassigned(RequestOutboxMessage $outbox): array
    {
        $target = $outbox->payload_json['target_user_id'] ?? null;
        $task = RequestTask::query()->with('run.requestInstance')->where('public_id', $outbox->aggregate_public_id)->first();

        return [$task?->run?->requestInstance, is_int($target) ? [$target] : [], 'approval_reassigned'];
    }
    ...
    private function requesterForTask(RequestOutboxMessage $outbox, string $template): array
    {
        ...
        return [$request, $request ? [$request->requester_id] : [], $template];
    }
    ...
    private function collaboration(RequestOutboxMessage $outbox, bool $comment): array
    {
        ...
        $record = is_string($publicId)
            ? ($comment ? RequestComment::query()->where('public_id', $publicId)->first() : RequestAttachment::query()->where('public_id', $publicId)->first())
            : null;
        ...
        return [$request, $recipients, $comment ? 'request_comment_added' : 'request_attachment_added'];
    }
}
