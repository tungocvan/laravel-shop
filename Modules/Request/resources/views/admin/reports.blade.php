...
<div class="mx-auto max-w-7xl space-y-6">
    ...
    <section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-6" aria-label="{{ __('Request::request.reports.status_summary') }}">
        @foreach($statuses as $status)
            <a
                href="{{ route('request.admin.reports', ['status' => $status->value]) }}"
                class="rounded-2xl border p-4 shadow-sm transition {{ $selectedStatus === $status->value ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 bg-white hover:border-indigo-300' }}"
            >
    ...
    <section class="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
        <strong>{{ __('Request::request.reports.export_foundation_title') }}</strong>
        <p class="mt-1">{{ __('Request::request.reports.export_foundation_help') }}</p>
    </section>
</div>
@endsection
