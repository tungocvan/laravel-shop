...
<div class="mx-auto w-full max-w-6xl p-3 sm:p-4 lg:p-6">
    ...
    <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-600">Phiên bản đã phát hành là bất biến. Mỗi thẻ so sánh với phiên bản ngay trước đó nếu có.</p>
        </div>
        ...
        @forelse($versions as $index => $version)
            ...
            <article class="overflow-hidden rounded-xl border border-slate-200 bg-white" aria-labelledby="version-{{ $version->version_number }}-title">
                ...
                <div class="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-4 sm:p-5">
                    <div class="rounded-lg bg-slate-50 p-3"><div class="text-xs text-slate-500">Số phần</div><div class="mt-1 text-xl font-semibold">{{ $sectionCount }}</div>@if($previous)<div class="text-xs text-slate-500">{{ $sectionCount - $previousSectionCount >= 0 ? '+' : '' }}{{ $sectionCount - $previousSectionCount }} so với v{{ $previous->version_number }}</div>@endif</div>
                    <div class="rounded-lg bg-slate-50 p-3"><div class="text-xs text-slate-500">Số trường</div><div class="mt-1 text-xl font-semibold">{{ $fieldCount }}</div>@if($previous)<div class="text-xs text-slate-500">{{ $fieldCount - $previousFieldCount >= 0 ? '+' : '' }}{{ $fieldCount - $previousFieldCount }} so với v{{ $previous->version_number }}</div>@endif</div>
                    <div class="rounded-lg bg-slate-50 p-3"><div class="text-xs text-slate-500">Cấp duyệt</div><div class="mt-1 text-xl font-semibold">{{ $version->stages->count() }}</div>@if($previous)<div class="text-xs text-slate-500">{{ $version->stages->count() - $previous->stages->count() >= 0 ? '+' : '' }}{{ $version->stages->count() - $previous->stages->count() }} so với v{{ $previous->version_number }}</div>@endif</div>
                    <div class="rounded-lg bg-slate-50 p-3"><div class="text-xs text-slate-500">Đối tượng</div><div class="mt-1 text-xl font-semibold">{{ $version->audiences->count() }}</div>@if($previous)<div class="text-xs text-slate-500">{{ $version->audiences->count() - $previous->audiences->count() >= 0 ? '+' : '' }}{{ $version->audiences->count() - $previous->audiences->count() }} so với v{{ $previous->version_number }}</div>@endif</div>
                </div>
                ...
                <details class="border-t border-slate-200">
                    ...
                    <div class="grid gap-4 border-t border-slate-100 p-4 lg:grid-cols-2 sm:p-5">
                        <div class="min-w-0"><h3 class="text-sm font-semibold text-slate-900">Schema biểu mẫu</h3><pre class="mt-2 max-h-96 overflow-auto rounded-lg bg-slate-950 p-3 text-xs text-slate-100"><code>{{ json_encode($version->form_schema_json, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) }}</code></pre></div>
                        <div class="min-w-0"><h3 class="text-sm font-semibold text-slate-900">Các cấp phê duyệt</h3><pre class="mt-2 max-h-96 overflow-auto rounded-lg bg-slate-950 p-3 text-xs text-slate-100"><code>{{ json_encode($version->stages->map->only(['stage_key','name','position','mode','resolver_key','resolver_config_json','allow_reassignment'])->values()->all(), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) }}</code></pre></div>
                    </div>
                </details>
            </article>
        @empty
