...
<div class="mx-auto max-w-5xl space-y-6 p-3 sm:p-4 lg:p-6">
    ...
    <header class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <p class="text-xs font-semibold uppercase tracking-wide text-indigo-600">Quản trị gói định nghĩa</p>
        ...
        <div class="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-3"><div class="text-xs font-semibold uppercase tracking-wide text-emerald-700">Phiên bản hiện hành</div><div class="mt-1 font-bold text-emerald-950">{{ $type->currentPublishedVersion ? 'v'.$type->currentPublishedVersion->version_number : 'Chưa phát hành' }}</div></div>
            <div class="rounded-xl border border-amber-200 bg-amber-50 p-3"><div class="text-xs font-semibold uppercase tracking-wide text-amber-700">Bản nháp hiện tại</div><div class="mt-1 font-bold text-amber-950">{{ $type->activeDraft ? 'v'.$type->activeDraft->version_number : 'Không có' }}</div></div>
            <div class="rounded-xl border border-slate-200 bg-slate-50 p-3"><div class="text-xs font-semibold uppercase tracking-wide text-slate-500">Định dạng</div><div class="mt-1 font-bold text-slate-900">JSON</div></div>
            <div class="rounded-xl border border-slate-200 bg-slate-50 p-3"><div class="text-xs font-semibold uppercase tracking-wide text-slate-500">Giới hạn tải lên</div><div class="mt-1 font-bold text-slate-900">256 KB</div></div>
        </div>
    </header>
    ...
    <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <h2 class="font-bold text-gray-900">Xuất bản phát hành hiện hành</h2>
        <p class="mt-1 text-sm text-gray-600">Tải gói định nghĩa từ phiên bản đã phát hành hiện hành. Dữ liệu runtime không nằm trong gói xuất.</p>
    ...
    <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <h2 class="font-bold text-gray-900">Nhập gói định nghĩa</h2>
        <p class="mt-1 text-sm text-gray-600">Tệp JSON tối đa 256 KB. Luôn kiểm tra trước khi nhập; bước kiểm tra không ghi dữ liệu.</p>
        <div class="mt-3 rounded-xl border border-sky-200 bg-sky-50 p-3 text-sm text-sky-900"><strong>Preview không ghi dữ liệu.</strong> Import chỉ tạo bản nháp mới và không tự phát hành.</div>
    ...
    @if(is_array($preview))
        <section class="rounded-2xl border {{ $preview['valid'] ? 'border-emerald-200 bg-emerald-50' : 'border-red-200 bg-red-50' }} p-5">
            <h2 class="font-bold text-gray-900">Kết quả kiểm tra trước khi nhập</h2>
            <p class="mt-1 text-sm {{ $preview['valid'] ? 'text-emerald-800' : 'text-red-800' }}">{{ $preview['valid'] ? __('Request::definition_package.valid') : __('Request::definition_package.invalid') }}</p>
            @if($previewChecksum)<div class="mt-3 rounded-lg bg-white/80 p-3 text-xs text-slate-600"><strong>Checksum gói:</strong> <code class="break-all">{{ $previewChecksum }}</code></div>@endif
        ...
        @if($preview['valid'] === true && $previewChecksum)
            <section class="rounded-2xl border border-indigo-200 bg-indigo-50 p-5">
                <h2 class="font-bold text-indigo-950">Xác nhận tạo bản nháp mới</h2>
                <p class="mt-1 text-sm text-indigo-800">Checksum của tệp được kiểm tra lại khi nhập. Hãy chọn lại đúng tệp vừa preview; nếu nội dung thay đổi, hệ thống sẽ từ chối.</p>
                <form method="POST" action="{{ route('request.admin.types.package.import', $type->public_id) }}" enctype="multipart/form-data" class="mt-4 space-y-4">@csrf<input type="hidden" name="preview_checksum" value="{{ $previewChecksum }}"><input type="hidden" name="mappings_json" value="{{ $mappingsJson }}"><div><label for="definition-package-confirm" class="mb-1 block text-sm font-semibold text-indigo-950">{{ __('Request::definition_package.package_file') }}</label><input id="definition-package-confirm" type="file" name="package" accept="application/json,.json" required class="block min-h-11 w-full rounded-xl border border-indigo-300 bg-white px-3 py-2 text-sm"></div><button type="submit" class="inline-flex min-h-11 items-center rounded-xl bg-indigo-700 px-4 py-2 text-sm font-semibold text-white">Xác nhận tạo bản nháp mới</button></form>
            </section>
        @endif
    @endif
</div>
