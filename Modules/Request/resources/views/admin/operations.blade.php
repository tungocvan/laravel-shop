            ...
            <div class="divide-y divide-slate-100 md:hidden">
                @foreach($failures as $failure)
                    ...
                    <article class="space-y-4 p-4">
                        ...
                        @if($failure['deletable'] && auth('admin')->user()?->can('request.operation.delete'))<form method="POST" action="{{ route('request.admin.operations.destroy') }}" onsubmit="return confirm('Xóa vĩnh viễn bản ghi lỗi này?')">@csrf @method('DELETE')<input type="hidden" name="kind" value="{{ $failure['kind'] }}"><input type="hidden" name="public_id" value="{{ $failure['public_id'] }}"><button class="min-h-11 w-full rounded-lg border border-rose-300 bg-white px-4 py-2 font-semibold text-rose-700">Xóa lịch sử lỗi</button></form>@endif
                    </article>
                @endforeach
            </div>
            ...
            <div class="hidden overflow-x-auto md:block">
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach($failures as $failure)
                            @php($kindLabel = match ($failure['kind']) { 'stage_activation' => 'Kích hoạt bước xử lý', 'outbox_dispatch' => 'Phân phối outbox', 'export_generation' => 'Tạo tệp xuất', default => $failure['kind'] })
                            <tr><td class="px-4 py-3 font-semibold text-slate-900">{{ $kindLabel }}</td><td class="px-4 py-3 text-slate-700"><span class="break-all">{{ $failure['label'] }}</span></td><td class="px-4 py-3"><code class="rounded bg-slate-100 px-2 py-1 text-xs text-slate-700">{{ $failure['error_code'] ?: '—' }}</code></td><td class="px-4 py-3 text-slate-700">{{ $failure['attempt_count'] }}</td><td class="whitespace-nowrap px-4 py-3 text-slate-600">{{ $failure['updated_at']?->timezone(config('app.timezone'))->format('d/m/Y H:i') ?: '—' }}</td><td class="px-4 py-3 text-right"><div class="flex justify-end gap-2"><form method="POST" action="{{ route('request.admin.operations.retry') }}">@csrf<input type="hidden" name="kind" value="{{ $failure['kind'] }}"><input type="hidden" name="public_id" value="{{ $failure['public_id'] }}"><input type="hidden" name="idempotency_key" value="{{ bin2hex(random_bytes(16)) }}"><button class="min-h-11 rounded-lg border border-indigo-300 bg-white px-4 py-2 font-semibold text-indigo-700">Thử lại</button></form>@if($failure['deletable'] && auth('admin')->user()?->can('request.operation.delete'))<form method="POST" action="{{ route('request.admin.operations.destroy') }}" onsubmit="return confirm('Xóa vĩnh viễn bản ghi lỗi này?')">@csrf @method('DELETE')<input type="hidden" name="kind" value="{{ $failure['kind'] }}"><input type="hidden" name="public_id" value="{{ $failure['public_id'] }}"><button class="min-h-11 rounded-lg border border-rose-300 bg-white px-4 py-2 font-semibold text-rose-700">Xóa</button></form>@endif</div></td></tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
