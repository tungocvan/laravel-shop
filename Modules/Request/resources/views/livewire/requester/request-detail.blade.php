<div class="mx-auto max-w-4xl space-y-5">
        ...
        @if(in_array($request->status->value, ['draft', 'returned'], true))
            <div class="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700" data-request-local-panel aria-live="polite">
                <div class="flex flex-wrap items-center gap-2">
                    <strong>{{ __('Request::request.local_draft') }}</strong>
                    <span data-request-local-status>{{ __('Request::request.local_draft_empty') }}</span>
                    <button type="button" data-request-restore-draft hidden class="ml-auto min-h-10 rounded-lg border border-indigo-300 bg-white px-3 py-2 text-sm font-medium text-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">{{ __('Request::request.local_draft_restore') }}</button>
                </div>
                <p class="mt-1 text-xs text-slate-500">{{ __('Request::request.local_draft_help') }}</p>
            </div>
        @endif
    ...
    <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><h2 class="text-lg font-bold">{{ __('Request::request.timeline') }}</h2><div class="mt-4 space-y-4">@foreach($request->runs as $run)<article class="rounded-xl border border-gray-200 p-4"><div class="flex justify-between gap-3"><strong>{{ __('Request::request.run') }} #{{ $run->sequence_number }}</strong><span>{{ __('Request::request.run_statuses.'.$run->status->value) }}</span></div>@if($run->terminal_reason)<p class="mt-2 text-sm text-gray-700">{{ $run->terminal_reason }}</p>@endif<ul class="mt-3 space-y-2">@foreach($run->tasks as $task)<li class="text-sm"><span class="font-medium">{{ $task->stage_name_snapshot }}</span> · {{ __('Request::request.task_statuses.'.$task->status->value) }}@if($task->decision?->reason)<p class="text-gray-600">{{ $task->decision->reason }}</p>@endif</li>@endforeach</ul></article>@endforeach</div></section>
