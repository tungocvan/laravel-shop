    ...
    <section aria-labelledby="request-content-heading" class="space-y-3">
        ...
        <form wire:submit="save" class="space-y-5" data-request-draft-form="{{ $request->public_id }}" data-request-schema-version="{{ $request->typeVersion?->schema_version ?? 1 }}" data-request-lock-version="{{ $request->lock_version }}" data-request-local-editable="{{ in_array($status, ['draft', 'returned'], true) ? '1' : '0' }}">
            ...
            @foreach((array) ($schema['sections'] ?? []) as $section)
                @php($sectionFields = (array) ($section['fields'] ?? []))
                @php($sectionRequired = collect($sectionFields)->contains(fn (array $field): bool => ($field['required'] ?? false) === true))
                <fieldset class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:p-5">
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 pb-3">
                        <h3 class="text-base font-bold text-gray-900">{{ $section['label'] ?? $section['key'] }}</h3>
                        <span class="rounded-full px-2.5 py-1 text-xs font-semibold {{ $sectionRequired ? 'bg-red-50 text-red-700' : 'bg-slate-100 text-slate-600' }}">{{ $sectionRequired ? 'Có thông tin bắt buộc' : 'Không bắt buộc' }}</span>
                    </div>
                    <div class="mt-4 grid gap-x-4 gap-y-4 sm:grid-cols-2 lg:grid-cols-6">
                        @foreach($sectionFields as $field)
                            @php($key = $field['key'])
                            @php($type = $field['type'])
                            @php($required = ($field['required'] ?? false) === true)
                            @php($width = $field['width'] ?? 'auto')
                            @php($fieldClass = match($width) {
                                'full' => 'sm:col-span-2 lg:col-span-6',
                                'third' => 'sm:col-span-1 lg:col-span-2',
                                'half' => 'sm:col-span-1 lg:col-span-3',
                                default => in_array($type, ['textarea', 'attachment', 'computed_display'], true) ? 'sm:col-span-2 lg:col-span-6' : 'sm:col-span-1 lg:col-span-3',
                            })
                            <div class="{{ $fieldClass }}" data-request-draft-field="{{ $key }}" data-request-field-type="{{ $type }}" data-request-classification="{{ $field['classification'] ?? 'internal' }}" data-request-offline-draft="{{ ($field['offline_draft'] ?? false) === true ? '1' : '0' }}">
                                <label for="request-field-{{ $key }}" class="block text-sm font-medium text-gray-700">{{ $field['label'] ?? $key }} @if($required)<span aria-hidden="true" class="text-red-600">*</span><span class="sr-only">(bắt buộc)</span>@endif</label>
                                @if($type === 'textarea')
                                    <textarea id="request-field-{{ $key }}" wire:model="values.{{ $key }}" rows="3" @if($required) aria-required="true" @endif @disabled(!in_array($status, ['draft', 'returned'], true)) class="mt-1 w-full rounded-xl border border-gray-300 bg-white px-3 py-2.5 text-sm disabled:bg-gray-50"></textarea>
                                @elseif($type === 'boolean')
                                    <input id="request-field-{{ $key }}" wire:model.live="values.{{ $key }}" type="checkbox" @disabled(!in_array($status, ['draft', 'returned'], true)) class="mt-3 h-5 w-5 rounded border-gray-300 text-indigo-600">
                                @elseif(in_array($type, ['select', 'multiselect'], true))
                                    <select id="request-field-{{ $key }}" wire:model.live="values.{{ $key }}" @if($required) aria-required="true" @endif @if($type === 'multiselect') multiple @endif @disabled(!in_array($status, ['draft', 'returned'], true)) class="mt-1 min-h-11 w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm disabled:bg-gray-50"><option value="">{{ __('Request::request.choose') }}</option>@foreach((array) ($field['options'] ?? []) as $option)<option value="{{ is_array($option) ? $option['key'] : $option }}">{{ is_array($option) ? ($option['label'] ?? $option['key']) : $option }}</option>@endforeach</select>
                                @elseif($type === 'currency')
                                    <div class="mt-1 grid grid-cols-3 gap-2"><input data-request-currency-part="amount" wire:model="values.{{ $key }}.amount" type="text" inputmode="decimal" class="col-span-2 min-h-11 rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm" aria-label="{{ __('Request::request.amount') }}"><input data-request-currency-part="currency" wire:model="values.{{ $key }}.currency" type="text" maxlength="3" class="min-h-11 rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm uppercase" aria-label="{{ __('Request::request.currency') }}"></div>
                                ...
                                @endif
                                @if(isset($field['help']))<p class="mt-1 text-xs leading-5 text-gray-500">{{ $field['help'] }}</p>@endif
