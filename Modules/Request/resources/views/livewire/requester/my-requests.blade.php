<div class="space-y-6">
    ...
    <header class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <a href="{{ route($catalogRouteName) }}" class="inline-flex min-h-11 items-center justify-center rounded-xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white">
    ...
    <nav aria-label="Trạng thái đề nghị của tôi" class="flex gap-2 overflow-x-auto pb-1">
        @foreach([
            ...
            <button type="button" wire:click="selectWorkspace('{{ $workspaceKey }}')" @class([
    ...
    <div class="grid gap-4 md:grid-cols-2">
        @forelse($requests as $item)
            ...
            @endphp
            <a href="{{ route($showRouteName, $item->public_id) }}" class="group rounded-2xl border border-gray-200 bg-white p-5 shadow-sm transition hover:border-indigo-300 hover:shadow-md">
                <div class="flex items-start justify-between gap-3"><div class="min-w-0"><div class="font-mono text-xs text-gray-500">{{ $item->request_number }}</div><h2 class="mt-1 truncate font-bold text-gray-900">{{ $item->title_snapshot }}</h2>@if($item->type)<div class="mt-1 text-xs text-gray-500">{{ $item->type->name }}</div>@endif</div><span class="shrink-0 rounded-full px-3 py-1 text-xs font-semibold {{ $statusClasses }}">{{ __('Request::request.statuses.'.$status) }}</span></div>
                <div class="mt-5 flex items-center justify-between gap-3 border-t border-gray-100 pt-4"><span class="text-sm text-gray-500">{{ __('Request::request.updated') }} {{ $item->updated_at->diffForHumans() }}</span><span class="text-sm font-semibold text-indigo-700 group-hover:text-indigo-900">{{ $actionLabel }} →</span></div>
            </a>
        @empty
            <div class="rounded-2xl border border-dashed border-gray-300 bg-white p-8 text-center md:col-span-2"><h2 class="font-semibold text-gray-900">{{ __('Request::request.mine.empty') }}</h2><p class="mt-1 text-sm text-gray-500">{{ __('Request::request.mine.empty_help') }}</p><a href="{{ route($catalogRouteName) }}" class="mt-4 inline-flex min-h-11 items-center justify-center rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white">Tạo đề nghị mới</a></div>
        @endforelse
    </div>
