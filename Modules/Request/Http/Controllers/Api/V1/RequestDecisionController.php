...
final class RequestDecisionController extends Controller
{
    ...
    {
        $validated = $request->validate(['decision' => ['required', 'in:approve,reject,return'], 'reason' => ['nullable', 'string', 'max:2000'], 'expected_request_version' => ['required', 'integer', 'min:1'], 'expected_task_version' => ['required', 'integer', 'min:1']]);
        $task = $query->findActionable($publicId, (int) $request->user()->getAuthIdentifier());
        Gate::forUser($request->user())->authorize('decide', $task);
        $decision = $service->handle($task, DecisionType::from($validated['decision']), $validated['reason'] ?? null, (int) $request->user()->getAuthIdentifier(), (int) $validated['expected_request_version'], (int) $validated['expected_task_version'], (string) $request->header('Idempotency-Key', ''));
