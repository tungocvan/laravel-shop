...
final class ApprovalStageActivator
{
    ...
    public function activate(InternalRequest $request, RequestRun $run, int $position, array $payload, int $actorId, string $correlationId, ?string $keyHash = null): RequestTask
    {
        ...
        if ($stage->mode !== StageMode::Single) {
            throw ValidationException::withMessages(['stage' => ['stage_mode_not_available_in_mr04']]);
        }
        ...
        $task->candidates()->create([
            ...
            'user_snapshot_json' => ['id' => $user->id, 'display_name' => $user->displayName],
            'is_effective' => true,
            'created_at' => $now,
        ]);
        $run->update(['current_stage_position' => $position, 'last_activation_correlation_id' => $correlationId]);
        $this->audit->append('request_instance', $request->public_id, 'request.run.stage_activated.v1', $actorId, $correlationId, ['run_public_id' => $run->public_id, 'stage_position' => $position, 'candidate_count' => 1], $keyHash, $request->id);
        $this->outbox->append('request.run.stage_activated.v1', 'request_instance', $request->public_id, $correlationId, ['run_public_id' => $run->public_id, 'stage_position' => $position]);
