...
class RequestE2EDemoSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $users = [
            'employee' => $this->upsertUser('request.employee@demo.local', 'Nguyễn An · Nhân viên E2E'),
            'approver' => $this->upsertUser('request.approver@demo.local', 'Trần Bình · Quản lý duyệt E2E'),
            'finance' => $this->upsertUser('request.finance@demo.local', 'Lê Chi · Tài chính E2E'),
            'auditor' => $this->upsertUser('request.auditor@demo.local', 'Phạm Dũng · Kiểm toán E2E'),
        ];

        $guard = (string) (Permission::query()
            ->where('name', 'request.dashboard.view')
