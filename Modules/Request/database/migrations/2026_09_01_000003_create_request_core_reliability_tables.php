...
return new class extends Migration
{
    ...
    {
        if (! Schema::hasTable('request_audit_events')) {
        ...
        if (! Schema::hasTable('request_outbox_messages')) {
            Schema::create('request_outbox_messages', function (Blueprint $table): void {
                ...
                $table->timestamps();
                $table->unique('public_id', 'request_outbox_public_id_unique');
                $table->index(['dispatched_at', 'failed_at', 'available_at'], 'request_outbox_dispatch_due_idx');
                $table->index(['aggregate_type', 'aggregate_public_id'], 'request_outbox_aggregate_idx');
            });
        } else {
            Schema::table('request_outbox_messages', function (Blueprint $table): void {
                if (! Schema::hasIndex('request_outbox_messages', ['public_id'], 'unique')) {
                    $table->unique('public_id', 'request_outbox_public_id_unique');
                }

                if (! Schema::hasIndex('request_outbox_messages', ['dispatched_at', 'failed_at', 'available_at'])) {
                    $table->index(['dispatched_at', 'failed_at', 'available_at'], 'request_outbox_dispatch_due_idx');
                }

                if (! Schema::hasIndex('request_outbox_messages', ['aggregate_type', 'aggregate_public_id'])) {
                    $table->index(['aggregate_type', 'aggregate_public_id'], 'request_outbox_aggregate_idx');
                }
            });
        }

        if (! Schema::hasTable('request_idempotency_keys')) {
