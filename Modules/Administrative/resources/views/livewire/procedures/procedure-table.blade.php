<div class="space-y-6">
        ...
        <div class="overflow-x-auto" wire:loading.class="opacity-60">
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                ...
                @forelse($procedures as $procedure)
                    ...
                    <td class="px-4 py-4"><div class="flex justify-end gap-2">@if(auth('admin')->user()?->can('administrative.procedure.update'))<a href="{{ route('admin.administrative.procedures.edit', $procedure->id) }}" class="rounded-lg border border-gray-300 px-3 py-2 font-medium text-gray-700 hover:bg-gray-50">Sửa</a><button wire:click="setActive({{ $procedure->id }}, {{ $procedure->is_active ? 'false' : 'true' }})" wire:loading.attr="disabled" wire:target="setActive" class="rounded-lg border border-gray-300 px-3 py-2 font-medium text-gray-700 disabled:opacity-60">{{ $procedure->is_active ? 'Tạm ngưng' : 'Kích hoạt' }}</button>@endif @if(auth('admin')->user()?->can('administrative.procedure.archive'))<button wire:click="requestArchive({{ $procedure->id }})" class="rounded-lg px-3 py-2 font-medium text-red-600 hover:bg-red-50">Lưu trữ</button>@endif</div></td></tr>
                @empty
    ...
    @if($pendingArchiveId)
        <div class="rounded-2xl border border-red-200 bg-red-50 p-5"><p class="font-semibold text-red-800">Xác nhận lưu trữ thủ tục chưa có hồ sơ?</p><div class="mt-4 flex gap-3"><button wire:click="archive" wire:loading.attr="disabled" wire:target="archive" class="rounded-xl bg-red-600 px-5 py-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60"><span wire:loading.remove wire:target="archive">Xác nhận</span><span wire:loading wire:target="archive">Đang lưu trữ...</span></button><button wire:click="$set('pendingArchiveId', null)" wire:loading.attr="disabled" wire:target="archive" class="rounded-xl border border-gray-300 bg-white px-5 py-3 text-sm font-semibold text-gray-700 disabled:opacity-60">Hủy</button></div></div>
    @endif
</div>
