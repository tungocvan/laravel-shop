...
@section('content')
    ...
    <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-200 bg-slate-50 px-5 py-5 sm:px-7">
            <p class="text-xs font-bold uppercase tracking-wider text-slate-400">Chi tiết kết quả Mua sắm công</p>
            <h1 class="mt-2 text-2xl font-bold text-slate-950">{{ $item['tenThuoc'] ?? 'Không có tên thuốc' }}</h1>
            <p class="mt-2 text-sm text-slate-600">{{ $item['tenHoatChat'] ?? '—' }}{{ !empty($item['nongDo']) ? ' · '.$item['nongDo'] : '' }}</p>
        </div>
        ...
        <div class="grid gap-0 md:grid-cols-2 xl:grid-cols-3">
            ...
            @foreach($fields as $label => $value)
                <div class="border-b border-slate-100 px-5 py-4 md:border-r xl:px-6">
                    <dt class="text-xs font-bold uppercase tracking-wide text-slate-400">{{ $label }}</dt>
                    <dd class="mt-1.5 break-words text-sm font-medium text-slate-800">{{ ($value !== null && $value !== '') ? $value : '—' }}</dd>
                </div>
            @endforeach
        </div>

        @if($canSync && !$synced && !empty($item['id']))
            <div class="border-t border-slate-200 bg-slate-50 px-5 py-5 sm:px-7">
                <form method="POST" action="{{ route('client.muasamcong.drug-pricing.sync') }}">
                    @csrf
                    <input type="hidden" name="keyword" value="{{ $keyword }}">
                    <input type="hidden" name="selected_ids[]" value="{{ $item['id'] }}">
                    <button type="submit" class="rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-emerald-700">Đồng bộ bản ghi này qua Queue</button>
                </form>
            </div>
        @endif
    </section>
