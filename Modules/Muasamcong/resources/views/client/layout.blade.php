...
<head>
    ...
    <title>@yield('title', 'Mua sắm công') · INAFO</title>
    ...
    <header class="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 backdrop-blur">
        <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
            <div class="flex min-w-0 items-center gap-3">
                ...
                <div class="min-w-0">
                    <div class="truncate font-bold">Mua sắm công</div>
                    <div class="truncate text-xs text-slate-500">INAFO Client Application</div>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('client.muasamcong.dashboard') }}" class="hidden rounded-xl px-3 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100 sm:block">Tổng quan</a>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="rounded-xl bg-slate-900 px-3 py-2 text-sm font-semibold text-white">Đăng xuất</button>
                </form>
            </div>
        </div>
    </header>

    <main class="mx-auto max-w-7xl px-4 py-6 pb-24 sm:px-6 sm:py-8 sm:pb-10 lg:px-8">
        @yield('content')
    </main>
