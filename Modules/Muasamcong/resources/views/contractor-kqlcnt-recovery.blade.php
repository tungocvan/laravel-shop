    ...
    <section class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500">Có chọn checkbox: export phần chọn. Không chọn: export toàn bộ lịch sử. TBMT thiếu dữ liệu có nút Bổ sung ngay tại dòng.</p>
            </div>
            <div class="max-w-xl rounded-lg bg-gray-50 px-3 py-2 text-xs leading-5 text-gray-600">
                Ưu tiên <strong>Đồng bộ lại</strong> từ nguồn. Nếu nguồn không còn dữ liệu, chọn <strong>Bổ sung KQLCNT</strong> để tải Excel hiện trạng, chỉnh các ô thiếu rồi import lại đúng TBMT đó.
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('muasamcong.contractors.kqlcnt-recovery.export', $contractorSearch) }}" class="mt-4">
            ...
            <div class="max-h-[620px] overflow-auto rounded-xl border border-gray-200">
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    ...
                    <tr>
                        <th class="px-4 py-3"></th><th class="px-4 py-3">Mã TBMT</th><th class="px-4 py-3">Tên gói</th><th class="px-4 py-3">Chủ đầu tư / BMT</th>
                        <th class="px-4 py-3">Nguồn / Chi tiết</th><th class="px-4 py-3 text-right">Số dòng trúng thầu</th><th class="px-4 py-3">KQLCNT</th><th class="px-4 py-3">Thao tác</th>
                    </tr>
                    </thead>
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                ...
                <button type="submit" formaction="{{ route('muasamcong.contractors.kqlcnt-recovery.enrich', $contractorSearch) }}" class="rounded-xl border border-indigo-300 bg-indigo-50 px-4 py-2.5 text-sm font-semibold text-indigo-700 hover:bg-indigo-100">Đồng bộ chi tiết KQLCNT đã chọn</button>
            </div>
        </form>
    </section>
