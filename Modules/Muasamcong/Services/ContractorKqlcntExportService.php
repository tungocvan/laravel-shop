...
class ContractorKqlcntExportService
{
    ...
    public function download(ContractorSearch $search, array $notifyNos = []): BinaryFileResponse
    {
        ...
        $awardItems = KqlcntAwardItem::query()->where('contractor_code', $search->contractor_code)->whereIn('notify_no', $notifyNos)->orderBy('notify_no')->orderBy('lot_no')->get();
        ...
        $importRows = $awardItems->map(fn ($item): array => [
            ...
            $item->route, $item->dosage_form, $item->unit, $item->quantity !== null ? (float) $item->quantity : null,
            $item->price_plan !== null ? (float) $item->price_plan : null, $item->winning_price !== null ? (float) $item->winning_price : null,
            $item->amount !== null ? (float) $item->amount : null, $item->manufacturer, $item->country, $item->decision_no,
            $item->decision_date?->format('Y-m-d'), $item->published_at?->format('Y-m-d H:i:s'), $item->investor_name,
            $item->contract_no, $item->packaging_spec, $item->shelf_life_months, $item->registration_or_import_license,
            strtoupper($item->source), $item->updated_at?->format('Y-m-d H:i:s'),
        ]);

        $savedRows = $savedCatalog->map(fn (array $row): array => [
            $row['notify_no'] ?? null, $row['contractor_code'] ?? $search->contractor_code, $row['contractor_name'] ?? $search->contractor_name,
            $row['lot_no'] ?? null, $row['lot_name'] ?? null, $row['medicine_code'] ?? null, $row['medicine_name'] ?? null,
            $row['drug_group'] ?? null, $row['active_ingredient'] ?? null, $row['concentration'] ?? null, $row['route'] ?? null,
            $row['dosage_form'] ?? null, $row['unit'] ?? null, $row['quantity'] ?? null, $row['price_plan'] ?? null,
            $row['winning_price'] ?? null, $row['amount'] ?? null, $row['manufacturer'] ?? null, $row['country'] ?? null,
            $row['decision_no'] ?? null, $row['decision_date'] ?? null, $row['published_at'] ?? null, $row['investor_name'] ?? null,
            $row['contract_no'] ?? null, $row['packaging_spec'] ?? null, $row['shelf_life_months'] ?? null,
            $row['registration_or_import_license'] ?? null, $row['source'] ?? 'SAVED', $row['updated_at'] ?? null,
        ]);

        $knownKeys = $importRows->concat($savedRows)->filter(fn (array $row) => trim((string) ($row[3] ?? '')) !== '')
        ...
        $detailRows = $this->mergeLogicalAwardRows($importRows->concat($savedRows)->concat($snapshotRows))
            ->map(fn (array $row): array => $this->enrichDetailRow($this->withCalculatedAmount($row), $records))->values();
        ...
        $overview = collect($notifyNos)->map(function (string $notifyNo) use ($records, $search, $detailRows): array {
            ...
            return [$notifyNo, $search->contractor_code, $search->contractor_name, $record?->bid_name, $record?->investor_name,
                ...
                is_array($record?->contracts) ? count($record->contracts) : 0, $count, $record?->contract_period,
                $record?->contract_period_unit, $record?->contract_period_text, $record?->effect_frame_period,
                strtoupper((string) ($record?->data_source ?: 'unknown')), $record?->synced_at?->format('Y-m-d H:i:s'),
        ...
        $contracts = $records->flatMap(function ($record) use ($detailRows) {
            ...
            return $recordContracts->map(function ($contract) use ($record, $recordDetails, $contractCount): array {
                ...
                return [$record->notify_no, $contract['contractNo'] ?? null, $record->contractor_code,
                    $contract['contractorName'] ?? $contract['newContractorName'] ?? null, $record->investor_name, $amount,
                    $contract['contractEffectiveDate'] ?? $contract['startDate'] ?? null, $contract['endDate'] ?? null,
                    $contract['effectFramePeriod'] ?? $record->effect_frame_period, strtoupper((string) ($record->data_source ?: 'api'))];
            });
        })->values()->all();
        ...
        $sheets = [
            ['title' => 'Tong_quan_KQLCNT', 'headings' => ['Mã TBMT', 'Mã nhà thầu', 'Tên nhà thầu', 'Tên gói thầu', 'Chủ đầu tư / BMT', 'Trạng thái', 'Đã công bố', 'Nhà thầu trúng?', 'Số hợp đồng', 'Số lô/thuốc', 'Thời gian thực hiện', 'Đơn vị thời gian', 'Thời gian thực hiện gói thầu', 'Thời hạn hiệu lực', 'Nguồn dữ liệu', 'Đồng bộ API lúc', 'Import lúc'], 'rows' => $overview],
            ...
            ['title' => 'Hop_dong', 'headings' => ['Mã TBMT', 'Số hợp đồng', 'Mã nhà thầu', 'Tên nhà thầu', 'Chủ đầu tư / BMT', 'Giá trị hợp đồng', 'Ngày hiệu lực', 'Ngày kết thúc', 'Thời hạn hiệu lực', 'Nguồn dữ liệu'], 'rows' => $contracts],
        ...
        return $records->flatMap(function (KqlcntRecord $record) use ($search, $knownKeys): array {
            ...
            foreach ((array) $record->verified_lots as $lot) {
                ...
                $rows[] = [$record->notify_no, $lot['contractorCode'] ?? $lot['winningCode'] ?? $record->contractor_code,
                    ...
                    $lot['lotName'] ?? $lot['medicineName'] ?? $lot['tenThuoc'] ?? null, $lot['medicineCode'] ?? $lot['medicine_code'] ?? $lot['drugCode'] ?? null,
                    $lot['medicineName'] ?? $lot['tenThuoc'] ?? $lot['lotName'] ?? null, $lot['medicineGroup'] ?? $lot['medicine_group'] ?? $lot['groupName'] ?? null,
                    $lot['activeIngredient'] ?? $lot['tenHoatChat'] ?? null, $lot['concentration'] ?? $lot['strength'] ?? $lot['hamLuong'] ?? null,
                    $lot['route'] ?? $lot['routeName'] ?? $lot['duongDung'] ?? null, $lot['dosageForm'] ?? $lot['dosage_form'] ?? $lot['dangBaoChe'] ?? null,
                    $lot['uom'] ?? $lot['unit'] ?? $lot['donViTinh'] ?? null, $quantity, $pricePlan, $winningPrice, $amount,
                    $lot['manufacturer'] ?? $lot['manufacturerName'] ?? $lot['producerName'] ?? null, $lot['country'] ?? $lot['countryName'] ?? $lot['nuocSanXuat'] ?? null,
                    $lot['decisionNo'] ?? $lot['decision_no'] ?? null, $lot['decisionDate'] ?? $lot['decision_date'] ?? null,
                    $lot['publishedAt'] ?? $lot['published_at'] ?? null, $lot['investorName'] ?? $record->investor_name, $lot['contractNo'] ?? $lot['contract_no'] ?? null,
